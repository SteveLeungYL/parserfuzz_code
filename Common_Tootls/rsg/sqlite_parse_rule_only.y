...
create_table_args ::= AS selectandvalue.
table_option_set ::=.
...
ifexists ::= IF EXISTS.
ifexists ::=.
cmd ::= createkw temp VIEW ifnotexists nm dbnm eidlist_opt AS selectandvalue.
cmdCreateView ::= createkw temp VIEW ifnotexists nm dbnm eidlist_opt AS selectandvalue.
...
seltablistnorecursive ::= LP selectandvalue RP as.
seltablist ::= stl_prefix nm dbnm as on_using.
seltablist ::= stl_prefix nm dbnm as indexed_by on_using.
seltablist ::= stl_prefix LP selectandvalue RP as on_using.
...
sortlistnorecursive ::= nm sortorder nulls.
sortlist ::= sortlistnorecursive COMMA nm sortorder nulls.
sortlist ::= expr nm nulls.
sortorder ::= ASC.
sortorder ::= DESC.
sortorder ::=.
nulls ::= NULLS FIRST.
...
cmd ::= with UPDATE orconf xfullname indexed_opt SET setlist fromopt where_opt_ret.
cmdUpdate ::= with UPDATE orconf xfullname indexed_opt SET setlist fromopt where_opt_ret.
...
cmd ::= with insert_cmd INTO xfullname idlist_opt selectandvalue upsert.
cmdInsert ::= with insert_cmd INTO xfullname idlist_opt selectandvalue upsert.
...
expr ::= exprFuncWindow.
exprnorecursive ::= exprFuncWindow.
exprFunc ::= FUNC LP distinct exprlistnorecursive RP.
exprFunc ::= FUNC LP STAR RP.
exprFuncWindow ::= FUNC LP distinct exprlistnorecursive RP filter_over.
...
expr ::= LP selectandvalue RP.
expr ::= exprnorecursive in_op LP selectandvalue RP.
expr ::= EXISTS LP selectandvalue RP.
...
tridxby ::= NOT INDEXED.
trigger_cmd ::= UPDATE orconf trnm tridxby SET setlist fromopt where_opt scanpt.
trigger_cmd ::= scanpt insert_cmd INTO trnm idlist_opt selectandvalue upsert scanpt.
trigger_cmd ::= DELETE FROM trnm tridxby where_opt scanpt.
trigger_cmd ::= scanpt selectandvalue scanpt.
raisetype ::= ROLLBACK.
raisetype ::= ABORT.
raisetype ::= FAIL.
...
wqitem ::= nm eidlist_opt wqas LP selectandvalue RP.
wqlist ::= wqitem.
