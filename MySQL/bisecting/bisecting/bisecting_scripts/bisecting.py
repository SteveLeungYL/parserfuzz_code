...
#        return constants.RESULT.SEG_FAULT, [], []
    ...
    for queries in queries_l:
        all_res_struct, result = mysql.execute_queries(queries, hexsha)
        all_res_struct_l.append(all_res_struct)

    if result == constants.RESULT.SEG_FAULT:
        logger.debug("Commit Segmentation fault. \n")
        return constants.RESULT.SEG_FAULT, [], []

    if not all_res_struct_l or result == constants.RESULT.ALL_ERROR:
    ...
    if oracle_str == "NOREC" or oracle_str == "NoREC":
        ...
        final_flag, all_res_flags = Oracle_NoREC.comp_query_res(all_res_struct_l)
    elif oracle_str == "TLP":
        logger.debug("Using Oracle TLP")
        final_flag, all_res_flags = Oracle_TLP.comp_query_res(all_res_struct_l, queries_l[0])
    elif oracle_str == "OPT":
        logger.debug("Using Oracle OPT")
        final_flag, all_res_flags = Oracle_OPT.comp_query_res(all_res_struct_l)
    else:
        logger.error("Oracle_str: %s not recognized. Using the default NOREC oracle instead. ")
        final_flag, all_res_flags = Oracle_NoREC.comp_query_res(all_res_struct_l)


    return final_flag, all_res_flags, all_res_struct_l
