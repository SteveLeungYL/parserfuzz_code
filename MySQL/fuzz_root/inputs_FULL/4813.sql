...
insert into t1 values  (1, "a", 1.5, 1.6), (1, "a", 1.5, 1.6), (2, "b", 2.5, 2.6), (3, "c", 3.5, 3.6), (4, "d", 4.5, 4.6), (1, "a", 1.5, 1.6), (3, "c", 3.5, 3.6), (1, "a", 1.5, 1.6);
...
set @previous_sql_mode_htnt542nh=@@sql_mode;
set sql_mode=(select replace(@@sql_mode,'ONLY_FULL_GROUP_BY',''));
SELECT MIN(t2.f1), @bar:= (SELECT MIN(t3.f2) FROM t3 WHERE t3.f2 > foo) FROM t1,t2 WHERE t1.f1 = t2.f1 ORDER BY t2.f1;
set @@sql_mode=@previous_sql_mode_htnt542nh;
DROP TABLE t1, t2, t3;
...
set @previous_sql_mode_htnt542nh=@@sql_mode;
set sql_mode=(select replace(@@sql_mode,'ONLY_FULL_GROUP_BY',''));
SELECT @var:=(SELECT f2 FROM t2 WHERE @var) FROM t1 GROUP BY f1 ORDER BY f2 DESC LIMIT 1;
SELECT @var;
set @@sql_mode=@previous_sql_mode_htnt542nh;
DROP TABLE t1, t2;
CREATE TABLE t1(a INT);
...
explain format=json SELECT t1.a, t2.a, t3.a, (@var1:= @var1+0) AS var FROM t1 LEFT JOIN t2 ON t1.a=t2.a AND t2.a < @var1 LEFT JOIN t3 ON t1.a=t3.a AND t3.a < @var1;
...
DROP TABLE numbers;
CREATE TABLE strings (pk INTEGER PRIMARY KEY, vc_ascii VARCHAR(20) COLLATE ascii_general_ci, vc_latin1 VARCHAR(20) COLLATE latin1_general_ci, vc_utf8mb4 VARCHAR(20) COLLATE utf8mb4_0900_ai_ci, vc_utf16 VARCHAR(20) COLLATE utf16_general_ci, vc_utf32 VARCHAR(20) COLLATE utf32_general_ci );
...
INSERT INTO strings VALUES (0, @str_ascii, @str_ascii, @str_ascii, @str_ascii, @str_ascii), (1, @str_ascii, @str_utf8mb4, @str_utf8mb4, @str_utf8mb4, @str_utf8mb4);
