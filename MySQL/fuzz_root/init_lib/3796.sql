...
SELECT 1 INTO @var17727401_1 FROM DUAL INTO @var17727401_2;
...
SELECT 1 INTO @var17727401_1 FROM t1 WHERE 1 GROUP BY 1 HAVING 1 ORDER BY 1 LIMIT 1 INTO @var17727401_2;
...
SELECT 1 AS f                   HAVING EXISTS(SELECT 1 FROM t1) IS TRUE ORDER BY f;
DROP TABLE t1;
...
CREATE TRIGGER trigger1 BEFORE INSERT ON t1 FOR EACH ROW SET default_storage_engine = NEW.INNODB;
DROP TABLE t1;
CREATE TABLE t1 (i INT);
...
EXPLAIN SELECT ( SELECT ROW(t1.b, a) = ROW( ROW(1, t2.c) = ROW(1, d), c) = a FROM t1 ) FROM t2 GROUP BY a;
...
CREATE PROCEDURE p1() BEGIN parse_gcol_expr: LOOP SELECT 1; END LOOP parse_gcol_expr; END;
DROP PROCEDURE p1;
PARSE_GCOL_EXPR (1);
...
ALTER TABLE t1 LOCK=SHARED, ALGORITHM=COPY, LOCK=NONE, ALGORITHM=DEFAULT, LOCK=EXCLUSIVE, ALGORITHM=INPLACE;
ALTER TABLE t1 WITH VALIDATION;
ALTER TABLE t1 WITHOUT VALIDATION;
ALTER TABLE t1 LOCK=SHARED, WITH VALIDATION, ALGORITHM=COPY, LOCK=EXCLUSIVE, WITHOUT VALIDATION, ALGORITHM=INPLACE;
DROP TABLE t1;
CREATE TABLE t1 ( a INT );
INSERT INTO t1 VALUES ( 1 );
...
(SELECT 1 FROM t1 LIMIT 2) UNION (SELECT 2 FROM t1 ORDER BY a LIMIT 1) ORDER BY a;
(SELECT 1 FROM t1 LIMIT 1) UNION ((SELECT 2 FROM t1 ORDER BY a LIMIT 2)) ORDER BY a;
...
(SELECT a FROM t1 LIMIT 1) UNION ALL ((SELECT 2)) ORDER BY t1.b;
(SELECT a FROM t1 LIMIT 1) UNION ALL (SELECT a FROM t1 ORDER BY a LIMIT 2) ORDER BY t1.b;
...
SELECT a FROM t1 UNION SELECT a FROM t1 ORDER BY a UNION SELECT a FROM t1;
(SELECT SQL_CALC_FOUND_ROWS a FROM t1 LIMIT 2) UNION (SELECT a FROM t2 ORDER BY a) LIMIT 2;
DROP TABLE t1, t2, t3;
...
SELECT * FROM t1 NATURAL LEFT JOIN (t2 NATURAL RIGHT JOIN t3);
DROP TABLE t1, t2, t3;
CREATE TABLE t1 ( a INT, b INT, c INT, d INT, e INT, f INT, g INT, h INT, i INT, j INT, k INT, l INT, m INT, n INT, o INT );
...
CREATE TABLE t3( a INT );
CREATE TABLE t4( a INT );
SELECT 1 FROM ( SELECT 1 FROM t1 JOIN t2 ON @@q ) AS d JOIN t3 LEFT JOIN t4 ON 1;
DROP TABLE t1, t2, t3, t4;
...
CREATE PROCEDURE p1() BEGIN DECLARE c CHAR(1) CHARSET DEFAULT; END;
CREATE PROCEDURE p1() BEGIN DECLARE c CHAR(1) COLLATE DEFAULT; END;
...
DROP TABLE t1, t2;
CREATE TABLE t1 ( x VARCHAR(10) COLLATE ascii_bin COLLATE ascii_bin );
CREATE TABLE t2 ( x VARCHAR(10) COLLATE ascii_bin COLLATE ascii_general_ci );
CREATE TABLE t3 ( x VARCHAR(10) COLLATE ascii_bin NOT NULL COLLATE ascii_bin );
CREATE TABLE t3 ( x VARCHAR(10) COLLATE ascii_bin GENERATED ALWAYS AS(NULL) COLLATE ascii_bin );
CREATE TABLE t4 ( x VARCHAR(10) GENERATED ALWAYS AS(NULL) COLLATE ascii_bin COLLATE ascii_bin );
...
EXPLAIN ((SELECT * FROM t3 ORDER BY a LIMIT 3) ORDER BY b DESC LIMIT 2) ORDER BY c LIMIT 1;
DROP VIEW v1;
DROP VIEW v2;
DROP TABLE t1, t2, t3;
