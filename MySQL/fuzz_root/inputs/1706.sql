...
SELECT 1 FROM ( SELECT CONVERT(t2.a USING UTF8) FROM t1, t1 t2 LIMIT 1 ) AS s LIMIT 1;
DROP TABLE t1;
...
SELECT CAST(TIME'23:59:59.123456' AS FLOAT), CAST(TIME'23:59:59.123456' AS DOUBLE);
...
EXPLAIN SELECT MAX(dt_t.d) AS max_d, MAX(n_t.i) AS max_i FROM dt_t, n_t HAVING max_d = max_i;
...
DROP TABLE t1;
CREATE TABLE t(c CHAR(64), v VARCHAR(256), txt TEXT, b BINARY(64), vb VARBINARY(32), e ENUM ("v1", "v2"), set1 SET('101', '102'), bl  BLOB, i INT, si SMALLINT, ti TINYINT, mi MEDIUMINT, bi BIGINT, bt BIT, d DECIMAL, f FLOAT, dbl DOUBLE, dt DATETIME, dd  DATE, t TIME, y YEAR);
INSERT INTO t VALUES ("char", "vchar","text", "binary", "varbinary", "v1", '101,102', "blob",  2001, 2, 3, 4, 200000002, 0x01, 2001.0, 2001.0, 2001.2, "2001-01-02 22:00", "2001-01-02", "20:01", 2010);
...
insert into t values ('2013-03-15 18:35:20', '2013-03-15', '18:35:20','L'), ('2003-01-10 00:00:23', '2003-01-10', '00:00:23', NULL);
...
CREATE TABLE `CC` ( `pk` int, `col_datetime_key` datetime, KEY `idx_CC_col_datetime_key` (`col_datetime_key`));
...
SELECT CAST('YYYY-MM-DD' AS DATE) AS d1, CAST(@bad_date AS DATE) AS d2, CAST('YYYY-MM-DD HH:MM:SS.ffffff' AS DATETIME(6)) AS dt1, CAST(@bad_datetime AS DATETIME(6)) AS dt2;
SET SQL_MODE=DEFAULT;
CREATE TABLE t AS SELECT CONCAT(CAST(-1 AS UNSIGNED)) AS col1, 1.0 + CAST(-1 AS UNSIGNED) AS col2, CONCAT(CAST(9223372036854775808 AS SIGNED)) AS col3;
SHOW CREATE TABLE t;
SELECT * FROM t;
DROP TABLE t;
