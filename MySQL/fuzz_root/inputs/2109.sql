...
drop table t3;
explain select * from t2, t1, lateral (select t1.a) as dt, lateral (select t2.a) as dt2;
select * from t2, t1, lateral (select t1.a) as dt, lateral (select t2.a) as dt2;
explain select * from t2, t1, lateral (select t1.a) as dt, lateral (select t1.a+1) as dt2;
explain format=json select * from t2, t1, lateral (select t1.a) as dt, lateral (select t1.a+1) as dt2;
...
ANALYZE TABLE t11,t12;
explain SELECT * FROM t11 LEFT JOIN t12 force index (idx) ON t12.t12b = t11.t11b JOIN LATERAL (SELECT t12a) dt;
flush status;
...
analyze table t3;
explain select * from t1, lateral (select t3.b from t3 where t3.a=t1.a) dt where dt.b=t1.a+9;
...
DROP TABLE t1, t2;
CREATE TABLE t1 ( pk INTEGER, col_int INT not null, col_int_key INT not null, col_time_gckey TIME, col_varchar VARCHAR(20) not null, col_varchar_key VARCHAR(15)  not null );
CREATE TABLE t2 ( pk INTEGER, col_int INT not null, col_varchar VARCHAR(20) not null, col_varchar_key VARCHAR(15) not null );
SET OPTIMIZER_SWITCH='derived_merge=off';
SET OPTIMIZER_SWITCH=DEFAULT;
DROP TABLE t1,t2;
CREATE TABLE t1 ( pk INTEGER, col_int_key INTEGER NOT NULL, col_date_key DATE NOT NULL, col_datetime DATETIME NOT NULL );
...
DROP TABLE t1;
CREATE TABLE t0007 ( c0008 date NOT NULL, c0009 char(234) NOT NULL );
CREATE TABLE t0008 ( c0005 tinytext NOT NULL );
CREATE TABLE t0009 ( c0000 time NOT NULL );
SET SQL_MODE=0;
SELECT (SELECT t0007.c0009         FROM (SELECT t0007.c0008 AS c0003 FROM t0009                   ) AS t0005                 ) FROM t0007 GROUP BY -23;
SELECT (SELECT c0009 FROM (SELECT 1 AS c0003 FROM t0009 INNER JOIN t0008 ON t0008.c0005 WHERE t0007.c0008 ) AS t0005 GROUP BY c0008 ), COUNT(c0009) FROM t0007 GROUP BY 1, 1;
...
SELECT * FROM t1 LEFT JOIN ( t3, LATERAL ( SELECT * FROM t2 WHERE table_id = t3.id ) l1 ) ON TRUE and t3.id=363;
set optimizer_switch=DEFAULT;
DROP TABLE t1, t2, t3;
