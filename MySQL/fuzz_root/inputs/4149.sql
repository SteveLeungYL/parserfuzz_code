...
DROP TABLE t1,t2;
CREATE TABLE t1( a int, b int, c int, d int, e int, f int, g int, h int, PRIMARY KEY (a,b,c,d,e,f,g) );
...
DROP TABLE t1;
create table t1 (a varchar(200), b int unsigned not null primary key auto_increment) default character set 'utf8';
create table t2 (c varchar(200), d int unsigned not null primary key auto_increment) default character set 'latin1';
...
SELECT 1 FROM t1 WHERE ROW(a, b) >= ROW('1', (SELECT 1 FROM t1 WHERE a > '1234abc'));
DROP TABLE t1;
...
EXPLAIN SELECT row( min(a), 1 ) <= row( a, 1 ) AS al FROM t1 GROUP BY a;
SELECT row( min(a), 1 ) <= row( a, 1 ) AS al FROM t1 GROUP BY a;
EXPLAIN SELECT row( row( min(a), 1 ), 1 ) = row( row( a, 1 ), 1 ) AS al FROM t1 GROUP BY a;
SELECT row( row( min(a), 1 ), 1 ) = row( row( a, 1 ), 1 ) AS al FROM t1 GROUP BY a;
...
CREATE ALGORITHM = TEMPTABLE VIEW v3 AS SELECT * FROM v0 WHERE (v1, FALSE) IN ((-1, EXISTS (SELECT (-128, 'x', 72) <= (76, 'x', -1))), (-1, 'x'));
DROP VIEW v3;
DROP TABLE v0;
