...
SELECT COUNT(*) FROM mysql.password_history WHERE User='def_interval' AND Host='localhost';
SET PASSWORD FOR def_interval@localhost = 'haha';
UPDATE mysql.password_history SET Password_timestamp = TIMESTAMPADD(DAY, -1, Password_timestamp) WHERE User='def_interval' AND Host='localhost';
SET PASSWORD FOR def_interval@localhost = 'haha';
SELECT COUNT(*) FROM mysql.password_history WHERE User='def_interval' AND Host='localhost' AND TO_DAYS(Password_timestamp)=TO_DAYS(NOW());
SET PASSWORD FOR def_interval@localhost = 'hihi';
SELECT COUNT(*) FROM mysql.password_history WHERE User='def_interval' AND Host='localhost' AND TO_DAYS(Password_timestamp)=TO_DAYS(NOW());
UPDATE mysql.password_history SET Password_timestamp = TIMESTAMPADD(MONTH, -1, Password_timestamp) WHERE User='def_interval' AND Host='localhost';
...
DROP USER mohit@localhost;
CREATE USER mohit_sha@localhost IDENTIFIED WITH sha256_password BY 'mohit_sha'   PASSWORD HISTORY 1;
...
SET GLOBAL password_history=0;
SET GLOBAL password_reuse_interval= 1;
CREATE USER amar@localhost IDENTIFIED BY 'amar'   PASSWORD REUSE INTERVAL 1 DAY;
SELECT COUNT(*) FROM mysql.password_history WHERE User='amar' AND Host='localhost';
ALTER USER amar@localhost IDENTIFIED BY 'amar';
SELECT COUNT(*) FROM mysql.password_history WHERE User='amar' AND Host='localhost';
DROP USER amar@localhost;
SET GLOBAL password_history=default;
SET GLOBAL password_reuse_interval=default;
CREATE USER mohit_sha@localhost IDENTIFIED WITH caching_sha2_password BY 'mohit_sha'   PASSWORD HISTORY 1;
ALTER USER mohit_sha@localhost IDENTIFIED BY 'mohit_sha';
ALTER USER mohit_sha@localhost IDENTIFIED BY 'mohit_sha2';
