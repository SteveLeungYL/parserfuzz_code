...
EXPLAIN SELECT (SELECT SUM(LENGTH(c)) FROM t1 WHERE c='13_characters') FROM t1;
...
INSERT INTO t2 (d) SELECT (SELECT SUM(LENGTH(c)) FROM t1 WHERE c IN (SELECT t1.c FROM t1))  FROM t1;
SELECT * FROM t2;
DROP TABLE t1,t2;
CREATE TABLE t1 ( id INT AUTO_INCREMENT PRIMARY KEY, prev_id INT, join_id INT DEFAULT 0);
INSERT INTO t1 (prev_id) VALUES (NULL), (1), (2);
SELECT * FROM t1;
CREATE TABLE t2 (join_id INT);
INSERT INTO t2 (join_id) VALUES (0);
INSERT INTO t1 (prev_id) SELECT id FROM t2 LEFT JOIN t1 ON t1.join_id = t2.join_id ORDER BY id DESC LIMIT 1;
SELECT * FROM t1;
DROP TABLE t1,t2;
CREATE TABLE t1 ( id INT AUTO_INCREMENT PRIMARY KEY, a VARCHAR(100) CHARACTER SET ARMSCII8 );
...
INSERT INTO t1 SELECT MAX(id) FROM t2 GROUP BY id ORDER BY id ON DUPLICATE KEY UPDATE t1.id = id + 10;
