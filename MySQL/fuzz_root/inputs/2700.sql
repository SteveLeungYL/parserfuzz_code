...
create algorithm=merge view v1a as select * from t1 natural join t2;
create algorithm=merge view v1b(a,b,c) as select * from t1 natural join t2;
create algorithm=merge view v1c as select b as a, c as b, a as c from t1 natural join t2;
create algorithm=merge view v1d(b, a, c) as select a as c, c as b, b as a from t1 natural join t2;
create algorithm=merge view v2a as select t1.c, t1.b, t2.a from t1 join (t2 join t4 on b + 1 = y) on t1.c = t4.c;
create algorithm=merge view v2b as select t1.c as b, t1.b as a, t2.a as c from t1 join (t2 join t4 on b + 1 = y) on t1.c = t4.c;
create algorithm=merge view v3a as select * from t1 natural join t2 natural join t3;
create algorithm=merge view v3b as select * from t1 natural join (t2 natural join t3);
create algorithm=merge view v4 as select * from v2a natural join v3a;
...
CREATE TABLE tv1 SELECT Description AS Name FROM v1 JOIN t2 USING (Test_ID);
DESCRIBE tv1;
CREATE TABLE tv2 SELECT Description AS Name FROM v1 JOIN t2 ON v1.Test_ID = t2.Test_ID;
DESCRIBE tv2;
DROP VIEW v1;
...
insert into t2  select @a:= A.a + 10*(B.a + 10*C.a), @a, 'filler' from t1 A, t1 B, t1 C;
...
EXPLAIN SELECT *  FROM  t1 JOIN t2 ON t1.a = t2.a  LEFT JOIN  ( ( t3 LEFT JOIN t4 ON t3.a = t4.a )  LEFT JOIN  ( t5 LEFT JOIN t6 ON t5.a = t6.a )  ON t4.a = t5.a )  ON t1.a = t3.a;
SELECT *  FROM  t1 JOIN t2 ON t1.a = t2.a  LEFT JOIN  ( ( t3 LEFT JOIN t4 ON t3.a = t4.a )  LEFT JOIN  ( t5 LEFT JOIN t6 ON t5.a = t6.a )  ON t4.a = t5.a )  ON t1.a = t3.a;
DROP TABLE t1,t2,t3,t4,t5,t6;
CREATE TABLE t1(f1 INT);
INSERT INTO t1 VALUES (1),(2);
CREATE VIEW v1 AS SELECT 1 FROM t1 LEFT JOIN t1 AS t2 on 1=1;
EXPLAIN SELECT 1 FROM v1 right join v1 AS v2 ON RAND();
DROP VIEW v1;
DROP TABLE t1;
CREATE TABLE t1 (a TINYINT, b TEXT, KEY (a));
INSERT INTO t1 VALUES (0,''),(0,'');
FLUSH TABLES;
EXPLAIN SELECT 1 FROM t1 LEFT JOIN t1 a ON 1 WHERE ROW(t1.a, 1111.11) = ROW(1111.11, 1111.11) AND ROW(t1.b, 1111.11) <=> ROW('','');
DROP TABLE t1;
...
CREATE TABLE t2 ( pk INTEGER NOT NULL, c1 INTEGER NOT NULL, PRIMARY KEY (pk) );
INSERT INTO t2 VALUES (1,4),(3,5),(2,6);
ANALYZE  TABLE t1, t2;
EXPLAIN SELECT t2.pk, t2.c1 FROM t2, t1  WHERE t2.pk = t1.c1 AND t2.pk >= 2;
SELECT t2.pk, t2.c1 FROM t2, t1  WHERE t2.pk = t1.c1 AND t2.pk >= 2;
CREATE VIEW v_t2 AS SELECT * FROM t2;
EXPLAIN SELECT v_t2.pk, v_t2.c1 FROM v_t2, t1  WHERE v_t2.pk = t1.c1 AND v_t2.pk >= 2;
SELECT v_t2.pk, v_t2.c1 FROM v_t2, t1  WHERE v_t2.pk = t1.c1 AND v_t2.pk >= 2;
DROP VIEW v_t2;
DROP TABLE t1, t2;
CREATE TABLE t1 (   pk INTEGER NOT NULL, i1 INTEGER NOT NULL, i2 INTEGER NOT NULL, PRIMARY KEY (pk) );
INSERT INTO t1 VALUES (7,8,1), (8,2,2);
CREATE VIEW v1 AS SELECT * FROM t1;
set @previous_sql_mode_htnt542nh=@@sql_mode;
set sql_mode=(select replace(@@sql_mode,'ONLY_FULL_GROUP_BY',''));
EXPLAIN SELECT t1.pk FROM v1, t1 WHERE v1.i2 = 211 AND v1.i2 > 7 OR t1.i1 < 3 GROUP BY t1.pk ORDER BY v1.i2;
SELECT t1.pk FROM v1, t1 WHERE v1.i2 = 211 AND v1.i2 > 7 OR t1.i1 < 3 GROUP BY t1.pk ORDER BY v1.i2;
set @@sql_mode=@previous_sql_mode_htnt542nh;
EXPLAIN SELECT t1.pk FROM v1, t1 WHERE (v1.i2 = 211 AND v1.i2 > 7) OR (t1.i1 < 3 AND v1.i2 < 10);
SELECT t1.pk FROM v1, t1 WHERE (v1.i2 = 211 AND v1.i2 > 7) OR (t1.i1 < 3 AND v1.i2 < 10);
DROP VIEW v1;
DROP TABLE t1;
...
EXPLAIN SELECT * FROM t4 JOIN (t1 join t3 on t3.ref_t1 = t1.c1 join t2 on t2.ref_t1 = t1.c1) on t4.ref_t1 = t1.c1;
EXPLAIN SELECT STRAIGHT_JOIN * FROM t4 JOIN (t1 join t3 on t3.ref_t1 = t1.c1 join t2 on t2.ref_t1 = t1.c1) on t4.ref_t1 = t1.c1;
EXPLAIN SELECT * FROM t4 STRAIGHT_JOIN (t1 join t3 on t3.ref_t1 = t1.c1 join t2 on t2.ref_t1 = t1.c1) on t4.ref_t1 = t1.c1;
drop table t1,t2,t3,t4;
CREATE TABLE t1(a INTEGER) engine=innodb;
explain SELECT 1 FROM (SELECT 1 FROM t1 WHERE a) AS q NATURAL LEFT JOIN t1 NATURAL LEFT JOIN t1 AS t2;
SELECT 1 FROM (SELECT 1 FROM t1 WHERE a) AS q NATURAL LEFT JOIN t1 NATURAL LEFT JOIN t1 AS t2;
explain SELECT 1 FROM t1 NATURAL RIGHT JOIN t1 AS t2 NATURAL RIGHT JOIN (SELECT 1 FROM t1 WHERE a) AS q;
SELECT 1 FROM t1 NATURAL RIGHT JOIN t1 AS t2 NATURAL RIGHT JOIN (SELECT 1 FROM t1 WHERE a) AS q;
DROP TABLE t1;
CREATE TABLE t1 (pk INTEGER, dummy VARCHAR(64), col_check TINYINT, PRIMARY KEY(pk) ) engine=innodb;
INSERT INTO t1 VALUES (13, '13', 13);
CREATE VIEW v1 AS SELECT * FROM t1 WHERE pk BETWEEN 13 AND 14;
...
DROP VIEW v1;
DROP TABLE t1;
CREATE TABLE t1 (pk INT, col_int_key INT DEFAULT NULL, col_varchar_10_latin1_key VARCHAR(10) DEFAULT NULL, col_varchar_255_utf8 VARCHAR(255) CHARACTER SET utf8 DEFAULT NULL, col_int INT DEFAULT NULL, col_datetime_key datetime DEFAULT NULL, col_varchar_255_latin1_key VARCHAR(255) DEFAULT NULL, col_date_key date DEFAULT NULL, col_datetime datetime DEFAULT NULL, PRIMARY KEY (pk), KEY col_date_key (col_date_key) ) charset latin1 ENGINE=MyISAM;
INSERT INTO t1 VALUES (8,9,'h','FDUMQ',-1356726272,'2007-09-02 05:48:23','she','2002-04-02','2002-12-20 17:54:07');
CREATE TABLE t2 (pk INT, col_int INT DEFAULT NULL, col_int_key INT DEFAULT NULL, PRIMARY KEY (pk) ) charset latin1 ENGINE=MyISAM;
CREATE TABLE t3 (col_int INT DEFAULT NULL, col_int_key INT DEFAULT NULL, pk INT, PRIMARY KEY (pk), KEY test_idx (col_int_key,pk,col_int) ) charset latin1 ENGINE=InnoDB;
INSERT INTO t3 VALUES (NULL,9, 41), (NULL,-1596719104, 48), (-1068105728,9, 49);
CREATE TABLE t4 (col_varchar_255_latin1_key VARCHAR(255) DEFAULT NULL, pk INT, PRIMARY KEY (pk) ) charset latin1 ENGINE=MyISAM;
INSERT INTO t4 VALUES ('RUXDY',8);
ANALYZE TABLE t1, t2, t3, t4;
explain SELECT alias1.pk AS field5 FROM t3 AS alias1 LEFT JOIN t4 AS alias2 LEFT JOIN t4 AS alias3 LEFT JOIN t1 AS alias4 LEFT JOIN t2 AS alias5 ON alias4.pk = alias5.col_int_key ON alias3.pk = alias5.col_int ON alias2.col_varchar_255_latin1_key = alias4.col_varchar_10_latin1_key RIGHT JOIN t1 AS alias6 ON alias4.pk = alias6.pk RIGHT JOIN t4 AS alias7 ON alias6.pk = alias7.pk ON alias1.col_int_key = alias6.col_int_key WHERE alias1.col_int > 5 OR alias5.col_int > 5;
SELECT alias1.pk AS field5 FROM t3 AS alias1 LEFT JOIN t4 AS alias2 LEFT JOIN t4 AS alias3 LEFT JOIN t1 AS alias4 LEFT JOIN t2 AS alias5 ON alias4.pk = alias5.col_int_key ON alias3.pk = alias5.col_int ON alias2.col_varchar_255_latin1_key = alias4.col_varchar_10_latin1_key RIGHT JOIN t1 AS alias6 ON alias4.pk = alias6.pk RIGHT JOIN t4 AS alias7 ON alias6.pk = alias7.pk ON alias1.col_int_key = alias6.col_int_key WHERE alias1.col_int > 5 OR alias5.col_int > 5;
DROP TABLE t1, t2, t3, t4;
...
explain format=json select straight_join count(*) from parent join eq_child as c1 on c1.a = parent.b and c1.b = parent.b join eq_child as c2 on c2.a = parent.b and c2.b = parent.b join eq_child as c3 on c3.a = parent.b and c3.b = parent.b join eq_child as c4 on c4.a = parent.b and c4.b = parent.b ;
...
explain format=tree select t1.a, (select count(*) from t t2 left join t t3 on t3.a>t2.a-t1.a) from t t1 group by t1.a;
select t1.a, (select count(*) from t t2 left join t t3 on t3.a>t2.a-t1.a) from t t1 group by t1.a;
create view v1 as select * from t;
explain format=tree select v1.*, (select count(*) from t t2 left join t t3 on t3.a>t2.a-v1.a) from v1;
select v1.*, (select count(*) from t t2 left join t t3 on t3.a>t2.a-v1.a) from v1;
drop view v1;
explain format=tree select t1.*, (select * from (select count(*) from t t2 left join t t3 on t3.a>t2.a-t1.a) as dt) from t t1;
select t1.*, (select * from (select count(*) from t t2 left join t t3 on t3.a>t2.a-t1.a) as dt) from t t1;
explain format=tree select t1.*, dt.c from t t1, (select count(*) as c from t t2 left join t t3 on t3.a>t2.a-t1.a) as dt;
...
explain format=tree select t1.*, dt.c from t t1 cross join lateral (select count(*) as c from t t2 left join t t3 on t3.a>t2.a-t1.a) as dt;
select t1.*, dt.c from t t1 cross join lateral (select count(*) as c from t t2 left join t t3 on t3.a>t2.a-t1.a) as dt;
explain format=tree select t1.*, dt.c from t t1 join lateral (select count(*) as c from t t2 left join t t3 on t3.a>t2.a-t1.a) as dt on true;
...
explain format=tree select * from t t1 where t1.a in (select dt.a from t t2 left join (select * from t t3 where t3.a>t1.a) dt on true);
select * from t t1 where t1.a in (select dt.a from t t2 left join (select * from t t3 where t3.a>t1.a) dt on true);
drop table t;
set optimizer_switch=default;
set global default_storage_engine=innodb;
CREATE TABLE t1 ( a INTEGER );
CREATE TABLE t2 ( pk INTEGER NOT NULL, a INTEGER );
SELECT * FROM ( t2 LEFT JOIN t2 AS t3 ON t2.pk IS NULL ) LEFT JOIN t1 ON t1.a = t3.a;
DROP TABLE t1, t2;
...
DROP TABLE A;
CREATE TABLE t ( a BIT(50), b VARCHAR(22772) character set ucs2, c INTEGER, d TINYBLOB, PRIMARY KEY (a), KEY i0001 (c,d(163)) );
INSERT INTO t VALUES (1,'',1,'1');
INSERT INTO t VALUES (0x03ffffffffffff,'2',0,'2');
SELECT t2.b FROM t AS t1 LEFT JOIN t AS t2 ON t2.c < 70 WHERE t2.a - from_unixtime(0) > 0;
DROP TABLE t;
CREATE TABLE t1 ( ts datetime, x integer, y integer );
INSERT INTO t1 VALUES ('2020-11-20 10:38:31',1,2);
CREATE TABLE t2 ( x integer, ts datetime, KEY i1 (x) );
INSERT INTO t2 VALUES (1,'2020-11-16 14:18:55');
CREATE TABLE t3 ( ts datetime, y integer, z INTEGER, KEY i2 (y,ts) );
INSERT INTO t3 VALUES ('2020-12-29 18:23:02',2,100);
CREATE TABLE t4 ( z INTEGER );
SET optimizer_switch='mrr=on,batched_key_access=on,mrr_cost_based=off';
SELECT COUNT(*) FROM t1 LEFT JOIN t2 ON t1.x = t2.x AND t2.ts <= t1.ts JOIN t3 ON t1.y = t3.y AND t3.ts <= t1.ts LEFT JOIN t4 ON t3.z = t4.z;
SET optimizer_switch=DEFAULT;
DROP TABLE t1, t2, t3, t4;
CREATE TABLE t1 ( a INTEGER );
INSERT INTO t1 VALUES (0), (0), (0), (1), (1), (0), (1), (0);
ANALYZE TABLE t1;
SELECT * FROM t1, t1 AS t2, t1 AS t3 WHERE ((t1.a = 5 AND t3.a < t1.a) OR t1.a > 0) AND t3.a <= t2.a;
DROP TABLE t1;
CREATE TABLE t1 ( a VARCHAR(10) );
CREATE TABLE t2 ( a VARCHAR(10) );
INSERT INTO t1 VALUES ('x');
ANALYZE TABLE t1, t2;
SELECT * FROM t1 LEFT JOIN t2 ON TRUE LEFT JOIN ( t1 AS t1i JOIN t2 AS t2i ON t1i.a = t2i.a ) ON t2.a = t2i.a WHERE t1i.a = '' OR t2i.a = '';
DROP TABLE t1, t2;
CREATE TABLE t1 ( a INTEGER NOT NULL, KEY (a) );
INSERT INTO t1 VALUES (1), (2);
ANALYZE TABLE t1;
SELECT * FROM t1 LEFT JOIN t1 AS t2 ON TRUE WHERE t1.a IS NULL OR t1.a = t2.a ORDER BY t2.a;
SELECT t1.a FROM t1 LEFT JOIN t1 AS t2 ON t1.a = t2.a WHERE t1.a IS NULL OR t1.a = t2.a GROUP BY t1.a;
DROP TABLE t1;
SELECT * FROM t1 NATURAL JOIN ( SELECT t2.* FROM t1 AS t2 JOIN t1 AS t3 ON t3.c1 = t2.c1 WHERE t2.c3 <> ANY (SELECT c3 FROM t1) ) AS d1;
DROP TABLE t1;
CREATE TABLE t1 ( a INTEGER );
SELECT * FROM t1 AS outer_t1 WHERE EXISTS ( SELECT t2.a FROM t1 LEFT JOIN t1 AS t2 ON t2.a = outer_t1.a );
DROP TABLE t1;
CREATE TABLE t1 ( a INTEGER );
SELECT * FROM (t1 STRAIGHT_JOIN t1 AS t2) JOIN t1 AS t3 ON t1.a = t3.a WHERE t2.a = t3.a ORDER BY t1.a;
DROP TABLE t1;
CREATE TABLE t1 (a INTEGER, b INTEGER);
SELECT * FROM (t1 STRAIGHT_JOIN t1 AS t2 ON t1.b = t2.b) JOIN t1 AS t3 ON t1.a = t3.b WHERE t1.a < t2.b AND t2.a < t3.b ORDER BY t1.b;
DROP TABLE t1;
CREATE TABLE t1 ( a INTEGER );
SELECT * FROM t1 LEFT JOIN t1 AS t2 ON t1.a = t2.a LEFT JOIN t1 AS t3 ON t2.a = t3.a WHERE t2.a = t3.a OR t2.a <> t2.a;
DROP TABLE t1;
CREATE TABLE t1 ( a INTEGER, b INTEGER );
INSERT INTO t1 VALUES (1,2);
ANALYZE TABLE t1;
...
DROP TABLE t1;
CREATE TABLE t1 ( pk INTEGER );
SELECT * FROM t1 JOIN t1 AS t2 ON t1.pk = t2.pk JOIN t1 AS t3 LEFT JOIN t1 AS t4 ON t2.pk = t3.pk AND t4.pk = 135;
DROP TABLE t1;
CREATE TABLE t1 ( a INTEGER, b INTEGER );
SELECT * FROM t1 JOIN t1 AS t2 ON t1.a = t2.b LEFT JOIN t1 AS t3 ON t2.b = t3.b LEFT JOIN ( t1 AS t4 JOIN t1 AS t5 ON t4.b = t5.b ) ON t2.b = t4.a WHERE t1.b >= 6 OR t3.b <> t4.b;
DROP TABLE t1;
...
DROP TABLE t1;
CREATE TABLE t1 ( a VARCHAR(1), b INTEGER );
SELECT * FROM t1 JOIN t1 AS t2 ON t1.b = t2.b JOIN t1 AS t3 JOIN ( t1 AS t4 STRAIGHT_JOIN t1 AS t5 ON t4.a = t5.b ) ON t1.b = t4.b WHERE t1.a = t5.b;
DROP TABLE t1;
CREATE TABLE t1 (a INTEGER);
INSERT INTO t1 VALUES (1), (2), (3), (4);
SELECT * FROM t1 LEFT JOIN ( t1 AS t2 LEFT JOIN t1 AS t3 ON FALSE ) ON t1.a <=> t3.a WHERE t1.a <=> t3.a;
DROP TABLE t1;
CREATE TABLE t1(a INT);
INSERT INTO t1 VALUES(1);
CREATE TABLE t2(x INT);
INSERT INTO t2 VALUES(2),(3);
EXPLAIN SELECT 6 FROM t1 WHERE EXISTS (SELECT 7 FROM (SELECT a) d1 WHERE EXISTS (SELECT 8 FROM t2));
DROP TABLE t1,t2;
CREATE TABLE t1( k INTEGER PRIMARY KEY, t2ref INTEGER );
INSERT INTO t1 VALUES(1,1), (2,NULL), (3,3);
CREATE TABLE t2( k INTEGER PRIMARY KEY, d INTEGER, t3ref INTEGER NOT NULL );
INSERT INTO t2 VALUES(1,11,1), (3,33,3);
CREATE TABLE t3( k INTEGER UNIQUE, e INTEGER );
INSERT INTO t3 VALUES(1,111), (3,333);
SET optimizer_switch='block_nested_loop=off';
SELECT t1.k, dt.d, t3.e FROM t1 LEFT JOIN (SELECT t2.* FROM t2 ) AS dt ON t1.t2ref = dt.k LEFT JOIN t3 ON dt.t3ref = t3.k ;
explain SELECT t1.k, dt.d, t3.e FROM t1 LEFT JOIN (SELECT t2.* FROM t2 ) AS dt ON t1.t2ref = dt.k LEFT JOIN t3 ON dt.t3ref = t3.k ;
DROP TABLE t1, t2, t3;
CREATE TABLE t1 ( a INTEGER );
SELECT * FROM t1, t1 AS t2, t1 AS t3 WHERE rand() > 0.5 OR (t1.a = 1 AND t2.a = 1);
DROP TABLE t1;
CREATE TABLE t1 ( a INTEGER, b INTEGER );
SELECT * FROM t1 STRAIGHT_JOIN t1 AS t2 ON t1.b = t2.b JOIN (t1 AS t3 JOIN t1 AS t4 ON t3.a = t4.a AND t3.b = t4.b) ON t2.b = t3.b AND t2.a < t3.a ORDER BY t2.b;
DROP TABLE t1;
CREATE TABLE t1 ( i INTEGER );
INSERT INTO t1 VALUES (1), (2);
SELECT * FROM t1 LEFT JOIN (SELECT i FROM t1 WHERE FALSE) AS d1 ON t1.i = d1.i WHERE NOT EXISTS (SELECT 1 FROM t1 AS inner_t1 WHERE i = d1.i);
DROP TABLE t1;
...
SELECT 1 FROM t1 NATURAL JOIN t1 AS t2 NATURAL JOIN t1 AS t3 WHERE t1.a <> ( SELECT SUM(a) FROM t1 );
DROP TABLE t1;
CREATE TABLE t1 ( f1 INTEGER );
INSERT INTO t1 VALUES (1), (2), (3);
CREATE TABLE t2 ( pk INTEGER, blobfield LONGTEXT );
