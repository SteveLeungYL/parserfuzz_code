...
DROP TABLE t1;
CREATE TABLE t1( pk INT NOT NULL, c1 CHAR(2), c2 INT, PRIMARY KEY(pk), UNIQUE KEY ukey(c1, c2) );
INSERT INTO t1(pk, c1, c2) VALUES (1,1,1),(2,2,2),(3,3,3),(4,5,4);
SET @a:=5;
INSERT IGNORE INTo t1(pk, c1, c2) SELECT (@a:=@a+1),@a,@a FROM t1, t1 t2,t1 t3, t1 t4;
ANALYZE TABLE t1;
SELECT * FROM t1 WHERE pk = 1 OR pk = 231;
EXPLAIN SELECT DISTINCT c1 FROM t1 FORCE INDEX(ukey) WHERE pk IN (1,231) and c1 IS NOT NULL;
EXPLAIN SELECT DISTINCT c1 FROM t1 IGNORE INDEX(ukey) WHERE pk IN (1,231) and c1 IS NOT NULL;
SELECT DISTINCT c1 FROM t1 FORCE INDEX(ukey) WHERE pk IN (1,231) and c1 IS NOT NULL;
SELECT DISTINCT c1 FROM t1 IGNORE INDEX(ukey) WHERE pk IN (1,231) and c1 IS NOT NULL;
DROP TABLE t1;
CREATE TABLE t1 ( pk INT NOT NULL AUTO_INCREMENT, c1 varchar(100) DEFAULT NULL, c2 INT NOT NULL, PRIMARY KEY (pk), UNIQUE KEY ukey (c2,c1) );
