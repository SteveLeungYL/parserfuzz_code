...
select is_updatable from information_schema.views where table_schema != 'sys' and table_schema != 'information_schema';
select * from t1;
drop view v2, v1;
drop table t1;
create table t1 (a int, b int);
...
CREATE VIEW v1 AS SELECT * FROM t1;
SELECT a.col1,a.col2,b.col2,b.col3 FROM t1 a LEFT JOIN t2 b ON a.col1=b.col1 WHERE b.col2 IS NULL OR b.col2=(SELECT MAX(col2) FROM t2 b WHERE b.col1=a.col1);
SELECT a.col1,a.col2,b.col2,b.col3 FROM v1 a LEFT JOIN t2 b ON a.col1=b.col1 WHERE b.col2 IS NULL OR b.col2=(SELECT MAX(col2) FROM t2 b WHERE b.col1=a.col1);
CREATE VIEW v2 AS SELECT * FROM t2;
SELECT a.col1,a.col2,b.col2,b.col3 FROM v2 b RIGHT JOIN v1 a ON a.col1=b.col1 WHERE b.col2 IS NULL OR b.col2=(SELECT MAX(col2) FROM v2 b WHERE b.col1=a.col1);
SELECT a.col1,a.col2,b.col2,b.col3 FROM v2 b RIGHT JOIN v1 a ON a.col1=b.col1 WHERE a.col1 IN (1,5,9) AND (b.col2 IS NULL OR b.col2=(SELECT MAX(col2) FROM v2 b WHERE b.col1=a.col1));
CREATE VIEW v3 AS SELECT * FROM t1 WHERE col1 IN (1,5,9);
SELECT a.col1,a.col2,b.col2,b.col3 FROM v2 b RIGHT JOIN v3 a ON a.col1=b.col1 WHERE b.col2 IS NULL OR b.col2=(SELECT MAX(col2) FROM v2 b WHERE b.col1=a.col1);
DROP VIEW v1,v2,v3;
DROP TABLE t1,t2;
...
create view v2 as select * from t3 where a in (select * from t1) or b in (select * from t2);
...
CREATE VIEW v1 AS SELECT t1.b as c, t2.b as d FROM t1,t2 WHERE t1.a=t2.a;
...
drop view v1;
drop table t1;
CREATE TABLE t1 ( aid int PRIMARY KEY, fn varchar(20) NOT NULL, ln varchar(20) NOT NULL );
CREATE TABLE t2 ( aid int NOT NULL, pid int NOT NULL );
...
SELECT pid,GROUP_CONCAT(CONCAT(fn,' ',ln) ORDER BY 1) FROM t1,t2 WHERE t1.aid = t2.aid GROUP BY pid;
...
CREATE VIEW v1 AS SELECT t1.pk as a FROM t1,t2,t3,t4,t5 WHERE t1.b IS NULL AND t1.pk=t2.fk AND t2.pk=t3.fk AND t3.pk=t4.fk AND t4.pk=t5.fk;
SELECT a FROM v1;
DROP VIEW v1;
...
drop view v1;
drop table t1;
create table t1 ( r_object_id char(16) NOT NULL, group_name varchar(32) NOT NULL ) engine = InnoDB;
create table t2 ( r_object_id char(16) NOT NULL, i_position int(11) NOT NULL, users_names varchar(32) default NULL ) Engine = InnoDB;
...
DROP TABLE t1,t2,t3;
create table t1 (f1 int);
create view v1 as select t1.f1 as '123 456' from t1;
select * from v1;
drop view v1;
drop table t1;
create table t1 (f1 int, f2 int);
...
DROP VIEW v1;
DROP TABLE t1;
drop table if exists t1; drop view if exists v1; create table t1 (id int); create view v1 as select * from t1; drop table t1; show create view v1; drop view v1; ;
create table t1(f1 int, f2 int);
...
DROP VIEW v2;
DROP TABLE t1, t2;
CREATE TABLE t1 (id int NOT NULL PRIMARY KEY, td date DEFAULT NULL, KEY idx(td));
...
CREATE VIEW v1 AS SELECT id, date(d) + INTERVAL TIME_TO_SEC(d) SECOND AS t, COUNT(*) FROM t1 GROUP BY id, t;
SHOW CREATE VIEW v1;
SELECT * FROM v1;
DROP VIEW v1;
DROP TABLE t1;
...
DROP VIEW v2, v1;
DROP TABLE t1;
CREATE TABLE t1( fName varchar(25) NOT NULL, lName varchar(25) NOT NULL, DOB date NOT NULL, test_date date NOT NULL, uID int unsigned NOT NULL AUTO_INCREMENT PRIMARY KEY);
INSERT INTO t1(fName, lName, DOB, test_date) VALUES ('Hank', 'Hill', '1964-09-29', '2007-01-01'), ('Tom', 'Adams', '1908-02-14', '2007-01-01'), ('Homer', 'Simpson', '1968-03-05', '2007-01-01');
...
INSERT INTO t1 VALUES ("Bart","Simpson"),("Milhouse","van Houten"),("Montgomery","Burns");
CREATE VIEW v1 AS SELECT CONCAT(firstname," ",surname) AS name FROM t1;
SELECT CONCAT(LEFT(name,LENGTH(name)-INSTR(REVERSE(name)," ")), LEFT(name,LENGTH(name)-INSTR(REVERSE(name)," "))) AS f1 FROM v1;
DROP VIEW v1;
DROP TABLE t1;
...
CREATE ALGORITHM=MERGE VIEW v1 AS SELECT t1.* FROM t1 JOIN t2 ON t2.fk = t1.pk AND t2.ver = (SELECT MAX(t.ver) FROM t2 t WHERE t.org = t2.org);
SHOW WARNINGS;
SHOW CREATE VIEW v1;
DROP VIEW v1;
DROP TABLE t1, t2;
...
CREATE TRIGGER t1_bi BEFORE INSERT ON t1 FOR EACH ROW SET NEW.i = (SELECT * FROM v1) + 1;
INSERT INTO t1 VALUES (1);
...
drop view v1;
drop table t1;
CREATE TABLE t1 ( id int(11) NOT NULL PRIMARY KEY, country varchar(32), code int(11) default NULL );
...
create table table_24532 ( a int, b bigint, c int(4), d bigint(48) );
create view view_24532_a as select a IS TRUE, a IS NOT TRUE, a IS FALSE, a IS NOT FALSE, a IS UNKNOWN, a IS NOT UNKNOWN, a is NULL, a IS NOT NULL, ISNULL(a), b IS TRUE, b IS NOT TRUE, b IS FALSE, b IS NOT FALSE, b IS UNKNOWN, b IS NOT UNKNOWN, b is NULL, b IS NOT NULL, ISNULL(b), c IS TRUE, c IS NOT TRUE, c IS FALSE, c IS NOT FALSE, c IS UNKNOWN, c IS NOT UNKNOWN, c is NULL, c IS NOT NULL, ISNULL(c), d IS TRUE, d IS NOT TRUE, d IS FALSE, d IS NOT FALSE, d IS UNKNOWN, d IS NOT UNKNOWN, d is NULL, d IS NOT NULL, ISNULL(d) from table_24532;
describe view_24532_a;
create view view_24532_b as select a IS TRUE, if(ifnull(a, 0), 1, 0) as old_istrue, a IS NOT TRUE, if(ifnull(a, 0), 0, 1) as old_isnottrue, a IS FALSE, if(ifnull(a, 1), 0, 1) as old_isfalse, a IS NOT FALSE, if(ifnull(a, 1), 1, 0) as old_isnotfalse from table_24532;
...
CREATE TABLE t1 ( lid int NOT NULL PRIMARY KEY, name char(10) NOT NULL );
INSERT INTO t1 (lid, name) VALUES (1, 'YES'), (2, 'NO');
CREATE TABLE t2 ( id int NOT NULL PRIMARY KEY, gid int NOT NULL, lid int NOT NULL, dt date );
CREATE TABLE t3 ( id int NOT NULL PRIMARY KEY, gid int NOT NULL, lid int NOT NULL, dt date );
INSERT INTO t2 (id, gid, lid, dt) VALUES (1, 1, 1, '2007-01-01'),(2, 1, 2, '2007-01-02'), (3, 2, 2, '2007-02-01'),(4, 2, 1, '2007-02-02');
INSERT INTO t3 (id, gid, lid, dt) VALUES (1, 1, 1, '2007-01-01'),(2, 1, 2, '2007-01-02'), (3, 2, 2, '2007-02-01'),(4, 2, 1, '2007-02-02');
SELECT DISTINCT t2.gid AS lgid, (SELECT t1.name FROM t1, t3 WHERE t1.lid  = t3.lid AND t3.gid = t2.gid ORDER BY t3.dt DESC LIMIT 1 ) as clid FROM t2;
CREATE VIEW v1 AS SELECT DISTINCT t2.gid AS lgid, (SELECT t1.name FROM t1, t3 WHERE t1.lid  = t3.lid AND t3.gid = t2.gid ORDER BY t3.dt DESC LIMIT 1 ) as clid FROM t2;
SELECT * FROM v1;
DROP VIEW v1;
DROP table t1,t2,t3;
CREATE TABLE t1 (a INT);
...
CREATE VIEW v1 AS SELECT t2.b,t2.c FROM t1, t2 WHERE t1.a=t2.b AND t2.b < 3 WITH CHECK OPTION;
SELECT * FROM v1;
...
CREATE VIEW v1 AS SELECT t2.c FROM t1, t2 WHERE t1.id=t2.id AND 1 IN (SELECT id FROM t1) WITH CHECK OPTION;
UPDATE v1 SET c=1;
DROP VIEW v1;
DROP TABLE t1,t2;
...
CREATE VIEW v1 AS SELECT t1.a1, t1.c FROM t1 JOIN t2 ON t1.a1=t2.a2 AND t1.c < 3 WITH CHECK OPTION;
SELECT * FROM v1;
UPDATE v1 SET c=3;
...
SELECT * FROM v1;
SELECT * FROM t1;
CREATE VIEW v2 AS SELECT t1.a1, t1.c FROM (t1 JOIN t2 ON t1.a1=t2.a2 AND t1.c < 3) JOIN (t3 JOIN t4 ON t3.a3=t4.a4) ON t2.a2=t3.a3 WITH CHECK OPTION;
SELECT * FROM v2;
UPDATE v2 SET c=3;
...
CREATE TABLE t3 ( role_id int NOT NULL auto_increment PRIMARY KEY, role_name varchar(100) default NULL, app_name varchar(40) NOT NULL, INDEX idx_app_name(app_name) );
CREATE VIEW v1 AS SELECT profile.person_id AS person_id FROM t1 profile, t2 userrole, t3 `role`     WHERE userrole.person_id = profile.person_id AND role.role_id = userrole.role_id AND profile.status_flg = 'A'   ORDER BY profile.person_id,role.app_name,role.role_name;
...
INSERT INTO t3 VALUES (1,'NUCANS_APP_USER','NUCANSAPP'),(2,'NUCANS_TRGAPP_USER','NUCANSAPP'), (3,'IA_INTAKE_COORDINATOR','IACANS'),(4,'IA_SCREENER','IACANS'), (5,'IA_SUPERVISOR','IACANS'),(6,'IA_READONLY','IACANS'), (7,'SOC_USER','SOCCANS'),(8,'CAYIT_USER','CAYITCANS'), (9,'RTOS_DCFSPOS_SUPERVISOR','RTOS');
ANALYZE TABLE t1,t2,t3;
...
DROP VIEW v1;
DROP TABLE t1;
CREATE TABLE t1 (a INT NOT NULL AUTO_INCREMENT, b INT NOT NULL DEFAULT 0, PRIMARY KEY(a), KEY (b));
...
create view v1 as select a b from t1;
create view v2 as select 1 a from t2, v1 where c in (select 1 from t1 where b = a);
...
INSERT INTO v1(a) SELECT 1 FROM t1,t1 AS c ON DUPLICATE KEY UPDATE `v1`.`a`= 1;
SELECT * FROM v1;
...
SELECT * FROM v2;
INSERT INTO v2(a) SELECT 1 FROM t1,t1 AS c ON DUPLICATE KEY UPDATE `v2`.`a`= 1;
SELECT * FROM v2;
DROP VIEW v1;
DROP VIEW v2;
DROP TABLE t1;
...
CREATE TABLE t1(f1 INT);
INSERT INTO t1 VALUES ();
CREATE VIEW v1 AS SELECT 1 FROM t1 WHERE ROW(1,1) >= ROW(1, (SELECT 1 FROM t1 WHERE  f1 >= ANY ( SELECT '1' )));
DROP VIEW v1;
DROP TABLE t1;
...
create view v_9801 as select sum(s1) from t_9801 with check option;
create view v_9801 as select sum(s1) from t_9801 group by s1 with check option;
create view v_9801 as select sum(s1) from t_9801 group by s1 with rollup with check option;
drop table t_9801;
DROP TABLE IF EXISTS t1;
DROP VIEW IF EXISTS v1;
...
CREATE TABLE t1 ( pk        INT AUTO_INCREMENT, c_int_key INT, PRIMARY KEY (pk), KEY (c_int_key) )  ENGINE=innodb;
CREATE VIEW v_t1 AS SELECT * FROM t1;
CREATE TABLE t2 ( pk              INT auto_increment, c_varchar_600_x VARCHAR(600), c_int_key       INT, c_varchar_600_y VARCHAR(600), c_varchar_600_z VARCHAR(600), PRIMARY KEY (pk), KEY (c_int_key) )  ENGINE=innodb;
CREATE VIEW v_t2 AS SELECT * FROM t2;
INSERT INTO t2 VALUES ( NULL, repeat('x', 600), 3, repeat('y', 600), repeat('z', 600) );
SELECT a1.pk AS f1 FROM v_t1 AS a1 LEFT JOIN v_t2 AS a2 ON a1.pk=a2.c_int_key WHERE  a1.pk > 8 OR ((a1.pk BETWEEN 9 AND 13) AND a1.pk = 90) ORDER BY f1 ;
DROP TABLE t1, t2;
DROP VIEW v_t1, v_t2;
CREATE TABLE t1 ( pk INTEGER, PRIMARY KEY (pk) );
INSERT INTO t1 VALUES (1), (2);
CREATE VIEW v_t1 AS SELECT * FROM t1;
SELECT pk FROM t1 WHERE pk > 8 OR ((pk BETWEEN 9 AND 13) AND pk = 90) ;
SELECT pk FROM v_t1 WHERE pk > 8 OR ((pk BETWEEN 9 AND 13) AND pk = 90) ;
DROP VIEW v_t1;
DROP TABLE t1;
...
CREATE VIEW v2 AS SELECT * FROM t2;
CREATE VIEW v2_sj AS SELECT * FROM t2 WHERE a IN (SELECT a FROM t1);
...
CREATE VIEW v12_3 AS SELECT t2.a, t2.b FROM t1 JOIN t2 USING (a) WHERE t1.b > 15;
...
ANALYZE TABLE t1,t2;
SELECT * FROM t1 JOIN v2 AS dt ON t1.a=dt.a;
explain SELECT * FROM t1 JOIN v2 AS dt ON t1.a=dt.a;
SELECT * FROM t1, v2 AS dt WHERE t1.a=dt.a;
explain SELECT * FROM t1, v2 AS dt WHERE t1.a=dt.a;
SELECT * FROM (t1 JOIN t2 ON t1.a=t2.a) JOIN v2 AS dt ON t1.a=dt.a;
explain SELECT * FROM (t1 JOIN t2 ON t1.a=t2.a) JOIN v2 AS dt ON t1.a=dt.a;
SELECT * FROM t1 JOIN v12_1 AS dt ON t1.a=dt.a;
explain SELECT * FROM t1 JOIN v12_1 AS dt ON t1.a=dt.a;
SELECT * FROM (t1 JOIN t2 USING (a)) JOIN v12_1 AS dt ON t1.a=dt.a;
explain SELECT * FROM (t1 JOIN t2 USING (a)) JOIN v12_1 AS dt ON t1.a=dt.a;
SELECT * FROM (t1 JOIN t2 USING (a)) JOIN v12_2 AS dt1 ON t1.a=dt1.a AND t2.b=dt1.b JOIN v12_1 AS dt2 ON dt1.a=dt2.a;
explain SELECT * FROM (t1 JOIN t2 USING (a)) JOIN v12_2 AS dt1 ON t1.a=dt1.a AND t2.b=dt1.b JOIN v12_1 AS dt2 ON dt1.a=dt2.a;
SELECT * FROM t1 JOIN v12_3 AS dt ON t1.a=dt.a;
explain SELECT * FROM t1 JOIN v12_3 AS dt ON t1.a=dt.a;
SELECT * FROM t1 JOIN v2_sj AS dt ON t1.a=dt.a;
explain SELECT * FROM t1 JOIN v2_sj AS dt ON t1.a=dt.a;
SELECT * FROM t1 JOIN vu_1 AS dt ON t1.a=dt.a;
explain SELECT * FROM t1 JOIN vu_1 AS dt ON t1.a=dt.a;
SELECT * FROM t1 JOIN vu_2 AS dt ON t1.a=dt.a;
explain SELECT * FROM t1 JOIN vu_2 AS dt ON t1.a=dt.a;
SELECT * FROM t1 JOIN vd_1 AS dt ON t1.a=dt.a;
explain SELECT * FROM t1 JOIN vd_1 AS dt ON t1.a=dt.a;
SELECT * FROM t1 JOIN va_1 AS dt ON t1.a=dt.a;
explain SELECT * FROM t1 JOIN va_1 AS dt ON t1.a=dt.a;
SELECT * FROM t1 JOIN vg_1 AS dt ON t1.a=dt.a;
explain SELECT * FROM t1 JOIN vg_1 AS dt ON t1.a=dt.a;
SELECT * FROM t1 JOIN vh_1 AS dt ON t1.a=dt.a;
explain SELECT * FROM t1 JOIN vh_1 AS dt ON t1.a=dt.a;
SELECT * FROM t1 JOIN vl_1 AS dt ON t1.a=dt.a;
explain SELECT * FROM t1 JOIN vl_1 AS dt ON t1.a=dt.a;
SELECT * FROM t1 JOIN vlo_1 AS dt ON t1.a=dt.a;
explain SELECT * FROM t1 JOIN vlo_1 AS dt ON t1.a=dt.a;
SELECT * FROM t1 JOIN vrow AS dt ON t1.a=dt.a;
explain SELECT * FROM t1 JOIN vrow AS dt ON t1.a=dt.a;
SELECT * FROM vo_1 AS dt;
explain SELECT * FROM vo_1 AS dt;
SELECT * FROM vo_2 AS dt;
explain SELECT * FROM vo_2 AS dt;
SELECT * FROM vo_1 AS dt WHERE dt.a > 0;
explain SELECT * FROM vo_1 AS dt WHERE dt.a > 0;
SELECT * FROM vo_2 AS dt WHERE dt.a > 0;
explain SELECT * FROM vo_2 AS dt WHERE dt.a > 0;
SELECT * FROM t1 JOIN vo_1 AS dt ON t1.a=dt.a;
explain SELECT * FROM t1 JOIN vo_1 AS dt ON t1.a=dt.a;
SELECT * FROM t1 JOIN vo_2 AS dt ON t1.a=dt.a;
explain SELECT * FROM t1 JOIN vo_2 AS dt ON t1.a=dt.a;
SELECT dt.a, COUNT(*) FROM vo_1 AS dt GROUP BY dt.a;
explain SELECT dt.a, COUNT(*) FROM vo_1 AS dt GROUP BY dt.a;
SELECT dt.a, COUNT(*) FROM vo_2 AS dt GROUP BY dt.a;
explain SELECT dt.a, COUNT(*) FROM vo_2 AS dt GROUP BY dt.a;
SELECT COUNT(*) FROM vo_1 AS dt;
explain SELECT COUNT(*) FROM vo_1 AS dt;
SELECT COUNT(*) FROM vo_2 AS dt;
explain SELECT COUNT(*) FROM vo_2 AS dt;
SELECT DISTINCT * FROM vo_1 AS dt;
explain SELECT DISTINCT * FROM vo_1 AS dt;
SELECT DISTINCT * FROM vo_2 AS dt;
explain SELECT DISTINCT * FROM vo_2 AS dt;
SELECT * FROM t1 JOIN vo_1 AS dt ON t1.a=dt.a ORDER BY t1.b;
explain SELECT * FROM t1 JOIN vo_1 AS dt ON t1.a=dt.a ORDER BY t1.b;
SELECT * FROM t1 JOIN vo_2 AS dt ON t1.a=dt.a ORDER BY t1.b;
explain SELECT * FROM t1 JOIN vo_2 AS dt ON t1.a=dt.a ORDER BY t1.b;
SELECT * FROM t1 JOIN vx AS dt ON t1.a=dt.a;
explain SELECT * FROM t1 JOIN vx AS dt ON t1.a=dt.a;
...
ANALYZE TABLE t1,t2;
CREATE VIEW v1 AS SELECT * FROM (SELECT * FROM t2) AS dt;
SELECT * FROM v1;
SELECT * FROM (SELECT * FROM t2) AS dt;
explain SELECT * FROM v1;
explain SELECT * FROM (SELECT * FROM t2) AS dt;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT * FROM (SELECT * FROM t1 WHERE b=a*10) AS dt;
SELECT * FROM v1;
SELECT * FROM (SELECT * FROM t1 WHERE b=a*10) AS dt;
explain SELECT * FROM v1;
explain SELECT * FROM (SELECT * FROM t1 WHERE b=a*10) AS dt;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT * FROM (SELECT a, SUM(b) AS s, COUNT(*) AS c FROM t1 GROUP BY a ORDER BY a) AS dt;
SELECT * FROM v1;
...
DROP VIEW v1;
CREATE VIEW v1 AS SELECT * FROM (SELECT DISTINCT a FROM t1) AS dt;
SELECT * FROM v1;
SELECT * FROM (SELECT DISTINCT a FROM t1) AS dt;
explain SELECT * FROM v1;
explain SELECT * FROM (SELECT DISTINCT a FROM t1) AS dt;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT * FROM (SELECT * FROM t1 LIMIT 3 OFFSET 3) AS dt;
SELECT * FROM v1;
SELECT * FROM (SELECT * FROM t1 LIMIT 3 OFFSET 3) AS dt;
explain SELECT * FROM v1;
explain SELECT * FROM (SELECT * FROM t1 LIMIT 3 OFFSET 3) AS dt;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT * FROM (SELECT DISTINCT a FROM t1 UNION ALL SELECT a FROM t2) AS dt;
SELECT * FROM v1;
...
DROP VIEW v1;
CREATE VIEW v1 AS SELECT * FROM (SELECT * FROM t1 WHERE (SELECT a FROM t1 LIMIT 1) = b/10) AS dt;
SELECT * FROM v1;
...
DROP VIEW v1;
CREATE VIEW v1 AS SELECT * FROM (SELECT * FROM t1 WHERE a IN (SELECT a FROM t2 WHERE a % 2 = 0)) AS dt;
SELECT * FROM v1;
...
DROP VIEW v1;
CREATE VIEW v1 AS SELECT * FROM (SELECT a, (SELECT a FROM t2 WHERE a=t1.a) FROM t1 WHERE b IN (SELECT a*10 FROM t2)) AS dt;
SELECT * FROM v1;
SELECT * FROM (SELECT a, (SELECT a FROM t2 WHERE a=t1.a) FROM t1 WHERE b IN (SELECT a*10 FROM t2)) AS dt;
explain SELECT * FROM v1;
explain SELECT * FROM (SELECT a, (SELECT a FROM t2 WHERE a=t1.a) FROM t1 WHERE b IN (SELECT a*10 FROM t2)) AS dt;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT * FROM (SELECT a, (SELECT a FROM t2) FROM t1 WHERE b=a*10) AS dt;
SELECT * FROM v1;
SELECT * FROM (SELECT a, (SELECT a FROM t2) FROM t1 WHERE b=a*10) AS dt;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT * FROM (SELECT a, b FROM t1 WHERE b IN (SELECT a*10 FROM t2)) AS dt;
...
INSERT INTO t2 VALUES (1), (2), (3), (4), (5), (NULL);
ANALYZE TABLE t1,t2;
CREATE VIEW v1 AS SELECT a, b, (SELECT 1 FROM t2 WHERE a=3) AS s FROM t1;
SELECT * FROM v1;
SELECT a FROM v1;
SELECT a, b, (SELECT 1 FROM t2 WHERE a=3) AS s FROM t1;
explain SELECT * FROM v1;
explain SELECT a FROM v1;
explain SELECT a, b, (SELECT 1 FROM t2 WHERE a=3) AS s FROM t1;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT a, b, (SELECT 1 FROM t2 WHERE a=6) AS s FROM t1;
SELECT * FROM v1;
SELECT a FROM v1;
SELECT a, b, (SELECT 1 FROM t2 WHERE a=6) AS s FROM t1;
explain SELECT * FROM v1;
explain SELECT a FROM v1;
explain SELECT a, b, (SELECT 1 FROM t2 WHERE a=6) AS s FROM t1;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT a, b, (SELECT 1 FROM t2 WHERE a>=3) AS s FROM t1;
SELECT * FROM v1;
SELECT a FROM v1;
...
explain SELECT a FROM v1;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT a, b, a IN (SELECT 1 FROM t2 WHERE a=6) AS s FROM t1;
SELECT * FROM v1;
SELECT a FROM v1;
SELECT a, b, a IN (SELECT 1 FROM t2 WHERE a=6) AS s FROM t1;
explain SELECT * FROM v1;
explain SELECT a FROM v1;
explain SELECT a, b, a IN (SELECT 1 FROM t2 WHERE a=6) AS s FROM t1;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT a, b, (SELECT COUNT(*) FROM t2) AS c FROM t1;
SELECT * FROM v1;
SELECT a FROM v1;
SELECT a, b, (SELECT COUNT(*) FROM t2) AS c FROM t1;
explain SELECT * FROM v1;
explain SELECT a FROM v1;
explain SELECT a, b, (SELECT COUNT(*) FROM t2) AS c FROM t1;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT a, b, a IN (SELECT COUNT(*) FROM t2) AS c FROM t1;
SELECT * FROM v1;
SELECT a FROM v1;
SELECT a, b, a IN (SELECT COUNT(*) FROM t2) AS c FROM t1;
explain SELECT * FROM v1;
explain SELECT a FROM v1;
explain SELECT a, b, a IN (SELECT COUNT(*) FROM t2) AS c FROM t1;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT a, b, (SELECT a*2 FROM t2 WHERE a=t1.a) AS s FROM t1;
SELECT * FROM v1;
SELECT a FROM v1;
SELECT a, b, (SELECT a*2 FROM t2 WHERE a=t1.a) AS s FROM t1;
explain SELECT * FROM v1;
explain SELECT a FROM v1;
explain SELECT a, b, (SELECT a*2 FROM t2 WHERE a=t1.a) AS s FROM t1;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT a, b, EXISTS (SELECT a*2 FROM t2 WHERE a=t1.a) AS s FROM t1;
SELECT * FROM v1;
SELECT a FROM v1;
SELECT a, b, EXISTS (SELECT a*2 FROM t2 WHERE a=t1.a) AS s FROM t1;
explain SELECT * FROM v1;
explain SELECT a FROM v1;
explain SELECT a, b, EXISTS (SELECT a*2 FROM t2 WHERE a=t1.a) AS s FROM t1;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT a, b, (SELECT COUNT(*) FROM t1 AS t2 WHERE a=t1.a) AS s FROM t1;
SELECT * FROM v1;
SELECT a FROM v1;
SELECT a, b, (SELECT COUNT(*) FROM t1 AS t2 WHERE a=t1.a) AS s FROM t1;
explain SELECT * FROM v1;
explain SELECT a FROM v1;
explain SELECT a, b, (SELECT COUNT(*) FROM t1 AS t2 WHERE a=t1.a) AS s FROM t1;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT a, b, EXISTS (SELECT COUNT(*) FROM t1 AS t2 WHERE a=t1.a) AS s FROM t1;
SELECT * FROM v1;
SELECT a FROM v1;
SELECT a, b, EXISTS (SELECT COUNT(*) FROM t1 AS t2 WHERE a=t1.a) AS s FROM t1;
explain SELECT * FROM v1;
explain SELECT a FROM v1;
explain SELECT a, b, EXISTS (SELECT COUNT(*) FROM t1 AS t2 WHERE a=t1.a) AS s FROM t1;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT (SELECT a FROM t2 WHERE a=FLOOR(COUNT(t1.a)/2)) AS s FROM t1;
SELECT * FROM v1;
SELECT (SELECT a FROM t2 WHERE a=FLOOR(COUNT(t1.a)/2)) AS s FROM t1;
explain SELECT * FROM v1;
explain SELECT (SELECT a FROM t2 WHERE a=FLOOR(COUNT(t1.a)/2)) AS s FROM t1;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT COUNT(*) AS a, (SELECT a FROM t2 WHERE a=FLOOR(COUNT(t1.a)/2)) AS s FROM t1;
SELECT * FROM v1;
SELECT a FROM v1;
SELECT COUNT(*) AS a, (SELECT a FROM t2 WHERE a=FLOOR(COUNT(t1.a)/2)) AS s FROM t1;
explain SELECT * FROM v1;
explain SELECT a FROM v1;
explain SELECT COUNT(*) AS a, (SELECT a FROM t2 WHERE a=FLOOR(COUNT(t1.a)/2)) AS s FROM t1;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT a IN (SELECT a FROM t2 WHERE a=FLOOR(COUNT(t1.a)/2)) AS s FROM t1;
SELECT * FROM v1;
SELECT a IN (SELECT a FROM t2 WHERE a=FLOOR(COUNT(t1.a)/2)) AS s FROM t1;
explain SELECT * FROM v1;
explain SELECT a IN (SELECT a FROM t2 WHERE a=FLOOR(COUNT(t1.a)/2)) AS s FROM t1;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT COUNT(*) AS a, a IN (SELECT a FROM t2 WHERE a=FLOOR(COUNT(t1.a)/2)) AS s FROM t1;
SELECT * FROM v1;
SELECT a FROM v1;
SELECT COUNT(*) AS a, a IN (SELECT a FROM t2 WHERE a=FLOOR(COUNT(t1.a)/2)) AS s FROM t1;
explain SELECT * FROM v1;
explain SELECT a FROM v1;
explain SELECT COUNT(*) AS a, a IN (SELECT a FROM t2 WHERE a=FLOOR(COUNT(t1.a)/2)) AS s FROM t1;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT a, COUNT(*) AS c, (SELECT a FROM t2 WHERE a=COUNT(t1.a)) AS s FROM t1 GROUP BY a;
SELECT * FROM v1;
SELECT a FROM v1;
SELECT a, COUNT(*) AS c, (SELECT a FROM t2 WHERE a=COUNT(t1.a)) AS s FROM t1 GROUP BY a;
explain SELECT * FROM v1;
explain SELECT a FROM v1;
explain SELECT a, COUNT(*) AS c, (SELECT a FROM t2 WHERE a=COUNT(t1.a)) AS s FROM t1 GROUP BY a;
DROP VIEW v1;
CREATE VIEW v1 AS SELECT a, COUNT(*) AS c, a IN (SELECT a FROM t2 WHERE a=COUNT(t1.a)) AS s FROM t1 GROUP BY a;
SELECT * FROM v1;
SELECT a FROM v1;
SELECT a, COUNT(*) AS c, a IN (SELECT a FROM t2 WHERE a=COUNT(t1.a)) AS s FROM t1 GROUP BY a;
explain SELECT * FROM v1;
explain SELECT a FROM v1;
explain SELECT a, COUNT(*) AS c, a IN (SELECT a FROM t2 WHERE a=COUNT(t1.a)) AS s FROM t1 GROUP BY a;
DROP VIEW v1;
DROP TABLE t1, t2;
CREATE TABLE t1 ( pk int NOT NULL, col_date_key date DEFAULT NULL, PRIMARY KEY (pk), KEY col_date_key (col_date_key) ) ;
CREATE TABLE t2 ( pk int NOT NULL, col_time_key time DEFAULT NULL, col_datetime_key datetime DEFAULT NULL, PRIMARY KEY (pk), KEY col_time_key (col_time_key), KEY col_datetime_key (col_datetime_key) );
CREATE ALGORITHM=MERGE VIEW v1 AS  SELECT col_date_key FROM t1 WHERE (pk, pk, col_date_key) IN (SELECT col_datetime_key, col_time_key, col_time_key FROM t2 WHERE pk <= 7);
SELECT * FROM v1;
DROP VIEW v1;
DROP TABLE t1, t2;
CREATE VIEW v1 AS (SELECT '' FROM DUAL);
CREATE VIEW v2 AS (SELECT 'BUG#14117018' AS col1 FROM DUAL) UNION ALL (SELECT '' FROM DUAL);
CREATE VIEW v3 AS (SELECT 'BUG#14117018' AS col1 FROM DUAL) UNION ALL (SELECT '' FROM DUAL) UNION ALL (SELECT '' FROM DUAL);
CREATE VIEW v4 AS (SELECT 'BUG#14117018' AS col1 FROM DUAL) UNION ALL (SELECT '' AS col2 FROM DUAL) UNION ALL (SELECT '' FROM DUAL);
SHOW CREATE VIEW v1;
SHOW CREATE VIEW v2;
SHOW CREATE VIEW v3;
SHOW CREATE VIEW v4;
DROP VIEW v1, v2, v3, v4;
CREATE TABLE t1 ( pk int NOT NULL, col_varchar_key varchar(1) NOT NULL, PRIMARY KEY (pk) );
CREATE TABLE t2 ( pk int NOT NULL, col_varchar_key varchar(1) NOT NULL, col_varchar_nokey varchar(1) NOT NULL, PRIMARY KEY (pk) );
CREATE TABLE t3 ( pk int NOT NULL, col_int_key int NOT NULL, col_varchar_nokey varchar(1) NOT NULL, PRIMARY KEY (pk) );
CREATE VIEW v2 AS SELECT * FROM t2;
ANALYZE TABLE t1,t2,t3;
explain SELECT STRAIGHT_JOIN alias1.pk FROM t2 AS alias1 RIGHT JOIN (SELECT sq1_alias2.* FROM t1 AS sq1_alias1 RIGHT OUTER JOIN v2 AS sq1_alias2 ON sq1_alias2.col_varchar_key = sq1_alias1.col_varchar_key AND sq1_alias2.col_varchar_nokey IN (SELECT c_sq1_alias1.col_varchar_nokey AS c_sq1_field1 FROM t3 AS c_sq1_alias1 WHERE c_sq1_alias1.col_int_key <> c_sq1_alias1.col_int_key ) ) AS alias2 ON alias2.col_varchar_key = alias1.col_varchar_key;
SELECT STRAIGHT_JOIN alias1.pk FROM t2 AS alias1 RIGHT JOIN (SELECT sq1_alias2.* FROM t1 AS sq1_alias1 RIGHT OUTER JOIN v2 AS sq1_alias2 ON sq1_alias2.col_varchar_key = sq1_alias1.col_varchar_key AND sq1_alias2.col_varchar_nokey IN (SELECT c_sq1_alias1.col_varchar_nokey AS c_sq1_field1 FROM t3 AS c_sq1_alias1 WHERE c_sq1_alias1.col_int_key <> c_sq1_alias1.col_int_key ) ) AS alias2 ON alias2.col_varchar_key = alias1.col_varchar_key;
DROP VIEW v2;
DROP TABLE t1, t2, t3;
...
CREATE VIEW vtt AS SELECT * FROM t1 JOIN t2 ON t1.a1=t2.a2;
CREATE VIEW vtr AS SELECT * FROM t1 JOIN vmat2 AS dt2 ON t1.a1=dt2.a2;
CREATE VIEW vtrd AS SELECT * FROM t1 JOIN (SELECT DISTINCT * FROM t2) AS dt2 ON t1.a1=dt2.a2;
CREATE VIEW vrt AS SELECT * FROM vmat1 AS dt1 JOIN t2 ON dt1.a1=t2.a2;
CREATE VIEW vrtd AS SELECT * FROM (SELECT DISTINCT * FROM t1) AS dt1 JOIN t2 ON dt1.a1=t2.a2;
CREATE VIEW vrr AS SELECT * FROM vmat1 AS dt1 JOIN vmat2 AS dt2 ON dt1.a1=dt2.a2;
CREATE VIEW vrrd AS SELECT * FROM (SELECT DISTINCT * FROM t1) AS dt1 JOIN (SELECT DISTINCT * FROM t2) AS dt2 ON dt1.a1=dt2.a2;
...
DELETE FROM t1;
DELETE FROM t2;
INSERT INTO t1 VALUES (1,100), (2,100), (3,100), (4,100), (5,100), (6,100), (7,100), (8,100), (9,100), (10,100), (11,100), (12,100), (13,100), (14,100);
INSERT INTO t2 VALUES (1,100), (2,100), (3,100), (4,100), (5,100), (6,100), (7,100), (8,100), (9,100), (10,100), (11,100), (12,100), (13,100), (14,100);
...
create view v2c as select * from t1 where a like '%v2c%'  with check option;
show create view v2c;
create view v3l as select * from t1 where a like '%v3l%'  with local check option;
...
create view v4l as select * from v2c where a like '%v4l%'  with local check option;
create view v5l as select * from v3l where a like '%v5l%'  with local check option;
...
create view v6c as select * from v5n where a like '%v6c%'  with cascaded check option;
...
create view v8l as select * from v7n where a like '%v8l%'  with local check option;
...
CREATE VIEW v1 AS SELECT 1 AS z from t1;
...
DROP VIEW v;
DROP TABLE t;
CREATE VIEW v1 (fld1, fld2) AS SELECT 1 AS a, 2 AS b UNION ALL SELECT 1 AS a, 1 AS a;
CREATE VIEW v2 (fld1, fld2) AS SELECT 1 AS a, 2 AS a UNION ALL SELECT 1 AS a, 1 AS a;
CREATE VIEW v3 AS SELECT 1 AS a, 2 AS b UNION ALL SELECT 1 AS a, 1 AS a;
CREATE VIEW v4 (fld1, fld1) AS SELECT 1 AS a, 2 AS b UNION ALL SELECT 1 AS a, 1 AS a;
CREATE VIEW v4 AS SELECT 1 AS a, 2 AS a UNION ALL SELECT 1 AS a, 1 AS a;
DROP VIEW v1, v2, v3;
...
CREATE VIEW v1 AS SELECT TIMESTAMPDIFF(MICROSECOND, ts1, ts2) duration FROM t;
SELECT * FROM v1;
CREATE VIEW v2 AS SELECT MIN(duration) AS dmin, MAX(duration) AS dmax FROM v1;
DROP VIEW v1, v2;
DROP TABLE t;
CREATE TABLE t1_base_N3 (pk INT, col_int INT);
CREATE VIEW t1_view_N3 AS SELECT * FROM t1_base_N3 WHERE `pk` BETWEEN 1 AND 2;
...
CREATE VIEW v11 AS SELECT * FROM db.t1 WHERE fld1 NOT IN (SELECT fld2 FROM db.t2);
...
DROP VIEW v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13, v14, v15, v16, v17, v18, v19;
DROP TABLE t1, t2;
DROP DATABASE db;
...
DROP TABLE t1;
DROP VIEW v1, v2, v3, v4, v5;
CREATE TABLE t1(pk INTEGER, cik INTEGER, UNIQUE KEY(cik)) PARTITION BY KEY(cik) PARTITIONS 10;
CREATE TABLE t2(pk INTEGER, cvk VARCHAR(20), cik INTEGER, UNIQUE KEY(cik)) PARTITION BY KEY(cik) PARTITIONS 10;
CREATE VIEW v1 AS SELECT MAX(t2.pk) AS field1 FROM t1, t2 WHERE (SELECT 7 FROM DUAL) IS NOT NULL AND t1.pk BETWEEN 123 AND (123 + 128) AND t2.cvk <= 'p' ORDER BY t1.pk DESC;
DROP VIEW v1;
DROP TABLE t1, t2;
CREATE TABLE t1(fld1 INT, fld2 INT);
INSERT INTO t1 VALUES(1, 1);
CREATE OR REPLACE VIEW v1 (`x12345678901234567890123456789012345678901234567890123456789012322`) AS SELECT fld1 FROM t1;
DROP TABLE t1;
CREATE TABLE t1 ( col_date_key DATE, KEY col_date_key (col_date_key) );
...
INSERT INTO t1 VALUES ('2009-01-14');
SELECT MAX(col_date_key) AS x FROM t1 HAVING x >= CAST('2009-01-01' AS DATE) ;
CREATE VIEW v1 AS SELECT MAX(col_date_key) AS x FROM t1 HAVING x >= CAST('2009-01-01' AS DATE) ;
SELECT * FROM v1;
DROP VIEW v1;
DROP TABLE t1;
CREATE TABLE t1 (pk INT);
CREATE VIEW v1 AS SELECT COUNT(*) FROM t1 ORDER BY (SELECT 1 FROM t1 WHERE 1 IN (SELECT * FROM (SELECT 1 as field) AS dt));
DROP TABLE t1;
DROP VIEW v1;
...
INSERT INTO t1 VALUES (), ();
CREATE VIEW v1 AS SELECT alias1.col_int_nokey AS field1, (SELECT alias2.col_int_key FROM t1 AS alias2 WHERE alias1.col_varchar_key <= alias1.col_varchar_nokey ) AS field2 FROM t1 AS alias1;
DROP VIEW v1;
