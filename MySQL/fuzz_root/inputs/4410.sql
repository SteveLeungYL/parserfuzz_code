...
explain select bin(a1), bin(a2) from t1bit where (a1, a2) in (select b1, b2 from t2bit);
select bin(a1), bin(a2) from t1bit where (a1, a2) in (select b1, b2 from t2bit);
...
explain select bin(a1), a2 from t1bb where (a1, a2) in (select b1, b2 from t2bb);
select bin(a1), a2 from t1bb where (a1, a2) in (select b1, b2 from t2bb);
drop table t1bb, t2bb;
drop table t1, t2, t3, t1i, t2i, t3i, columns;
/****************************************************************************** * Test the cache of the left operand of IN. ******************************************************************************/ # Test that default values of Cached_item are not used for comparison create table t1 (s1 int);
...
ANALYZE TABLE t1, t2, t3;
explain select a from t1 where a in (select c from t2 where d >= 20);
...
explain select a from t1 where a in (select c from t2 where d >= 20);
select a from t1 where a in (select c from t2 where d >= 20);
explain select/*+ NO_RANGE_OPTIMIZATION(t1) */ a from t1 group by a having a in (select c from t2 where d >= 20);
select a from t1 group by a having a in (select c from t2 where d >= 20);
create index iab on t1(a, b);
ANALYZE TABLE t1;
explain select /*+ NO_RANGE_OPTIMIZATION(t1) */ a from t1 group by a having a in (select c from t2 where d >= 20);
select a from t1 group by a having a in (select c from t2 where d >= 20);
explain select a from t1 group by a having a in (select c from t2 where d >= some(select e from t3 where max(b)=e));
select a from t1 group by a having a in (select c from t2 where d >= some(select e from t3 where max(b)=e));
explain select a from t1 where a in (select c from t2 where d >= some(select e from t3 where b=e));
...
select 1 from t2 where   t2.a > 1  or  t2.a = 3 and not t2.a not in (select t2.b from t2);
drop table t2;
...
EXPLAIN  SELECT t1i FROM t1 JOIN t4 ON t1i=t4i   WHERE (t1i)  IN (   SELECT t2i FROM t2   WHERE (t2i)  IN (   SELECT t3i FROM t3   GROUP BY t3i )   );
DROP TABLE t1,t2,t3,t4;
CREATE TABLE t1 ( pk INT, v VARCHAR(1) DEFAULT NULL, PRIMARY KEY(pk) ) charset latin1;
...
ANALYZE TABLE t1, t2, t3;
SELECT MIN(t2.pk) FROM t2 JOIN t1 ON t1.pk=t2.pk WHERE 'j' HAVING ('m') IN (  SELECT v FROM t2);
EXPLAIN SELECT MIN(t2.pk) FROM t2 JOIN t1 ON t1.pk=t2.pk WHERE 'j' HAVING ('m') IN (  SELECT v FROM t2);
SELECT MIN(t2.pk) FROM t2  WHERE t2.pk>10 HAVING ('m') IN (  SELECT v FROM t2);
EXPLAIN SELECT MIN(t2.pk) FROM t2  WHERE t2.pk>10 HAVING ('m') IN (  SELECT v FROM t2);
SELECT MIN(pk) FROM t1 WHERE pk=NULL HAVING ('m') IN (  SELECT v FROM t2);
EXPLAIN SELECT MIN(pk) FROM t1 WHERE pk=NULL HAVING ('m') IN (  SELECT v FROM t2);
SELECT MIN(a) FROM (SELECT a FROM empty1) tt HAVING ('m') IN (  SELECT v FROM t2);
EXPLAIN  SELECT MIN(a) FROM (SELECT a FROM empty1) tt HAVING ('m') IN (  SELECT v FROM t2);
SELECT min(t1.pk) FROM t1 WHERE t1.pk IN (SELECT 1 from t3 where pk>10) HAVING ('m') IN (  SELECT v FROM t2);
EXPLAIN SELECT min(t1.pk) FROM t1 WHERE t1.pk IN (SELECT 1 from t3 where pk>10) HAVING ('m') IN (  SELECT v FROM t2);
...
ANALYZE TABLE t1, t2, t3;
EXPLAIN SELECT i, j FROM t1 LEFT JOIN t2 ON (j) IN (SELECT k FROM t3);
SELECT i, j FROM t1 LEFT JOIN t2 ON (j) IN (SELECT k FROM t3);
EXPLAIN SELECT i, j FROM t1 LEFT JOIN t2 ON (j) IN (SELECT max(k) FROM t3);
SELECT i, j FROM t1 LEFT JOIN t2 ON (j) IN (SELECT max(k) FROM t3);
DROP TABLE t1, t2, t3;
CREATE TABLE t1 ( pk INTEGER AUTO_INCREMENT, col_int_nokey INTEGER, col_int_key INTEGER, col_varchar_key VARCHAR(1), PRIMARY KEY (pk), KEY (col_int_key), KEY (col_varchar_key, col_int_key) ) ;
INSERT INTO t1 ( col_int_key, col_int_nokey, col_varchar_key )  VALUES (2, NULL, 'w'), (9, 7, 'm'), (3, 9, 'm'), (9, 7, 'k'), (NULL, 4, 'r'), (9, 2, 't'), (3, 6, 'j'), (8, 8, 'u'), (8, NULL, 'h'), (53, 5, 'o'), (0, NULL, NULL), (5, 6, 'k'), (166, 188, 'e'), (3, 2, 'n'), (0, 1, 't'), (1, 1, 'c'), (9, 0, 'm'), (5, 9, 'y'), (6, NULL, 'f'), (2, 4, 'd') ;
SELECT table2.col_varchar_key AS field1, table2.col_int_nokey AS field2 FROM ( t1 AS table1 LEFT OUTER JOIN t1 AS table2 ON (table2.col_varchar_key = table1.col_varchar_key  ) )  WHERE table1.pk = 6 HAVING  ( field2 ) IN  ( SELECT SUBQUERY2_t2.col_int_nokey AS SUBQUERY2_field2  FROM ( t1 AS SUBQUERY2_t1 JOIN t1 AS SUBQUERY2_t2 ON (SUBQUERY2_t2.col_varchar_key = SUBQUERY2_t1.col_varchar_key ) ) ) ORDER BY field2  ;
drop table t1;
CREATE TABLE t1(track varchar(15));
INSERT INTO t1 VALUES ('CAD'), ('CAD');
PREPARE STMT FROM "SELECT 1 FROM t1   WHERE         track IN (SELECT track FROM t1                                     GROUP BY track                                        HAVING track>='CAD')";
EXECUTE STMT ;
EXECUTE STMT ;
...
set @previous_sql_mode_htnt542nh=@@sql_mode;
set sql_mode=(select replace(@@sql_mode,'ONLY_FULL_GROUP_BY',''));
SELECT COUNT( i ),i FROM t1 HAVING ('c')   IN (SELECT t2.c FROM (t2 JOIN t3));
set @@sql_mode=@previous_sql_mode_htnt542nh;
DROP TABLE t1,t2,t3;
CREATE TABLE t1 (a INTEGER);
CREATE TABLE t2 (b INTEGER);
INSERT INTO t2 VALUES (1);
explain SELECT a FROM ( SELECT t1.* FROM t1 LEFT JOIN t2 ON t1.a > 3 OR t2.b IN (SELECT a FROM t1) ) table1;
SELECT a FROM ( SELECT t1.* FROM t1 LEFT JOIN t2 ON t1.a > 3 OR t2.b IN (SELECT a FROM t1) ) table1;
DROP TABLE t1, t2;
CREATE TABLE t1 ( pk int NOT NULL, f1 int DEFAULT NULL, PRIMARY KEY (pk) ) ENGINE=INNODB;
CREATE TABLE t2 ( pk int NOT NULL, f1 int DEFAULT NULL, PRIMARY KEY (pk) ) ENGINE=INNODB;
INSERT INTO t1 VALUES (10,0);
INSERT INTO t2 VALUES (10,0),(11,0);
ANALYZE TABLE t1, t2, t3;
explain SELECT * FROM t1 JOIN t2 USING (f1) WHERE t1.f1 IN (SELECT t1.pk FROM t1 ORDER BY t1.f1);
SELECT * FROM t1 JOIN t2 USING (f1) WHERE t1.f1 IN (SELECT t1.pk FROM t1 ORDER BY t1.f1);
DROP TABLE t1, t2;
CREATE TABLE t1 ( col_varchar_key varchar(1) DEFAULT NULL, col_varchar_nokey varchar(1) DEFAULT NULL, KEY col_varchar_key (col_varchar_key))  ;
INSERT INTO t1 VALUES ('v','v'),('r','r');
CREATE TABLE t2 ( col_varchar_key varchar(1) DEFAULT NULL, col_varchar_nokey varchar(1) DEFAULT NULL, KEY col_varchar_key(col_varchar_key))  ;
INSERT INTO t2 VALUES ('r','r'),('c','c');
CREATE VIEW v3 AS SELECT * FROM t2;
SELECT DISTINCT alias2.col_varchar_key  FROM t1 AS alias1 JOIN v3 AS alias2  ON alias2.col_varchar_key = alias1.col_varchar_key HAVING col_varchar_key IN (SELECT col_varchar_nokey FROM t2) ;
DROP TABLE t1, t2;
DROP VIEW v3;
...
SELECT MIN(t3.col_int_nokey),t1.col_int_nokey AS field3 FROM t3 LEFT JOIN t1 ON t1.col_int_nokey WHERE (194, 200) IN ( SELECT SQ4_alias1.col_int_nokey, SQ4_alias2.col_int_nokey FROM t2 AS SQ4_alias1 JOIN t2 AS SQ4_alias2 ON SQ4_alias2.col_int_nokey = 5 ) GROUP BY field3 ;
DROP TABLE t1;
DROP TABLE t2;
DROP TABLE t3;
CREATE TABLE t1(a int);
...
DROP TABLE t1,t2,t3;
CREATE TABLE t1 ( col_varchar_nokey varchar(1) NOT NULL ) ENGINE=INNODB;
INSERT INTO t1 VALUES ('b');
CREATE TABLE t2 ( col_varchar_nokey varchar(1) NOT NULL ) ENGINE=INNODB;
INSERT INTO t2 VALUES ('k');
CREATE TABLE t3 ( col_varchar_nokey varchar(1) NOT NULL ) ENGINE=INNODB;
ANALYZE TABLE t1, t2, t3;
explain SELECT STRAIGHT_JOIN * FROM t1 LEFT JOIN t2 ON t1.col_varchar_nokey IN (SELECT col_varchar_nokey FROM t3);
SELECT STRAIGHT_JOIN * FROM t1 LEFT JOIN t2 ON t1.col_varchar_nokey IN (SELECT col_varchar_nokey FROM t3);
DROP TABLE t1, t2, t3;
CREATE TABLE t1 (v INTEGER) ENGINE=INNODB;
INSERT INTO t1 VALUES(1);
CREATE TABLE t2 (v INTEGER) ENGINE=INNODB;
SELECT * FROM t1 LEFT JOIN t2 ON t2.v IN(SELECT v FROM t1);
DROP TABLE t1, t2;
CREATE TABLE t1 ( pk int NOT NULL, col_int_nokey int DEFAULT NULL, col_int_key int DEFAULT NULL, PRIMARY KEY (pk), KEY col_int_key (col_int_key) ) ENGINE=INNODB;
INSERT INTO t1 VALUES (1,2,4), (2,150,62);
CREATE TABLE t2 ( pk int NOT NULL, col_int_key int DEFAULT NULL, PRIMARY KEY (pk) ) ENGINE=INNODB;
INSERT INTO t2 VALUES (1,7);
ANALYZE TABLE t1, t2;
explain SELECT table1.pk, table2.pk FROM t2 AS table1 LEFT JOIN t2 AS table2 ON table2.pk = table1.pk AND table2.col_int_key IN (SELECT col_int_key FROM t1 AS innr WHERE innr.col_int_nokey > innr.col_int_nokey GROUP BY col_int_key HAVING COUNT(*) > 0 );
FLUSH STATUS;
SELECT table1.pk, table2.pk FROM t2 AS table1 LEFT JOIN t2 AS table2 ON table2.pk = table1.pk AND table2.col_int_key IN (SELECT col_int_key FROM t1 AS innr WHERE innr.col_int_nokey > innr.col_int_nokey GROUP BY col_int_key HAVING COUNT(*) > 0 );
SHOW SESSION STATUS LIKE 'Sort_scan%';
DROP TABLE t1, t2;
...
explain select t1.a, t2.a, (t1.a,t1.a) in (select a,a from t2 as t3) from t1 join t2 on t1.a+t2.a=1000;
select t1.a, t2.a, (t1.a,t1.a) in (select a,a from t2 as t3) from t1 join t2 on t1.a+t2.a=1000;
explain select t1.a, t2.a, (t2.a,t2.a) in (select a,a from t2 as t3) from t1 left join t2 on t1.a+t2.a=1000;
select t1.a, t2.a, (t2.a,t2.a) in (select a,a from t2 as t3) from t1 left join t2 on t1.a+t2.a=1000;
...
ANALYZE TABLE t2;
explain select t1.a, t2.a from t1 join t2 on t1.a+t2.a=3 where (t2.a,t2.a) in (select a,a from t2 as t3);
select t1.a, t2.a from t1 join t2 on t1.a+t2.a=3 where (t2.a,t2.a) in (select a,a from t2 as t3);
explain select t1.a, t2.a from t1 join t2 on t1.a+t2.a=3 where (t2.a,t2.a) not in (select a,a from t2 as t3);
select t1.a, t2.a from t1 join t2 on t1.a+t2.a=3 where (t2.a,t2.a) not in (select a,a from t2 as t3);
drop table t1,t2;
...
explain select t1.a, t2.a, t2.a in (select * from t2 as t3) from t1 left join t2 on t1.a+t2.a=1000;
select t1.a, t2.a, t2.a in (select * from t2 as t3) from t1 left join t2 on t1.a+t2.a=1000;
explain select t1.a, t2.a, (t2.a,t2.a) in (select a,a from t2 as t3) from t1 left join t2 on t1.a+t2.a=1000;
select t1.a, t2.a, (t2.a,t2.a) in (select a,a from t2 as t3) from t1 left join t2 on t1.a+t2.a=1000;
drop table t1,t2;
...
ANALYZE TABLE t1, t2;
explain select t1.a as z, sum(t1.b) from t1 group by t1.a having (z in (select * from t2)) is null;
select t1.a as z, sum(t1.b) from t1 group by t1.a having (z in (select * from t2)) is null;
...
drop table t1,t2;
CREATE TABLE t1 ( pk int NOT NULL, col_varchar_nokey varchar(1) DEFAULT NULL, PRIMARY KEY (pk) );
INSERT INTO t1 VALUES (10,'x');
CREATE TABLE t2 ( pk int NOT NULL, col_varchar_nokey varchar(1) DEFAULT NULL, PRIMARY KEY (pk) );
INSERT INTO t2 VALUES (1,'v'), (5,'x'), (11,'z'), (12,'c'), (15,'y');
CREATE TABLE t3 ( pk int NOT NULL, col_int_key int DEFAULT NULL, PRIMARY KEY (pk), KEY col_int_key (col_int_key) );
INSERT INTO t3 VALUES (10,8);
CREATE TABLE t4 ( pk int NOT NULL, col_varchar_nokey varchar(1) DEFAULT NULL, PRIMARY KEY (pk) );
INSERT INTO t4 VALUES (1,'x');
ANALYZE TABLE t1, t2, t3, t4;
EXPLAIN SELECT outr.pk, outr.col_varchar_nokey, outr2.col_varchar_nokey FROM t2 AS outr2 JOIN t4 AS outr ON (outr2.col_varchar_nokey > outr.col_varchar_nokey) WHERE outr.col_varchar_nokey IN ( SELECT innr.col_varchar_nokey FROM t3 AS innr2 LEFT JOIN t1 AS innr ON (innr2.col_int_key >= innr.pk) ) XOR outr.pk < 6 ;
FLUSH STATUS;
SELECT outr.pk, outr.col_varchar_nokey, outr2.col_varchar_nokey FROM t2 AS outr2 JOIN t4 AS outr ON (outr2.col_varchar_nokey > outr.col_varchar_nokey) WHERE outr.col_varchar_nokey IN ( SELECT innr.col_varchar_nokey FROM t3 AS innr2 LEFT JOIN t1 AS innr ON (innr2.col_int_key >= innr.pk) ) XOR outr.pk < 6 ;
SHOW STATUS LIKE "HANDLER_READ%";
DROP TABLE t1,t2,t3,t4;
CREATE TABLE t1 ( col_int_key INT, KEY col_int_key (col_int_key) );
INSERT INTO t1 VALUES (1);
CREATE TABLE t2 ( col_int_key INT, col_time_key TIME, col_datetime_nokey DATETIME, KEY col_int_key (col_int_key), KEY col_time_key (col_time_key) );
INSERT INTO t2 VALUES (7,'14:03:03','2001-11-28 00:50:27'), (1,'01:46:09','2007-10-09 19:53:04');
ANALYZE TABLE t1, t2;
EXPLAIN SELECT col_datetime_nokey AS x FROM t2 AS outr WHERE col_int_key IN ( SELECT STRAIGHT_JOIN col_int_key FROM t1 ) AND outr.col_int_key = 0 HAVING x = '2000-09-09' ORDER BY col_time_key;
SELECT col_datetime_nokey AS x FROM t2 AS outr WHERE col_int_key IN ( SELECT STRAIGHT_JOIN col_int_key FROM t1 ) AND outr.col_int_key = 0 HAVING x = '2000-09-09' ORDER BY col_time_key;
DROP TABLE t1, t2;
CREATE TABLE t1 (c1 bigint,c2 char,pk INT,c3 char,c4 int,c5 INT,key (c5)) ENGINE=InnoDB;
INSERT INTO t1 VALUES (763078661862588416,0,1,'',1,'');
CREATE TABLE t2 (c4k int,c4 int,cminnuk INT,key (cminnuk)) ENGINE=InnoDB;
INSERT INTO t2 VALUES(0,'','');
CREATE TABLE t3 (c4 int,pk INT,c1 bigint,cyk year,cy year,key (cyk)) ENGINE=InnoDB;
INSERT INTO t3 VALUES(0,8,'',0,'');
ANALYZE TABLE t1, t2, t3;
EXPLAIN SELECT o.c2 AS x FROM t1 AS o WHERE o.c1 IN (SELECT innr.c4 AS y FROM t2 AS innr2 JOIN t3 AS innr ON (innr2.c4k=innr.c4) WHERE innr.c1=6 OR NOT innr.c1=innr.pk ORDER BY innr.c4) AND o.c4=7 XOR o.pk=3 ORDER BY o.pk;
SELECT o.c2 AS x FROM t1 AS o WHERE o.c1 IN (SELECT innr.c4 AS y FROM t2 AS innr2 JOIN t3 AS innr ON (innr2.c4k=innr.c4) WHERE innr.c1=6 OR NOT innr.c1=innr.pk ORDER BY innr.c4) AND o.c4=7 XOR o.pk=3 ORDER BY o.pk;
DROP TABLE t1,t2,t3;
...
CREATE TABLE t2 ( col_int_key INT, col_varchar_key VARCHAR(1), col_varchar_nokey VARCHAR(1), KEY col_int_key (col_int_key), KEY col_varchar_key (col_varchar_key,col_int_key) ) charset utf8mb4;
...
EXPLAIN SELECT alias2.col_int_key FROM t1 AS alias1 , t2 AS alias2 WHERE alias1.col_varchar_key NOT IN ( SELECT sq1_alias1.col_varchar_nokey FROM t3 AS sq1_alias1 JOIN ( t1 AS sq1_alias2 STRAIGHT_JOIN view_d AS sq1_alias3 ) WHERE ( SELECT col_int_key FROM t1 AS c_sq1_alias1 WHERE c_sq1_alias1.col_varchar_key = sq1_alias3.col_varchar_nokey ) ) ;
SELECT alias2.col_int_key FROM t1 AS alias1 , t2 AS alias2 WHERE alias1.col_varchar_key NOT IN ( SELECT sq1_alias1.col_varchar_nokey FROM t3 AS sq1_alias1 JOIN ( t1 AS sq1_alias2 STRAIGHT_JOIN view_d AS sq1_alias3 ) WHERE ( SELECT col_int_key FROM t1 AS c_sq1_alias1 WHERE c_sq1_alias1.col_varchar_key = sq1_alias3.col_varchar_nokey ) ) ;
CREATE PROCEDURE stored_proc_2581 () LANGUAGE SQL SELECT alias2.col_int_key FROM t1 AS alias1 , t2 AS alias2 WHERE alias1.col_varchar_key NOT IN ( SELECT sq1_alias1.col_varchar_nokey FROM t3 AS sq1_alias1 JOIN ( t1 AS sq1_alias2 STRAIGHT_JOIN view_d AS sq1_alias3 ) WHERE ( SELECT col_int_key FROM t1 AS c_sq1_alias1 WHERE c_sq1_alias1.col_varchar_key = sq1_alias3.col_varchar_nokey ) ) ;
CALL stored_proc_2581;
CALL stored_proc_2581;
DROP PROCEDURE stored_proc_2581;
PREPARE s FROM 'SELECT alias2.col_int_key FROM t1 AS alias1 , t2 AS alias2 WHERE alias1.col_varchar_key NOT IN ( SELECT sq1_alias1.col_varchar_nokey FROM t3 AS sq1_alias1 JOIN ( t1 AS sq1_alias2 STRAIGHT_JOIN view_d AS sq1_alias3 ) WHERE ( SELECT col_int_key FROM t1 AS c_sq1_alias1 WHERE c_sq1_alias1.col_varchar_key = sq1_alias3.col_varchar_nokey ) ) ';
EXECUTE s;
EXECUTE s;
DROP VIEW view_d;
DROP TABLE t1,t2,t3;
...
EXPLAIN SELECT COUNT(*) FROM t1 WHERE t1.a NOT IN ( SELECT t2.a FROM t1 as t2 );
SELECT COUNT(*) FROM t1 WHERE t1.a NOT IN ( SELECT t2.a FROM t1 as t2 );
DROP TABLE t1;
...
ANALYZE TABLE t1, t2;
SELECT (SELECT NULL IN (NULL) FROM t1 WHERE a) = ANY(SELECT 1 FROM t2) FROM t1;
DROP TABLE t1, t2;
