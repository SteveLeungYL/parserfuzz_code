...
show create table t1;
drop table t1;
CREATE TABLE t1(a MEDIUMTEXT CHARACTER SET gbk, b MEDIUMTEXT CHARACTER SET big5);
...
DROP TABLE t1;
CREATE TABLE t1 AS SELECT concat(head, tail) AS code, ' ' AS a FROM head, tail WHERE (head BETWEEN '80' AND 'FF') AND (tail BETWEEN '20' AND 'FF') ORDER BY head, tail;
...
do is_ipv4_mapped(inet6_aton(convert(_ascii "a:" using gbk)));
