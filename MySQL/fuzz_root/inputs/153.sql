...
SELECT * FROM t;
DROP TABLE t;
CREATE TABLE t1(a INT PRIMARY KEY, b GEOMETRY NOT NULL DEFAULT (ST_GEOMFROMTEXT('LINESTRING(0 0,9.299720368548e18 0,0 0,0 0)')));
...
ALTER TABLE t ADD COLUMN b int DEFAULT( date_sub(a, INTERVAL A MONTH));
...
CREATE PROCEDURE p1() BEGIN SELECT 42; END ;
CREATE FUNCTION f1() RETURNS INT BEGIN RETURN 42; END ;
...
select * from t1;
DROP TABLE t1;
CREATE TABLE IF NOT EXISTS D1 (i2 INT DEFAULT 568447044, d1 DOUBLE, d2 DOUBLE DEFAULT 0.0, c2 CHAR(255), def2 DOUBLE DEFAULT( i2 DIV d2 ), v1 INT AS ( d2 = c2 % ASIN( d1 ) > i2 ) VIRTUAL);
INSERT ignore INTO D1 (  i2, d1, d2, c2, def2) VALUES ( 1548385958 , d1 ,128158532 , 0.0 , DEFAULT);
ALTER TABLE D1 ADD UNIQUE KEY uidx ( def2 , v1 , d2 );
DROP TABLE D1;
CREATE TABLE IF NOT EXISTS D1 (def2 DOUBLE DEFAULT( 100 DIV 0 ), v1 INT AS (1) VIRTUAL);
...
DROP TABLE t1;
CREATE TABLE t1 ( i1 INTEGER, i2 INTEGER DEFAULT (i1 + i1) );
...
SELECT * FROM t1;
DROP TABLE t1;
CREATE TABLE t1 ( i1 INTEGER, i2 INTEGER DEFAULT (i1 + RAND()) );
INSERT INTO t1 (i1, i2) SELECT 5, 6;
INSERT INTO t1 (i1) SELECT 5;
INSERT INTO t1 (i1) SELECT 5 ON DUPLICATE KEY UPDATE i2= 4;
INSERT INTO t1 (i1) SELECT 5 ON DUPLICATE KEY UPDATE i2= DEFAULT;
...
CREATE TABLE bug(`id` binary(16) NOT NULL DEFAULT (uuid_to_bin(uuid())), PRIMARY KEY (`id`));
SHOW CREATE TABLE bug;
DROP TABLE bug;
CREATE TABLE pk_t1(i INT NOT NULL);
INSERT INTO pk_t1 VALUES (1),(2),(3),(4);
ALTER TABLE pk_t1 ADD COLUMN (`id` BINARY(16) DEFAULT (uuid_to_bin(uuid())) PRIMARY KEY NOT NULL);
ALTER TABLE pk_t1 ADD COLUMN (`id` INT DEFAULT (10 + i) PRIMARY KEY NOT NULL);
...
INSERT INTO t2 SELECT * FROM t1;
INSERT INTO t3 SELECT * FROM t1;
ALTER TABLE t1 ADD COLUMN b INT DEFAULT (a), ADD COLUMN c INT GENERATED ALWAYS AS (-b) STORED, ADD COLUMN d INT DEFAULT (c), ADD COLUMN e INT GENERATED ALWAYS AS (-d) STORED;
ALTER TABLE t2 ADD COLUMN b INT DEFAULT (a), ADD COLUMN c INT GENERATED ALWAYS AS (-b) STORED, ADD COLUMN d INT DEFAULT (c), ADD COLUMN e INT GENERATED ALWAYS AS (-d) STORED, ALGORITHM=COPY;
ALTER TABLE t3 ADD COLUMN b INT DEFAULT (a), ADD COLUMN c INT GENERATED ALWAYS AS (-b) STORED, ADD COLUMN d INT DEFAULT (c), ADD COLUMN e INT GENERATED ALWAYS AS (-d) STORED, ALGORITHM=INPLACE;
SELECT * FROM t1;
SELECT * FROM t2;
SELECT * FROM t3;
DROP TABLES t1, t2, t3;
...
DROP TABLE  t2;
CREATE TABLE t1 ( pk_col      INT NOT NULL AUTO_INCREMENT, json_col    JSON, blob_col    BLOB, text_col    TEXT, tblob_col   TINYBLOB, ttext_col   TINYTEXT, json_col_d  JSON DEFAULT (JSON_OBJECT('key','val')), blob_col_d  BLOB DEFAULT ('asda'), text_col_d  TEXT DEFAULT (CONCAT('adasdada','ds')), tblob_col_d TINYBLOB DEFAULT ('asda'), ttext_col_d TINYTEXT DEFAULT (CONCAT('adasdada','ds')), PRIMARY KEY (`pk_col`));
SELECT COLUMN_NAME, COLUMN_DEFAULT, DATA_TYPE, EXTRA FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='t1'   ORDER BY COLUMN_NAME;
SHOW COLUMNS FROM t1;
SHOW CREATE TABLE t1;
DROP TABLE t1;
