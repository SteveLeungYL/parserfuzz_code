...
ANALYZE TABLE t1;
SELECT pk, c1 FROM t1   WHERE (pk BETWEEN 4 AND 5 OR pk < 2) AND c1 < 240 ORDER BY c1 LIMIT 1;
EXPLAIN SELECT pk, c1 FROM t1   WHERE (pk BETWEEN 4 AND 5 OR pk < 2) AND c1 < 240 ORDER BY c1 LIMIT 1;
DROP TABLE t1;
CREATE TABLE t1 ( c1 TINYTEXT NOT NULL, c2 INT NOT NULL, PRIMARY KEY (c2), KEY id1 (c1(4)) ) charset utf8mb4;
...
DROP TABLE t1;
CREATE TABLE t1 ( col_int_nokey INTEGER, col_int_key INTEGER, col_varchar_key VARCHAR(1), KEY (col_int_key), KEY (col_varchar_key, col_int_key) ) charset utf8mb4 stats_persistent=0;
...
EXPLAIN SELECT table1.col_int_nokey FROM t1 AS table1 STRAIGHT_JOIN ( t1 AS table2 INNER JOIN t1 AS table3 ON table3.col_varchar_key = table2.col_varchar_key) ON table3.col_int_nokey = table1.col_int_key WHERE table3.col_int_key != SOME ( SELECT c1 FROM t2 );
SELECT table1.col_int_nokey FROM t1 AS table1 STRAIGHT_JOIN ( t1 AS table2 INNER JOIN t1 AS table3 ON table3.col_varchar_key = table2.col_varchar_key) ON table3.col_int_nokey = table1.col_int_key WHERE table3.col_int_key != SOME ( SELECT c1 FROM t2 );
DROP TABLE t1, t2;
CREATE TABLE t1 ( pk INTEGER NOT NULL, c1 INTEGER NOT NULL, c2 INTEGER NOT NULL, PRIMARY KEY (pk) );
INSERT INTO t1 VALUES (1,6,7);
CREATE TABLE t2 ( c1 INTEGER NOT NULL );
ANALYZE TABLE t1;
EXPLAIN SELECT t1.c1 FROM t1 WHERE t1.pk < 317 AND 2 IN (SELECT COUNT(t2.c1) FROM t2) ORDER BY t1.c2;
SELECT t1.c1 FROM t1 WHERE t1.pk < 317 AND 2 IN (SELECT COUNT(t2.c1) FROM t2) ORDER BY t1.c2;
DROP TABLE t1, t2;
CREATE TABLE t1 ( i1 INTEGER NOT NULL, c1 VARCHAR(1) NOT NULL ) charset latin1;
INSERT INTO t1 VALUES (2,'w');
CREATE TABLE t2 ( i1 INTEGER NOT NULL, c1 VARCHAR(1) NOT NULL, c2 VARCHAR(1) NOT NULL, KEY (c1, i1) ) charset latin1;
INSERT INTO t2 VALUES (8,'d','d');
INSERT INTO t2 VALUES (4,'v','v');
CREATE TABLE t3 ( c1 VARCHAR(1) NOT NULL ) charset latin1;
INSERT INTO t3 VALUES ('v');
ANALYZE TABLE t1;
ANALYZE TABLE t2;
ANALYZE TABLE t3;
EXPLAIN SELECT i1 FROM t1 WHERE EXISTS (SELECT t2.c1 FROM (t2 INNER JOIN t3 ON (t3.c1 = t2.c1))  WHERE t2.c2 != t1.c1 AND t2.c2 = (SELECT MIN(t3.c1) FROM t3));
SELECT i1 FROM t1 WHERE EXISTS (SELECT t2.c1 FROM (t2 INNER JOIN t3 ON (t3.c1 = t2.c1))  WHERE t2.c2 != t1.c1 AND t2.c2 = (SELECT MIN(t3.c1) FROM t3));
DROP TABLE t1,t2,t3;
...
SELECT * FROM t1 WHERE pk IN (SELECT it.pk FROM t2 JOIN t2 AS it ON it.i=it.i WHERE t1.i);
DROP TABLE t1, t2;
set @save_optimizer_switch_bug58816= @@optimizer_switch;
CREATE TABLE t1 ( pk INT NOT NULL, c1 INT NOT NULL, PRIMARY KEY (pk) );
...
CREATE TABLE t1 ( t1_int INT, t1_time TIME );
CREATE TABLE t2 (  t2_int int PRIMARY KEY, t2_int2 INT );
INSERT IGNORE INTO t2 VALUES ();
INSERT INTO t1 VALUES ();
ANALYZE TABLE t1;
ANALYZE TABLE t2;
SELECT * FROM t1 AS t1a  WHERE NOT EXISTS (SELECT * FROM t1 AS t1b WHERE t1b.t1_int NOT IN (SELECT t2.t2_int  FROM t2 WHERE t1b.t1_time LIKE t1b.t1_int OR t1b.t1_time <> t2.t2_int2 AND 6=7  ) ) ;;
EXPLAIN SELECT * FROM t1 AS t1a  WHERE NOT EXISTS (SELECT * FROM t1 AS t1b WHERE t1b.t1_int NOT IN (SELECT t2.t2_int  FROM t2 WHERE t1b.t1_time LIKE t1b.t1_int OR t1b.t1_time <> t2.t2_int2 AND 6=7  ) ) ;;
DROP TABLE t1,t2;
CREATE TABLE t1 ( pk INTEGER NOT NULL, c1 VARCHAR(3) NOT NULL, PRIMARY KEY (pk) );
INSERT INTO t1 VALUES (1,'y'),(0,'or');
CREATE TABLE t2 ( pk INTEGER NOT NULL, c1 VARCHAR(3) NOT NULL, c2 VARCHAR(6) NOT NULL, PRIMARY KEY (pk) );
INSERT INTO t2 VALUES (6,'y','RPOYT'),(10,'m','JINQE');
ANALYZE TABLE t1;
ANALYZE TABLE t2;
EXPLAIN SELECT c2 FROM t1 JOIN t2 ON t1.c1 = t2.c1 WHERE (t2.pk <= 4 AND t1.pk IN (2,1)) OR (t1.pk > 1 AND t2.pk BETWEEN 6 AND 6);
SELECT c2 FROM t1 JOIN t2 ON t1.c1 = t2.c1 WHERE (t2.pk <= 4 AND t1.pk IN (2,1)) OR (t1.pk > 1 AND t2.pk BETWEEN 6 AND 6);
DROP TABLE t1, t2;
CREATE TABLE t1 ( pk INT NOT NULL, c1 INT, PRIMARY KEY (pk), KEY col_int_key (c1) );
...
DROP TABLE t1;
CREATE TABLE t1 ( pk INTEGER AUTO_INCREMENT PRIMARY KEY, i1 INTEGER, c1 CHAR(6), i2 INTEGER NOT NULL, KEY (i2) );
INSERT INTO t1 VALUES (NULL, 4, 'that', 8), (NULL, 1, 'she', 6), (NULL, 6, 'tell', 2);
SELECT * FROM t1 WHERE i2 IN (3, 6) LIMIT 2 FOR UPDATE;
INSERT INTO t1 (i2) VALUES (1);
DROP TABLE t1;
CREATE TABLE t1 ( col999 FLOAT NOT NULL, COL1000 VARBINARY(179) NOT NULL, col1003 DATE DEFAULT NULL, KEY idx4267 (col1000, col1003) );
INSERT IGNORE INTO t1 VALUES (),();
SELECT col999 FROM t1 WHERE col1000 = "3" AND col1003 <=> sysdate();
DROP TABLE t1;
CREATE TABLE t1 (   pk INTEGER NOT NULL, i1 INTEGER, PRIMARY KEY (pk), KEY col_int_key (i1) );
INSERT INTO t1 VALUES (14,NULL), (18,133);
CREATE TABLE t2 (   pk INTEGER NOT NULL, i1 INTEGER, c1 VARCHAR(1), PRIMARY KEY (pk), KEY col_int_key (i1) );
INSERT INTO t2 VALUES (1,7,'f');
set @old_opt_switch=@@optimizer_switch;
set optimizer_switch='materialization=off';
EXPLAIN SELECT t1.i1 FROM t1 WHERE t1.i1 NOT IN ( SELECT STRAIGHT_JOIN subquery_t1.pk FROM t1 AS subquery_t1 JOIN t2 AS subquery_t2 ON subquery_t2.i1 = subquery_t1.pk WHERE subquery_t1.i1 > 0 OR subquery_t2.c1 = 'a' );
SELECT t1.i1 FROM t1 WHERE t1.i1 NOT IN ( SELECT STRAIGHT_JOIN subquery_t1.pk FROM t1 AS subquery_t1 JOIN t2 AS subquery_t2 ON subquery_t2.i1 = subquery_t1.pk WHERE subquery_t1.i1 > 0 OR subquery_t2.c1 = 'a' );
set @@optimizer_switch=@old_opt_switch;
DROP TABLE t1,t2;
CREATE TABLE t1 (   i1 INTEGER, c1 VARCHAR(1), KEY col_varchar_key (c1) ) charset utf8mb4;
INSERT INTO t1 VALUES (1,'j'), (0,'e'), (210,'f'), (8,'v'), (7,'x'), (5,'m'), (NULL,'c');
CREATE TABLE t2 (   i1 INTEGER, c1 VARCHAR(1), KEY col_varchar_key (c1) ) charset utf8mb4;
INSERT INTO t2 VALUES (8,NULL);
CREATE TABLE t3 (   i1 INTEGER, c1 VARCHAR(1), KEY col_varchar_key (c1) ) charset utf8mb4 ENGINE=InnoDB;
...
set @old_opt_switch=@@optimizer_switch;
set optimizer_switch='materialization=off';
EXPLAIN SELECT i1 FROM t3 WHERE c1 IN ( SELECT t1.c1 FROM t2 JOIN t1 ON t2.i1 >= t1.i1 WHERE t1.c1 > t2.c1 ) XOR i1;
SELECT i1 FROM t3 WHERE c1 IN ( SELECT t1.c1 FROM t2 JOIN t1 ON t2.i1 >= t1.i1 WHERE t1.c1 > t2.c1 ) XOR i1;
set @@optimizer_switch=@old_opt_switch;
DROP TABLE t1, t2, t3;
CREATE TABLE t1 ( pk INTEGER PRIMARY KEY,  a INTEGER NOT NULL,  b CHAR(1),  KEY(b) ) charset utf8mb4;
INSERT INTO t1 VALUES (23,5,'d');
ANALYZE TABLE t1;
EXPLAIN SELECT a1.pk  FROM t1 AS a1 JOIN (SELECT * FROM t1 LIMIT 1) AS a2 ON a2.b = a1.b WHERE a1.a = (SELECT pk FROM t1 LIMIT 1)  AND (a1.a != a2.a OR a1.b IS NULL);
SELECT a1.pk  FROM t1 AS a1 JOIN (SELECT * FROM t1 LIMIT 1) AS a2 ON a2.b = a1.b WHERE a1.a = (SELECT pk FROM t1 LIMIT 1)  AND (a1.a != a2.a OR a1.b IS NULL);
...
DROP VIEW v1;
DROP TABLE t1;
CREATE TABLE t1 ( pk INTEGER NOT NULL,  i1 INTEGER NOT NULL, c1 VARCHAR(1) NOT NULL, PRIMARY KEY (pk) );
INSERT INTO t1 VALUES (1,3,'j'), (20,8,'e');
ANALYZE TABLE t1;
EXPLAIN SELECT alias2.i1 FROM t1 AS alias1 STRAIGHT_JOIN t1 AS alias2 ON alias2.pk AND alias2.pk <= alias1.c1 WHERE alias2.pk = 1;
SELECT alias2.i1 FROM t1 AS alias1 STRAIGHT_JOIN t1 AS alias2 ON alias2.pk AND alias2.pk <= alias1.c1 WHERE alias2.pk = 1;
DROP TABLE t1;
CREATE TABLE t1 ( i1 INTEGER NOT NULL, d1 DOUBLE, KEY k1 (d1) );
INSERT INTO t1 VALUES (10,1), (17,NULL), (22,NULL);
CREATE TABLE t2 ( pk INTEGER NOT NULL, i1 INTEGER NOT NULL, PRIMARY KEY (pk) );
INSERT INTO t2 VALUES (4,1);
ANALYZE TABLE t1;
ANALYZE TABLE t2;
EXPLAIN SELECT t1.d1, t2.pk, t2.i1 FROM t1 STRAIGHT_JOIN t2 ON t2.i1 WHERE t2.pk <> t1.d1 AND t2.pk = 4;
SELECT t1.d1, t2.pk, t2.i1 FROM t1 STRAIGHT_JOIN t2 ON t2.i1 WHERE t2.pk <> t1.d1 AND t2.pk = 4;
DROP TABLE t1, t2;
CREATE TABLE t1 ( col_int_key INT, pk INT, PRIMARY KEY (pk), KEY (col_int_key) );
INSERT INTO t1 VALUES (2,3),(3,2),(3,5),(4,6);
CREATE TABLE t2 ( col_int_key INT, pk INT, PRIMARY KEY (pk), KEY (col_int_key) );
INSERT INTO t2 VALUES (0,9),(3,10),(4,6),(6,1),(100,3),(200,5);
ANALYZE TABLE t1;
ANALYZE TABLE t2;
set @old_opt_switch=@@optimizer_switch;
SET optimizer_switch="block_nested_loop=off";
EXPLAIN SELECT t2.col_int_key AS field1 FROM t2 USE INDEX(col_int_key) STRAIGHT_JOIN t1 ON t2.col_int_key WHERE t2.pk < 7 AND t2.col_int_key <> 7 GROUP BY field1;
SELECT t2.col_int_key AS field1 FROM t2 USE INDEX(col_int_key) STRAIGHT_JOIN t1 ON t2.col_int_key WHERE t2.pk < 7 AND t2.col_int_key <> 7 GROUP BY field1;
SET @@optimizer_switch=@old_opt_switch;
DROP TABLE t1,t2;
CREATE TABLE t1 ( i1 INTEGER NOT NULL, i2 INTEGER NOT NULL );
INSERT INTO t1 VALUES (14,1), (15,2), (16,3);
CREATE TABLE t2 ( i1 INTEGER NOT NULL, i2 INTEGER NOT NULL, c1 TINYTEXT );
INSERT INTO t2 SELECT i1, 10 * i2, "MySQL" FROM t1;
CREATE PROCEDURE proc1(id INTEGER) BEGIN SELECT i2 FROM ( (SELECT i1, i2, NULL AS a1 FROM t1) UNION (SELECT i1, i2, c1 AS a1 FROM t2) ) u1 WHERE i1 = id; END;
CALL proc1(15);
DROP PROCEDURE proc1;
DROP TABLE t1, t2;
CREATE TABLE t1 ( i1 INTEGER NOT NULL, i2 INTEGER NOT NULL, KEY (i1) );
INSERT INTO t1 VALUES (4,4), (5,5);
CREATE TABLE t2 ( pk INTEGER NOT NULL, PRIMARY KEY (pk) );
INSERT INTO t2 VALUES (1);
ANALYZE TABLE t1;
ANALYZE TABLE t2;
CREATE FUNCTION f1() RETURNS INTEGER RETURN (SELECT MOD(COUNT(DISTINCT pk), 10) FROM t2);
