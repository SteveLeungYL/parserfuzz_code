...
SELECT t.*, STDDEV_POP(t.rank) OVER (ROWS UNBOUNDED PRECEDING) std, VARIANCE(t.rank) OVER (ROWS UNBOUNDED PRECEDING) variance FROM (SELECT sex, id, date, ROW_NUMBER() OVER w AS row_no, RANK() OVER w AS `rank` FROM t1,t2 WHERE t1.id=t2.user_id WINDOW w AS (PARTITION BY date ORDER BY id) ) AS t;
...
SELECT t1.*, STD(id) OVER (ORDER BY id ROWS UNBOUNDED PRECEDING) std, variance(id) OVER (ORDER BY id ROWS UNBOUNDED PRECEDING) VARIANCE, RANK() OVER (ORDER BY sex,id) `rank`, ROW_NUMBER() OVER (ORDER BY sex,id) row_num FROM t1;
SELECT t.*, STDDEV(id + r00 + r01) OVER (ORDER BY id ROWS UNBOUNDED PRECEDING) AS std, variance(id + r00 + r01) OVER (ORDER BY id ROWS UNBOUNDED PRECEDING) AS VARIANCE FROM ( SELECT t1.*, RANK() OVER (ORDER BY sex, id) AS r00, RANK() OVER (ORDER BY sex, id DESC) AS r01, RANK() OVER (ORDER BY sex, id DESC) AS r02, RANK() OVER (PARTITION BY id ORDER BY sex) AS r03, RANK() OVER (ORDER BY sex,id) AS r04, RANK() OVER (ORDER BY sex,id) AS r05, RANK() OVER (ORDER BY sex, id) AS r06, RANK() OVER (ORDER BY sex, id) AS r07, RANK() OVER (ORDER BY sex, id) AS r08, RANK() OVER (ORDER BY sex, id) AS r09, RANK() OVER (ORDER BY sex, id) AS r10, RANK() OVER (ORDER BY sex, id) AS r11, RANK() OVER (ORDER BY sex, id) AS r12, RANK() OVER (ORDER BY sex, id) AS r13, RANK() OVER (ORDER BY sex, id) AS r14 FROM t1) t;
DROP TABLE t;
SELECT AVG(id) OVER w, STD(id) OVER w std, VARIANCE(id) OVER w variance FROM t1 WINDOW w AS (PARTITION BY sex);
SELECT * FROM ( SELECT id, SUM(id) OVER w, STD(id) OVER w std, VARIANCE(id) OVER w variance, sex FROM t1 WINDOW w AS (PARTITION BY sex) ) alias ORDER BY id;
...
SELECT id, STD(id) OVER w std, VARIANCE(id) OVER w variance, sex FROM t1 WINDOW w AS (PARTITION BY sex ORDER BY id ROWS BETWEEN 2 PRECEDING AND 1 PRECEDING);
CREATE VIEW v AS SELECT id, SUM(id) OVER w, STD(id) OVER w std, VARIANCE(id) OVER w variance, sex FROM t1 WINDOW w AS (PARTITION BY sex ORDER BY id ROWS BETWEEN 2 PRECEDING AND 1 PRECEDING);
SHOW CREATE VIEW v;
SELECT * FROM v;
DROP VIEW v;
SELECT SUM(id) OVER w, STDDEV(id) OVER w std, VARIANCE(id) OVER w variance FROM t1 WINDOW w AS (PARTITION BY sex ORDER BY id ROWS BETWEEN 2 PRECEDING AND 1 PRECEDING);
SELECT id, SUM(id) OVER w, STD(id) OVER w std, VARIANCE(id) OVER w variance, sex FROM t1 WINDOW w AS (PARTITION BY sex ORDER BY id ROWS BETWEEN 1 FOLLOWING AND 2 FOLLOWING);
SELECT SUM(id) OVER w, COUNT(*) OVER w, STD(id) OVER w std, VARIANCE(id) OVER w variance FROM t1 WINDOW w AS (PARTITION BY sex ORDER BY id ROWS BETWEEN 1 FOLLOWING AND 2 FOLLOWING);
SELECT id, AVG(id) OVER (ROWS UNBOUNDED PRECEDING) avg, STD(id) OVER (ROWS UNBOUNDED PRECEDING) std FROM t1;
SELECT id, AVG(id) OVER (ROWS UNBOUNDED PRECEDING), VARIANCE(id) OVER (ROWS UNBOUNDED PRECEDING) variance FROM t1;
SELECT id, AVG(id) OVER w avg, COUNT(id) OVER w count, STD(id) OVER w std, VARIANCE(id) OVER w variance FROM t1 WINDOW w AS (ORDER BY id ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING);
...
SELECT d, STD(d) OVER w std, VARIANCE(d) OVER w variance FROM td WINDOW w AS (ORDER BY d RANGE BETWEEN 2 PRECEDING AND CURRENT ROW);
SELECT d, SUM(d) OVER w sum, STD(d) OVER w std, VARIANCE(d) OVER w variance FROM td WINDOW w AS (ORDER BY d RANGE BETWEEN 2 PRECEDING AND 2 FOLLOWING);
SELECT d, SUM(d) OVER w sum, STD(d) OVER w std, VARIANCE(d) OVER w variance FROM td WINDOW w AS (ORDER BY d RANGE BETWEEN CURRENT ROW AND 2 FOLLOWING);
INSERT INTO td SELECT * FROM td;
SELECT d, SUM(d) OVER w sum, VARIANCE(d) OVER w variance, STD(d) OVER w std FROM td WINDOW w AS (ORDER BY d RANGE BETWEEN 2 PRECEDING AND CURRENT ROW);
SELECT d, SUM(d) OVER w sum, VARIANCE(d) OVER w variance, STD(d) OVER w std FROM td WINDOW w AS (ORDER BY d RANGE BETWEEN 2 PRECEDING AND 2 FOLLOWING);
SELECT d, SUM(d) OVER w sum, VARIANCE(d) OVER w variance, STD(d) OVER w std FROM td WINDOW w AS (ORDER BY d RANGE BETWEEN CURRENT ROW AND 2 FOLLOWING);
DROP TABLE td;
SELECT ROW_NUMBER() OVER w `row_number`, id, STD(id) OVER w std, VARIANCE(id) OVER w variance, sex FROM t1 WINDOW w AS (PARTITION BY sex ORDER BY id ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING);
SELECT ROW_NUMBER() OVER w `row_number`, STD(id) OVER w std, VARIANCE(id) OVER w variance FROM t1 WINDOW w AS (PARTITION BY sex ORDER BY id ROWS BETWEEN 1 FOLLOWING AND 2 FOLLOWING);
INSERT INTO t1 VALUES (10, NULL);
SELECT RANK() OVER w `rank`, id, STD(id) OVER w std, VARIANCE(id) OVER w variance, sex FROM t1 WINDOW w AS (PARTITION BY sex ORDER BY id);
SELECT RANK() OVER w `rank`, STD(id) OVER w std, VARIANCE(id) OVER w variance FROM t1 WINDOW w AS (PARTITION BY sex ORDER BY id ROWS BETWEEN 1 FOLLOWING AND 2 FOLLOWING);
SELECT id, sex, STD(id) OVER w std, VARIANCE(id) OVER w variance, ROW_NUMBER() OVER w `row_number`, RANK() OVER w  `rank` FROM t1 WINDOW w AS (PARTITION BY sex ORDER BY id ROWS BETWEEN 2 PRECEDING AND 1 PRECEDING);
SELECT id, sex, STD(id) OVER w std, VARIANCE(id) OVER w variance, ROW_NUMBER() OVER w `row_number`, CUME_DIST() OVER w  `cume_dist` FROM t1 WINDOW w AS (PARTITION BY sex ORDER BY id ROWS BETWEEN 2 PRECEDING AND 1 PRECEDING);
...
SELECT user_id, STD(user_id) OVER w, VARIANCE(user_id) OVER w1 FROM t2 WINDOW w AS (ORDER BY user_id), w1 AS (ORDER BY user_id);
SELECT NTILE(5) OVER w, ROW_NUMBER() OVER w, id, STD(id) OVER w FROM t1 WINDOW w AS (ORDER BY id DESC RANGE 2 PRECEDING);
SELECT p, NTILE(5) OVER w, ROW_NUMBER() OVER w, id, VARIANCE(id) OVER w FROM t11 WINDOW w AS (PARTITION BY p ORDER BY id DESC RANGE 2 PRECEDING);
SELECT NTILE(5) OVER w, ROW_NUMBER() OVER w, id, STD(id) OVER w FROM t1 WINDOW w AS (ORDER BY id DESC RANGE INTERVAL 2 MONTH PRECEDING);
...
SELECT d, STD(d) OVER w std, VARIANCE(d) OVER w variance, COUNT(*) OVER w FROM t3 WINDOW w AS (ORDER BY d RANGE BETWEEN 2.1 PRECEDING AND 1.1 FOLLOWING);
DROP TABLE t3;
...
INSERT INTO tj(j) VALUES ('3.14');
INSERT INTO tj(j) VALUES ('[1,2,3]');
SELECT j, JSON_TYPE(j), STD(CASE WHEN JSON_TYPE(j) = 'ARRAY' THEN j->"$[0]" ELSE j END) OVER (ORDER BY j ROWS 3 PRECEDING) FROM tj;
...
INSERT INTO t1 VALUES (NULL,3),(NULL,3),(NULL,30),(NULL,30),(4,3), (2,5),(3,7),(1,10),(5,20),(3,30);
SELECT i, j, STD(i) OVER (PARTITION BY j ORDER BY i) FROM t1;
DROP TABLE t1;
CREATE TABLE t1(c1 INT,c2 INT);
INSERT INTO t1 VALUES (1,5),(2,-1900740608),(3,1938161664),(4,2),(5,6),(6,5),(7,5), (8,-921239552),(9,-1347354624),(10,5),(11,-870580224),(12,-30913), (13,1267597312),(14,93454336),(15,6),(16,-67174400),(17,10751180), (18,659685376),(19,-790691840),(20,882114560),(21,1952907264);
