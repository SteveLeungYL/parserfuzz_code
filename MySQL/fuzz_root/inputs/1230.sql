...
execute stmt1;
execute stmt1;
prepare stmt1 from "select * from t1 into outfile '/data/yu/Squirrel_DBMS_Fuzzing/MySQL_source/mysql-server-inst/bld/mysql-test/var/tmp/f1.txt'";
execute stmt1;
deallocate prepare stmt1;
drop table t1;
...
PREPARE st_19182 FROM "CREATE TABLE t2 (i INT, j INT, KEY (i), KEY(j)) SELECT i FROM t1";
EXECUTE st_19182;
DESC t2;
DROP TABLE t2;
EXECUTE st_19182;
...
prepare stmt from "create table t1 (c char(10)) data directory='/data/yu/Squirrel_DBMS_Fuzzing/MySQL_source/mysql-server-inst/bld/mysql-test/var/tmp'";
execute stmt;
...
deallocate prepare stmt;
drop table t1,t2;
SET @aux= "SELECT COUNT(*)                 FROM INFORMATION_SCHEMA.COLUMNS A,                 INFORMATION_SCHEMA.COLUMNS B                 WHERE A.TABLE_SCHEMA = B.TABLE_SCHEMA                 AND A.TABLE_NAME = B.TABLE_NAME                 AND A.COLUMN_NAME = B.COLUMN_NAME AND                 A.TABLE_NAME = 'user'";
...
create table t1 (id int);
insert into t1 values(1);
create procedure p1(a int, b int) begin declare c int; select max(id)+1 into c from t1; insert into t1 select a+b; insert into t1 select a-b; insert into t1 select a-c; end;
set @a= 3, @b= 4;
...
INSERT INTO t1 VALUES (1), (2);
SELECT t1.i FROM t1 JOIN v1 ON t1.i = v1.i WHERE EXISTS (SELECT * FROM t1 WHERE v1.i = 1);
PREPARE stmt FROM "SELECT t1.i FROM t1 JOIN v1 ON t1.i = v1.i WHERE EXISTS (SELECT * FROM t1 WHERE v1.i = 1)";
EXECUTE stmt;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
DROP VIEW v1;
...
prepare stmt from "delete from t2 where (select (select max(b) from t1 group by a having a < 2) x from t1) > 10000";
delete from t2 where (select (select max(b) from t1 group by a having a < 2) x from t1) > 10000;
execute stmt;
execute stmt;
deallocate prepare stmt;
drop table t1, t2;
...
PREPARE stmt FROM "SELECT 1 FROM t1 WHERE ROW(a, b) >= ROW('1', (SELECT 1 FROM t1 WHERE a > '1234abc'))";
EXECUTE stmt;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
DROP TABLE t1;
...
CREATE PROCEDURE p_string( IN v0 INT, OUT v1 CHAR(32), IN v2 CHAR(32), INOUT v3 CHAR(32)) BEGIN SET v0 = -1; SET v1 = 'test_v1'; SET v2 = 'n/a'; SET v3 = 'test_v3'; END;
CREATE PROCEDURE p_double( IN v0 INT, OUT v1 DOUBLE(4, 2), IN v2 DOUBLE(4, 2), INOUT v3 DOUBLE(4, 2)) BEGIN SET v0 = -1; SET v1 = 12.34; SET v2 = 98.67; SET v3 = 56.78; END;
CREATE PROCEDURE p_int( IN v0 CHAR(10), OUT v1 INT, IN v2 INT, INOUT v3 INT) BEGIN SET v0 = 'n/a'; SET v1 = 1234; SET v2 = 9876; SET v3 = 5678; END;
CREATE PROCEDURE p_decimal( IN v0 INT, OUT v1 DECIMAL(4, 2), IN v2 DECIMAL(4, 2), INOUT v3 DECIMAL(4, 2)) BEGIN SET v0 = -1; SET v1 = 12.34; SET v2 = 98.67; SET v3 = 56.78; END;
...
CREATE PROCEDURE p2(OUT v2 CHAR(10)) BEGIN SET @query = 'CALL p1(?)'; PREPARE stmt1 FROM @query; EXECUTE stmt1 USING @u1; DEALLOCATE PREPARE stmt1; SET v2 = @u1; END;
CALL p2(@a);
SELECT @a;
DROP PROCEDURE p1;
DROP PROCEDURE p2;
...
PREPARE s1 FROM  " SELECT c1, t2.c2, count(c3) FROM   (   SELECT 3 as c2 FROM dual WHERE @x = 1   UNION   SELECT 2       FROM dual WHERE @x = 1 OR @x = 2   ) AS t1,   (   SELECT '2012-03-01 01:00:00' AS c1, 3 as c2, 1 as c3 FROM dual   UNION   SELECT '2012-03-01 02:00:00',       3,       2       FROM dual   UNION   SELECT '2012-03-01 01:00:00',       2,       1       FROM dual   ) AS t2 WHERE t2.c2 = t1.c2 GROUP BY c1,c2 ORDER BY c1,c2 ";
SET @x = 1;
SELECT c1, t2.c2, count(c3) FROM ( SELECT 3 as c2 FROM dual WHERE @x = 1 UNION SELECT 2       FROM dual WHERE @x = 1 OR @x = 2 ) AS t1, ( SELECT '2012-03-01 01:00:00' AS c1, 3 as c2, 1 as c3 FROM dual UNION SELECT '2012-03-01 02:00:00',       3,       2       FROM dual UNION SELECT '2012-03-01 01:00:00',       2,       1       FROM dual ) AS t2 WHERE t2.c2 = t1.c2 GROUP BY c1, c2 ORDER BY c1, c2;
EXECUTE s1;
SET @x = 2;
SELECT c1, t2.c2, count(c3) FROM ( SELECT 3 as c2 FROM dual WHERE @x = 1 UNION SELECT 2       FROM dual WHERE @x = 1 OR @x = 2 ) AS t1, ( SELECT '2012-03-01 01:00:00' AS c1, 3 as c2, 1 as c3 FROM dual UNION SELECT '2012-03-01 02:00:00',       3,       2       FROM dual UNION SELECT '2012-03-01 01:00:00',       2,       1       FROM dual ) AS t2 WHERE t2.c2 = t1.c2 GROUP BY c1, c2 ORDER BY c1, c2;
EXECUTE s1;
...
PREPARE stmt FROM ' SELECT (SELECT 1 FROM t2 WHERE ot.a) AS d FROM t1 AS ot GROUP BY d';
EXECUTE stmt;
EXECUTE stmt;
