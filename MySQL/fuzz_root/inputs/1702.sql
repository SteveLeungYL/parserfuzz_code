...
SELECT CASE _latin1'a' COLLATE latin1_general_ci WHEN _latin1'a' COLLATE latin1_danish_ci  THEN 1 WHEN _latin1'a' COLLATE latin1_swedish_ci THEN 2 END;
...
CREATE TABLE t1 SELECT  COALESCE(1), COALESCE(1.0),COALESCE('a'), COALESCE(1,1.0), COALESCE(1,'1'),COALESCE(1.1,'1'), COALESCE('a' COLLATE latin1_bin,'b');
explain SELECT  COALESCE(1), COALESCE(1.0),COALESCE('a'), COALESCE(1,1.0), COALESCE(1,'1'),COALESCE(1.1,'1'), COALESCE('a' COLLATE latin1_bin,'b');
SHOW CREATE TABLE t1;
DROP TABLE t1;
...
INSERT INTO source VALUES (1,0,0,-9223372036854775808), (1,0,18446744073709551615,9223372036854775807);
CREATE TABLE target SELECT IF(bt,i8u,i8s) AS u, IF(bf,i8u,i8s) AS s FROM source;
SHOW CREATE TABLE target;
SELECT IF(bt,i8u,i8s) AS u, IF(bf,i8u,i8s) AS s FROM source;
SELECT * FROM target;
DROP TABLE target;
CREATE TABLE target SELECT CASE WHEN bt THEN i8u ELSE i8s END AS u, CASE WHEN bf THEN i8u ELSE i8s END AS s FROM source;
SHOW CREATE TABLE target;
SELECT CASE WHEN bt THEN i8u ELSE i8s END AS u, CASE WHEN bf THEN i8u ELSE i8s END AS s FROM source;
SELECT * FROM target;
DROP TABLE target;
CREATE TABLE target SELECT CASE bt WHEN TRUE THEN i8u WHEN FALSE THEN i8s END AS u, CASE bf WHEN TRUE THEN i8u WHEN FALSE THEN i8s END AS s FROM source;
SHOW CREATE TABLE target;
...
SELECT * FROM target;
DROP TABLE target;
CREATE TABLE target SELECT COALESCE(i8u, i8s) AS u, COALESCE(i8s, i8u) AS s FROM source;
SHOW CREATE TABLE target;
...
CREATE TABLE u AS SELECT HEX(a), IFNULL(a,a) AS b, IFNULL(a,a)+0 AS c, IFNULL(a+0,a+0) AS d, IFNULL(a+0,a) AS e, IFNULL(a,a+0) AS f FROM t;
SHOW CREATE TABLE u;
DROP TABLE t, u;
...
CREATE FUNCTION f(a INTEGER) RETURNS INTEGER DETERMINISTIC RETURN IFNULL(a, 666);
...
SELECT IFNULL(e2,i1) nullif_c, IF(e2 IS NULL,i1,e2) if_c, SUM(d1) FROM t GROUP BY e2,i1 ORDER BY nullif_c, SUM(d1);
DROP TABLE t;
...
SELECT col_int FROM t1 WHERE CASE WHEN POWER(col_double, col_int) THEN j ELSE j END;
DROP TABLE t1;
