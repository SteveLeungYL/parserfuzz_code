...
select * from t1 where (a = 1 AND b < 'd' AND (c = 'b' OR (c = 'c' AND d = 1)) OR (a = 1 AND b >= 'a' AND (c = 'c' OR (c = 'd' AND d = 2))));
drop table t1;
...
select data_type from information_schema.columns where column_name='to_seconds';
drop table t1;
...
drop table t1;
create table t1 (a char(6)) partition by range columns(a) (partition p0 values less than (H23456), partition p1 values less than (M23456));
create table t1 (a char(6)) partition by range columns(a) (partition p0 values less than (23456), partition p1 values less than (23456));
create table t1 (a int, b int) partition by range columns(a,b) (partition p0 values less than (10));
create table t1 (a int, b int) partition by range columns(a,b) (partition p0 values less than (1,1,1));
...
alter table t1 reorganize partition p0 into (partition p01 values less than (0,3), partition p02 values less than (1,1));
alter table t1 reorganize partition p2 into (partition p2 values less than(9,6,1));
