...
DROP TABLE t;
CREATE PROCEDURE p1() BEGIN CREATE TABLE t(a INT,b INT,UNIQUE INDEX i((a+b))); END;
CALL p1();
CALL p1();
DROP TABLE t;
CALL p1();
DROP TABLE t;
DROP PROCEDURE p1;
CREATE TABLE t1 ( col1 FLOAT , col2 TIMESTAMP , col3 YEAR , INDEX ((ABS(col1))) , INDEX ((ADDDATE(col2, INTERVAL 2 DAY))) , INDEX ((ABS(col3))) );
DROP TABLE t1;
CREATE TABLE t1 ( col4 TEXT NOT NULL , INDEX ((ST_AsBinary(col4))) );
...
ALTER TABLE t1 DROP COLUMN col3;
DROP TABLE t1;
CREATE TABLE t ( col1 INT , UNIQUE INDEX regular_index (col1) , UNIQUE INDEX functional_index ((ABS(col1))));
...
CREATE TABLE t1 ( j1 JSON, j3 JSON, KEY my_idx ((CAST(j1->'$[0]' as SIGNED))), KEY my_idx_char ((CAST(j3->'$[0]' as CHAR(10)))) );
...
LOAD DATA INFILE '/data/yu/Squirrel_DBMS_Fuzzing/MySQL_source/mysql-server-inst/bld/mysql-test/var/tmp/bug30838749' INTO TABLE t2 LINES STARTING BY 'EXPLAIN' TERMINATED BY 'EOL';
...
CREATE TABLE t5 (id INT, name VARCHAR(50), INDEX ((SUBSTR(name, 1, 2))));
CREATE TABLE t6 (id INT, name VARCHAR(50), INDEX ((SUBSTR(NAME, 1, 2))));
CREATE TABLE t7 (id INT, NAME VARCHAR(50), INDEX ((SUBSTR(name, 1, 2))));
CREATE TABLE t8 (id INT, NAME VARCHAR(50), INDEX ((SUBSTR(NAME, 1, 2))));
DROP TABLE t1;
DROP TABLE t2;
DROP TABLE t3;
DROP TABLE t4;
