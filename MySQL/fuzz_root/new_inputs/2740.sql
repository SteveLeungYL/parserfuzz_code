...
CREATE TABLE t4 ( seq_0_id bigint(21) DEFAULT '0' NOT NULL, seq_1_id bigint(21) DEFAULT '0' NOT NULL, KEY seq_0_id (seq_0_id), KEY seq_1_id (seq_1_id) );
CREATE TABLE t5 ( seq_0_id bigint(21) DEFAULT '0' NOT NULL, seq_1_id bigint(21) DEFAULT '0' NOT NULL, KEY seq_1_id (seq_1_id), KEY seq_0_id (seq_0_id) );
...
drop table t1;
drop table t2;
CREATE TABLE t1 ( pcode varchar(8) DEFAULT '' NOT NULL );
...
CREATE TABLE t2 ( pcode varchar(8) DEFAULT '' NOT NULL, KEY pcode (pcode) );
...
drop table t1,t2;
create table t1 ( match_id tinyint(3) unsigned not null auto_increment, home tinyint(3) unsigned default '0', unique key match_id (match_id), key match_id_2 (match_id) );
insert into t1 values("1", "2");
create table t2 ( player_id tinyint(3) unsigned default '0', match_1_h tinyint(3) unsigned default '0', key player_id (player_id) );
...
ANALYZE TABLE t1, t2;
explain select s.*, '*', m.*, (s.match_1_h - m.home) UUX from  (t2 s left join t1 m on m.match_id = 1)  order by m.match_id desc;
explain select s.*, '*', m.*, (s.match_1_h - m.home) UUX from  (t2 s left join t1 m on m.match_id = 1)  order by UUX desc;
select s.*, '*', m.*, (s.match_1_h - m.home) UUX from  (t2 s left join t1 m on m.match_id = 1)  order by UUX desc;
explain select s.*, '*', m.*, (s.match_1_h - m.home) UUX from  t2 s straight_join t1 m where m.match_id = 1  order by UUX desc;
...
drop table t1, t2;
CREATE TABLE t1 ( ts_id bigint(20) default NULL, inst_id tinyint(4) default NULL, flag_name varchar(64) default NULL, flag_value text, UNIQUE KEY ts_id (ts_id,inst_id,flag_name) ) ENGINE=MyISAM DEFAULT CHARSET=utf8;
CREATE TABLE t2 ( ts_id bigint(20) default NULL, inst_id tinyint(4) default NULL, flag_name varchar(64) default NULL, flag_value text, UNIQUE KEY ts_id (ts_id,inst_id,flag_name) ) ENGINE=MyISAM DEFAULT CHARSET=utf8;
...
DROP TABLE t1,t2;
CREATE TABLE t1 ( id int(11) unsigned NOT NULL auto_increment, text_id int(10) unsigned default NULL, PRIMARY KEY  (id) );
INSERT INTO t1 VALUES("1", "0");
INSERT INTO t1 VALUES("2", "10");
CREATE TABLE t2 ( text_id char(3) NOT NULL default '', language_id char(3) NOT NULL default '', text_data text, PRIMARY KEY  (text_id,language_id) );
...
explain select name from t1 left join t2 on t1.x = t2.x and t1.y = t2.y left join t3 on t1.art = t3.id where t2.id =1 and t2.x = -5 and t2.y =-8 and t1.gid =1 and t2.gid =1 and t3.set_id =1;
...
CREATE VIEW v1 AS SELECT COALESCE(t2.EMPNUM,t1.EMPNUM) AS EMPNUM, NAME, GRP FROM t2 LEFT OUTER JOIN t1 ON t2.EMPNUM=t1.EMPNUM;
SELECT * FROM v1;
...
DROP TABLE t1,t2;
CREATE TABLE t1 ( f1 varchar(16) collate latin1_swedish_ci PRIMARY KEY, f2 varchar(16) collate latin1_swedish_ci );
CREATE TABLE t2 ( f1 varchar(16) collate latin1_swedish_ci PRIMARY KEY, f3 varchar(16) collate latin1_swedish_ci );
...
ANALYZE TABLE t1, t2;
EXPLAIN SELECT t1.a, COUNT( t2.b ), SUM( t2.b ), MAX( t2.b ) FROM t1 LEFT JOIN t2 USING( a ) GROUP BY t1.a WITH ROLLUP;
SELECT t1.a, COUNT( t2.b ), SUM( t2.b ), MAX( t2.b ) FROM t1 LEFT JOIN t2 USING( a ) GROUP BY t1.a WITH ROLLUP;
EXPLAIN SELECT t1.a, COUNT( t2.b ), SUM( t2.b ), MAX( t2.b ) FROM t1 JOIN t2 USING( a ) GROUP BY t1.a WITH ROLLUP;
SELECT t1.a, COUNT( t2.b ), SUM( t2.b ), MAX( t2.b ) FROM t1 JOIN t2 USING( a ) GROUP BY t1.a WITH ROLLUP;
DROP TABLE t1, t2;
CREATE TABLE t1(f1 INT, f2 INT, f3 INT);
INSERT INTO t1 VALUES (1, NULL, 3);
CREATE TABLE t2(f1 INT, f2 INT);
INSERT INTO t2 VALUES (2, 1);
EXPLAIN SELECT * FROM t1 LEFT JOIN t2 ON t1.f2 = t2.f2 WHERE (COALESCE(t1.f1, t2.f1), f3) IN ((1, 3), (2, 2));
SELECT * FROM t1 LEFT JOIN t2 ON t1.f2 = t2.f2 WHERE (COALESCE(t1.f1, t2.f1), f3) IN ((1, 3), (2, 2));
DROP TABLE t1, t2;
CREATE TABLE t1( a INT );
INSERT INTO t1 VALUES (1),(2);
SET optimizer_search_depth = 0;
EXPLAIN SELECT 1 FROM t1 tt3 LEFT  OUTER JOIN t1 tt4 ON 1 LEFT  OUTER JOIN t1 tt5 ON 1 LEFT  OUTER JOIN t1 tt6 ON 1 LEFT  OUTER JOIN t1 tt7 ON 1 LEFT  OUTER JOIN t1 tt8 ON 1 RIGHT OUTER JOIN t1 tt2 ON 1 RIGHT OUTER JOIN t1 tt1 ON 1 STRAIGHT_JOIN    t1 tt9 ON 1;
...
EXPLAIN SELECT STRAIGHT_JOIN COUNT(*) FROM t1 ta1 RIGHT JOIN t2 ta2 JOIN t2 ta3 ON ta2.f1 ON ta3.f1;
DROP TABLE t1, t2;
CREATE TABLE t1(f1 INT, PRIMARY KEY (f1));
INSERT INTO t1 VALUES (1),(2);
EXPLAIN SELECT STRAIGHT_JOIN jt1.f1 FROM t1 AS jt1 LEFT JOIN t1 AS jt2 RIGHT JOIN t1 AS jt3 JOIN t1 AS jt4 ON 1 LEFT JOIN t1 AS jt5 ON 1 ON 1 RIGHT JOIN t1 AS jt6 ON jt6.f1 ON 1;
EXPLAIN SELECT STRAIGHT_JOIN jt1.f1 FROM t1 AS jt1 RIGHT JOIN t1 AS jt2 RIGHT JOIN t1 AS jt3 JOIN t1 AS jt4 ON 1 LEFT JOIN t1 AS jt5 ON 1 ON 1 RIGHT JOIN t1 AS jt6 ON jt6.f1 ON 1;
DROP TABLE t1;
...
EXPLAIN SELECT * FROM t1 LEFT JOIN t2 ON t2.f1 = t1.f1 WHERE t1.f1 = 4 GROUP BY t2.f1, t2.f2;
SELECT * FROM t1 LEFT JOIN t2 ON t2.f1 = t1.f1 WHERE t1.f1 = 4 GROUP BY t2.f1, t2.f2;
EXPLAIN SELECT * FROM t1 LEFT JOIN t2 ON t2.f1 = t1.f1 WHERE t1.f1 = 4 AND t2.f1 IS NOT NULL AND t2.f2 IS NOT NULL GROUP BY t2.f1, t2.f2;
...
EXPLAIN SELECT * FROM t1 LEFT JOIN t2 ON t1.col_int_unique = t2.col_int_unique AND t1.col_int = t2.col_int  WHERE t1.pk=1;
SELECT * FROM t1 LEFT JOIN t2 ON t1.col_int_unique = t2.col_int_unique AND t1.col_int = t2.col_int  WHERE t1.pk=1;
DROP TABLE t1,t2;
CREATE TABLE `BB` ( `pk` int(11) NOT NULL AUTO_INCREMENT, `time_key` time DEFAULT NULL, `varchar_key` varchar(1) DEFAULT NULL, `varchar_nokey` varchar(1) DEFAULT NULL, PRIMARY KEY (`pk`), KEY `time_key` (`time_key`), KEY `varchar_key` (`varchar_key`) ) ENGINE=MyISAM AUTO_INCREMENT=11 DEFAULT CHARSET=latin1;
...
DROP TABLE BB;
CREATE TABLE `BB` ( `col_datetime_key` datetime DEFAULT NULL, `col_varchar_key` varchar(1) DEFAULT NULL, `col_varchar_nokey` varchar(1) DEFAULT NULL, KEY `col_datetime_key` (`col_datetime_key`), KEY `col_varchar_key` (`col_varchar_key`) ) ENGINE=MyISAM DEFAULT CHARSET=latin1;
...
explain SELECT t1.a FROM t1 LEFT JOIN (t2 LEFT JOIN t3 ON t2.a) ON 0 WHERE t1.a OR t3.a;
SELECT t1.a FROM t1 LEFT JOIN (t2 LEFT JOIN t3 ON t2.a) ON 0 WHERE t1.a OR t3.a;
explain SELECT t1.a FROM t1 LEFT JOIN (t2 LEFT JOIN (t3 LEFT JOIN t4 ON 1) ON t2.a) ON 0 WHERE t1.a OR t4.a;
SELECT t1.a FROM t1 LEFT JOIN (t2 LEFT JOIN (t3 LEFT JOIN t4 ON 1) ON t2.a) ON 0 WHERE t1.a OR t4.a;
DROP TABLE t1,t2,t3,t4;
CREATE TABLE t1 ( pk INT NOT NULL, col_int_key INT, col_int INT, PRIMARY KEY (pk), KEY col_int_key (col_int_key) );
INSERT INTO t1 VALUES (6, -448724992, NULL);
CREATE TABLE t2 ( col_int INT, col_varchar_10 VARCHAR(10) );
INSERT INTO t2 VALUES (6,'afasdkiyum');
CREATE TABLE t3 ( col_varchar_10 VARCHAR(10), col_int INT );
CREATE TABLE t4 ( pk INT NOT NULL, PRIMARY KEY (pk) );
...
EXPLAIN FORMAT=tree SELECT t1.col_int FROM t1 LEFT JOIN t2 LEFT JOIN t3 JOIN t4 ON t3.col_int  = t4.pk ON t2.col_varchar_10 = t3.col_varchar_10 ON t2.col_int = t1.pk WHERE   t1.col_int_key IS NULL OR t4.pk < t3.col_int;
DROP TABLE t1,t2,t3,t4;
...
drop table t1;
CREATE TABLE t1 ( col_varchar_10 VARCHAR(10), col_int_key INTEGER, col_varchar_10_key VARCHAR(10), pk INTEGER NOT NULL, PRIMARY KEY (pk), KEY (col_int_key), KEY (col_varchar_10_key) );
INSERT INTO t1 VALUES ('q',NULL,'o',1);
CREATE TABLE t2 ( pk INTEGER NOT NULL AUTO_INCREMENT, col_varchar_10_key VARCHAR(10), col_int_key INTEGER, col_varchar_10 VARCHAR(10), PRIMARY KEY (pk), KEY (col_varchar_10_key), KEY col_int_key (col_int_key) );
INSERT INTO t2 VALUES (1,'r',NULL,'would'),(2,'tell',-655032320,'t'), (3,'d',9,'a'),(4,'gvafasdkiy',6,'ugvafasdki'), (5,'that\'s',NULL,'she'),(6,'bwftwugvaf',7,'cbwftwugva'), (7,'f',-700055552,'mkacbwftwu'),(8,'a',9,'be'), (9,'d',NULL,'u'),(10,'ckiixcsxmk',NULL,'o');
...
drop table t1,t2,t3;
CREATE TABLE t1 ( pk int(11) NOT NULL, col_varchar_10_latin1_key varchar(10) DEFAULT NULL ) ENGINE=InnoDB DEFAULT CHARSET=latin1;
INSERT INTO t1 VALUES (1,'1');
CREATE TABLE t2 ( pk int(11) NOT NULL ) ENGINE=InnoDB DEFAULT CHARSET=latin1;
INSERT INTO t2 VALUES (1);
CREATE TABLE t3 ( pk int(11) NOT NULL ) ENGINE=InnoDB DEFAULT CHARSET=latin1;
INSERT INTO t3 VALUES (1);
CREATE TABLE t4 ( pk int(11) NOT NULL, col_int int(11) DEFAULT NULL, col_int_key int(11) DEFAULT NULL, col_varchar_10_latin1_key varchar(10) DEFAULT NULL ) ENGINE=InnoDB DEFAULT CHARSET=latin1;
INSERT INTO t4 VALUES (1,1,1,'1');
CREATE TABLE t5 ( col_int int(11) DEFAULT NULL, col_varchar_10_utf8_key varchar(10) CHARACTER SET utf8 DEFAULT NULL ) ENGINE=InnoDB DEFAULT CHARSET=latin1;
INSERT INTO t5 VALUES (1,'1');
CREATE TABLE t6 ( col_int_key int(11) DEFAULT NULL, col_varchar_10_latin1_key varchar(10) DEFAULT NULL, pk int(11) NOT NULL ) ENGINE=InnoDB DEFAULT CHARSET=latin1;
INSERT INTO t6 VALUES (1,'1',1);
SELECT STRAIGHT_JOIN t6a.pk, t2.pk FROM t6 AS t6a LEFT JOIN ( t2 RIGHT JOIN ( ( t1 LEFT JOIN ( t4 JOIN t3 ON t4.col_int ) ON t4.col_int_key = t1.pk ) LEFT JOIN ( t5 JOIN t6 AS t6b ON t5.col_varchar_10_utf8_key = t6b.col_varchar_10_latin1_key ) ON t1.pk = t5.col_int ) ON t4.col_varchar_10_latin1_key = t1.col_varchar_10_latin1_key AND t5.col_varchar_10_utf8_key = 0 ) ON t6a.pk IS TRUE WHERE t6b.col_int_key IS TRUE ;
EXPLAIN SELECT STRAIGHT_JOIN t6a.pk, t2.pk FROM t6 AS t6a LEFT JOIN ( t2 RIGHT JOIN ( ( t1 LEFT JOIN ( t4 JOIN t3 ON t4.col_int ) ON t4.col_int_key = t1.pk ) LEFT JOIN ( t5 JOIN t6 AS t6b ON t5.col_varchar_10_utf8_key = t6b.col_varchar_10_latin1_key ) ON t1.pk = t5.col_int ) ON t4.col_varchar_10_latin1_key = t1.col_varchar_10_latin1_key AND t5.col_varchar_10_utf8_key = 0 ) ON t6a.pk IS TRUE WHERE t6b.col_int_key IS TRUE ;
SELECT t6a.pk, t2.pk FROM t6 AS t6a LEFT JOIN ( t2 RIGHT JOIN ( ( t1 LEFT JOIN ( t4 JOIN t3 ON t4.col_int ) ON t4.col_int_key = t1.pk ) LEFT JOIN ( t5 JOIN t6 AS t6b ON t5.col_varchar_10_utf8_key = t6b.col_varchar_10_latin1_key ) ON t1.pk = t5.col_int ) ON t4.col_varchar_10_latin1_key = t1.col_varchar_10_latin1_key AND t5.col_varchar_10_utf8_key = 0 ) ON t6a.pk IS TRUE WHERE t6b.col_int_key IS TRUE ;
EXPLAIN SELECT t6a.pk, t2.pk FROM t6 AS t6a LEFT JOIN ( t2 RIGHT JOIN ( ( t1 LEFT JOIN ( t4 JOIN t3 ON t4.col_int ) ON t4.col_int_key = t1.pk ) LEFT JOIN ( t5 JOIN t6 AS t6b ON t5.col_varchar_10_utf8_key = t6b.col_varchar_10_latin1_key ) ON t1.pk = t5.col_int ) ON t4.col_varchar_10_latin1_key = t1.col_varchar_10_latin1_key AND t5.col_varchar_10_utf8_key = 0 ) ON t6a.pk IS TRUE WHERE t6b.col_int_key IS TRUE ;
EXPLAIN SELECT * FROM t5 LEFT JOIN t6 ON t5.col_int=1000 WHERE t6.col_int_key IS TRUE;
EXPLAIN SELECT * FROM t5 LEFT JOIN t6 ON t5.col_int=1000 WHERE t6.col_int_key IS NOT TRUE;
...
explain format=json SELECT straight_join t1.vc, t1.ik FROM t1 JOIN t1 AS t2 ON t1.vc=t2.vc LEFT JOIN t1 AS t3 ON t1.vc=t3.vc;
...
insert into t1 values(1);
explain select * from t1 left join t1 as t2 on t2.a=12 where t1.a=1;
select * from t1 left join t1 as t2 on t2.a=12 where t1.a=1;
drop table t1;
CREATE TABLE t1 ( pk INT, col_int_key INT, col_varchar_key VARCHAR(1), PRIMARY KEY (pk), KEY col_varchar_key (col_varchar_key, col_int_key) ) ENGINE=MyISAM;
...
CREATE TABLE t2 ( pk INT, col_int_key INT, col_varchar_key VARCHAR(1), PRIMARY KEY (pk) ) ENGINE=MyISAM;
...
CREATE TABLE t2 ( id int(11) NOT NULL PRIMARY KEY, f_id int(11), FOREIGN KEY (f_id) REFERENCES t2(id) ) ENGINE=InnoDB;
INSERT INTO t1 VALUES (1,'A1'),(2,'A2'),(3,'B');
INSERT INTO t2 VALUES (1,1),(2,2),(3,2),(4,3),(5,3);
ANALYZE TABLE t1;
ANALYZE TABLE t2;
EXPLAIN SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE t1.name LIKE 'A%';
SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE t1.name LIKE 'A%';
PREPARE stmt FROM "SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE t1.name LIKE 'A%'";
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
EXPLAIN SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE t1.name LIKE 'A%' OR 'ASDFGH' IS NULL;
SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE t1.name LIKE 'A%' OR 'ASDFGH' IS NULL;
PREPARE stmt FROM "SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE t1.name LIKE 'A%' OR 'ASDFGH' IS NULL";
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
EXPLAIN SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE 0 OR 0;
SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE 0 OR 0;
PREPARE stmt FROM "SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE 0 OR 0";
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
EXPLAIN SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE 1 OR 0;
SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE 1 OR 0;
PREPARE stmt FROM "SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE 1 OR 0";
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
EXPLAIN SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE NOT(NOT (0)) OR 1;
SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE NOT(NOT (0)) OR 1;
PREPARE stmt FROM "SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE NOT(NOT (0)) OR 1";
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
EXPLAIN SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE NOT(NOT (0)) OR 0 = 1;
SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE NOT(NOT (0)) OR 0 = 1;
PREPARE stmt FROM "SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE NOT(NOT (0)) OR 0 = 1";
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
EXPLAIN SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE 1 IS NOT TRUE AND 0 IS TRUE ;
SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE 1 IS NOT TRUE AND 0 IS TRUE ;
PREPARE stmt FROM "SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE 1 IS NOT TRUE AND 0 IS TRUE ";
EXECUTE stmt;
EXPLAIN SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE 'a' LIKE 1 OR 2 LIKE '2%' ;
SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE 'a' LIKE 1 OR 2 LIKE '2%' ;
PREPARE stmt FROM "SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE 'a' LIKE 1 OR 2 LIKE '2%' ";
EXECUTE stmt;
EXPLAIN SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE 1 IS NOT NULL;
SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE 1 IS NOT NULL;
PREPARE stmt FROM "SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE 1 IS NOT NULL";
EXECUTE stmt;
EXPLAIN SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE (1<2 AND 2<3) OR (1>1 AND 2<3);
SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE (1<2 AND 2<3) OR (1>1 AND 2<3);
PREPARE stmt FROM "SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE (1<2 AND 2<3) OR (1>1 AND 2<3)";
EXECUTE stmt;
EXPLAIN SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE (1>=1 AND 2<=2) OR (1=1 AND 2<3);
SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE (1>=1 AND 2<=2) OR (1=1 AND 2<3);
PREPARE stmt FROM "SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE (1>=1 AND 2<=2) OR (1=1 AND 2<3)";
EXECUTE stmt;
EXPLAIN SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE (1<>1 AND 2=2) OR (1 BETWEEN 0 AND 2);
SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE (1<>1 AND 2=2) OR (1 BETWEEN 0 AND 2);
PREPARE stmt FROM "SELECT * FROM t2 LEFT JOIN t1 ON t2.f_id = t1.id WHERE (1<>1 AND 2=2) OR (1 BETWEEN 0 AND 2)";
EXECUTE stmt;
DROP TABLE t1,t2;
...
DROP TABLE t1, t2;
CREATE TABLE t1 ( col_int INT, pk INT NOT NULL, PRIMARY KEY (pk) );
INSERT INTO t1 VALUES (2,1), (2,2), (6,3), (4,4), (7,5), (188,6), (0,7), (6,8), (0,9), (9,10);
CREATE TABLE t2 ( pk INT NOT NULL, col_int INT, PRIMARY KEY (pk) );
INSERT INTO t2 VALUES (1,0), (2,0), (3,2), (4,NULL), (5,2), (6,3), (7,3), (8,100), (9,3), (10,6);
explain SELECT table2.pk, table1.col_int FROM t2 AS table1 LEFT JOIN t1 AS table2 ON table2.pk < table1.col_int AND table2.pk = table1.col_int;
...
explain SELECT table2.pk, table1.col_int FROM t2 AS table1 LEFT JOIN (SELECT * FROM t1) AS table2 ON table2.pk < table1.col_int AND table2.pk = table1.col_int;
...
DROP TABLE t1, t2;
CREATE TABLE t1 ( col_int INT DEFAULT NULL, col_int_key INT DEFAULT NULL, pk INT NOT NULL, PRIMARY KEY (pk), KEY test_idx (col_int_key,col_int) );
INSERT INTO t1 VALUES (0, -7, 1), (9, NULL, 15), (182, NULL, 25);
CREATE TABLE t2 ( col_int INT DEFAULT NULL, pk INT NOT NULL, PRIMARY KEY (pk) );
INSERT INTO t2 VALUES (NULL, 4), (-208, 5), (5, 6), (NULL, 75);
CREATE TABLE t3 ( col_datetime_key DATETIME DEFAULT NULL, pk INT NOT NULL, PRIMARY KEY (pk) );
INSERT INTO t3 VALUES ('1970-01-01 00:00:00', 5);
CREATE TABLE t4 ( col_int INT DEFAULT NULL, pk INT NOT NULL, col_int_key INT DEFAULT NULL, PRIMARY KEY (pk), KEY col_int_key (col_int_key) );
...
explain SELECT ta.a1, tb.b1, tb.b2 FROM ta LEFT OUTER JOIN tb ON ta.a2 = tb.b1 AND tb.b3 = 0;
SELECT ta.a1, tb.b1, tb.b2 FROM ta LEFT OUTER JOIN tb ON ta.a2 = tb.b1 AND tb.b3 = 0;
DROP TABLE ta, tb;
CREATE TABLE m ( machineid VARCHAR(32) NOT NULL, orderid bigint unsigned DEFAULT NULL, extra bigint unsigned DEFAULT NULL, PRIMARY KEY (machineid) );
INSERT INTO m (machineid, orderid) VALUES ('m1', NULL), ('m2', 2), ('m3', NULL), ('m4', NULL);
CREATE TABLE o ( orderid bigint unsigned NOT NULL, machineid VARCHAR(32) DEFAULT NULL, PRIMARY KEY (orderid) );
INSERT INTO o (orderid, machineid) VALUES (1, 'm2'), (2, 'm2');
SELECT o.*,'|' as sep, m.* FROM o LEFT JOIN m ON m.machineid = o.machineid AND m.orderid = o.orderid;
DROP TABLE m, o;
CREATE TABLE t1 ( adslot varchar(5) NOT NULL );
INSERT INTO t1(adslot) VALUES ('1'), ('2'), ('3');
CREATE TABLE t2 ( ionumber varchar(20) NOT NULL, adslot varchar(5) NOT NULL );
INSERT INTO t2 (ionumber, adslot) VALUES ('01602', 1), ('01602', 3);
CREATE TABLE t3 ( ionumber varchar(20) NOT NULL, ioattribute varchar(5) NOT NULL, PRIMARY KEY (ionumber) );
...
ANALYZE TABLE t1, t2;
EXPLAIN SELECT t1.a, subq.st_value FROM t1 LEFT JOIN (SELECT t2.t1_a, 'red' AS st_value FROM t2) AS subq ON subq.t1_a = t1.a;
EXPLAIN SELECT t1.a, subq.st_value FROM t1 LEFT JOIN (SELECT t2.t1_a, 'red' AS st_value FROM t2) AS subq ON subq.t1_a = t1.a ORDER BY t1.a;
EXPLAIN SELECT t1.a, subq.st_value FROM (SELECT t2.t1_a, 'red' AS st_value FROM t2) AS subq LEFT JOIN t1 ON subq.t1_a = t1.a ORDER BY t1.a;
...
EXPLAIN SELECT * FROM t1 LEFT JOIN ( (t2 LEFT JOIN t3 ON t3.i= t2.i) LEFT JOIN t4 ON t3.i= t4.i )ON t2.i= t1.i;
EXPLAIN SELECT * FROM t1 LEFT JOIN ( (t2 INNER JOIN t3 ON t3.i= t2.i) LEFT JOIN t4 ON t3.i= t4.i )ON t2.i= t1.i;
EXPLAIN SELECT * FROM t1 LEFT JOIN t2 ON t2.i= t1.i LEFT JOIN t3 ON t3.i= t2.i LEFT JOIN t4 ON t3.i= t4.i;
flush status;
...
INSERT INTO t1 VALUES (1), (2);
INSERT INTO t2 VALUES (1);
EXPLAIN SELECT * FROM (SELECT id FROM t1) AS a LEFT JOIN (SELECT id, 2 AS tall FROM t2) AS b ON a.id = b.id WHERE b.tall IS NOT NULL;
...
DROP TABLE t1,t2;
CREATE TABLE t1 ( pk int primary key auto_increment, col_int_unique int unique ) ENGINE=InnoDB;
INSERT INTO t1(col_int_unique) values (6),(7);
CREATE TABLE t2 ( pk int primary key auto_increment, col_int_key int(11) DEFAULT NULL, col_int_unique int(11) DEFAULT NULL, UNIQUE KEY `ix2` (col_int_key,col_int_unique), KEY col_int_key (col_int_key) ) ENGINE=InnoDB;
CREATE TABLE t3 ( pk int NOT NULL ) ENGINE=InnoDB;
...
DROP TABLE t1,t2,t3;
CREATE TABLE t1 ( col_int_unique INT DEFAULT NULL, col_int_key INT DEFAULT NULL, UNIQUE KEY col_int_unique (col_int_unique) ) ENGINE=InnoDB;
INSERT INTO t1 VALUES (5,0);
CREATE TABLE t2 ( col_char_16_unique char(16) DEFAULT NULL, col_int_key INT DEFAULT NULL, col_int_unique INT DEFAULT NULL, UNIQUE KEY col_int_unique (col_int_unique) ) ENGINE=InnoDB;
INSERT INTO t2 VALUES ("just",21,5);
CREATE TABLE t3 ( col_int INT DEFAULT NULL, col_char_16_unique CHAR(16) DEFAULT NULL, UNIQUE KEY col_char_16_unique (col_char_16_unique) ) ENGINE=InnoDB;
INSERT INTO t3 VALUES (9,"foo");
CREATE TABLE t4 ( col_int INT DEFAULT NULL, col_int_unique INT DEFAULT NULL, UNIQUE KEY col_int_unique (col_int_unique) ) ENGINE=InnoDB;
INSERT INTO t4 VALUES (9,5);
explain SELECT STRAIGHT_JOIN t3.col_int, t4.col_int, t3.col_int = t4.col_int or t4.col_int IS NULL FROM (t1 LEFT JOIN t2 ON t1.col_int_key = t2.col_int_key AND t1.col_int_unique = t2.col_int_unique LEFT JOIN t3 ON t3.col_char_16_unique = t2.col_char_16_unique LEFT JOIN t4 ON t4.col_int = t3.col_int AND    #Note, this pred term t4.col_int_unique = t1.col_int_unique ) WHERE t1.col_int_unique = 5;
SELECT STRAIGHT_JOIN t3.col_int, t4.col_int, t3.col_int = t4.col_int or t4.col_int IS NULL FROM (t1 LEFT JOIN t2 ON t1.col_int_key = t2.col_int_key AND t1.col_int_unique = t2.col_int_unique LEFT JOIN t3 ON t3.col_char_16_unique = t2.col_char_16_unique LEFT JOIN t4 ON t4.col_int = t3.col_int AND    #Note, this pred term t4.col_int_unique = t1.col_int_unique ) WHERE t1.col_int_unique = 5;
DROP TABLE t1, t2, t3, t4;
...
CREATE TABLE t3 ( f1 INTEGER, f2 INTEGER, KEY k2 (f2) );
...
DROP TABLE t1, t2, t3;
CREATE TABLE t1 ( col_int INTEGER, a INTEGER, b varchar(10), KEY key_a (a) );
...
CREATE TABLE t2 ( a INTEGER, b varchar(10), KEY key_b (b) );
INSERT INTO t2 VALUES (1,'j');
INSERT INTO t2 VALUES (2,'o');
INSERT INTO t2 VALUES (3,'z');
INSERT INTO t2 VALUES (4,'really');
ANALYZE TABLE t1, t2;
EXPLAIN format=tree SELECT t1.col_int as t1_ci, t1.a as t1_a, t1.b as t1_b, t2.a as t2_a, t2.b as t2_b, t3.col_int as t3_ci, t3.a as t3_a, t3.b as t3_b FROM t1 LEFT JOIN ( t2 LEFT JOIN t1 AS t3 ON t2.a=t3.a ) ON t1.b = t2.b;
...
CREATE TABLE t1 ( id INTEGER NOT NULL, b INTEGER, PRIMARY KEY (id) );
