...
select * from t1;
drop table t1;
CREATE TABLE t1 (EMPNUM   CHAR(3) NOT NULL, HOURS    DECIMAL(5));
CREATE TABLE t2 (EMPNUM   CHAR(3) NOT NULL, HOURS    BIGINT);
...
INSERT INTO t1 VALUES (1, 101.40, 21.40), (1, -80.00, 0.00), (2, 0.00, 0.00), (2, -13.20, 0.00), (2, 59.60, 46.40), (2, 30.40, 30.40), (3, 37.00, 7.40), (3, -29.60, 0.00), (4, 60.00, 15.40), (4, -10.60, 0.00), (4, -34.00, 0.00), (5, 33.00, 0.00), (5, -25.80, 0.00), (5, 0.00, 7.20), (6, 0.00, 0.00), (6, -51.40, 0.00);
SELECT i, SUM(d1) AS a, SUM(d2) AS b FROM t1 GROUP BY i HAVING a <> b;
SELECT i, ROUND(SUM(d1), 2) AS a, ROUND(SUM(d2), 2) AS b FROM t1 GROUP BY i HAVING a <> b;
drop table t1;
...
drop table t1;
create table t1 ( strippedproductid char(15) not null default '', zlevelprice decimal(10,2) default null, primary key (strippedproductid) );
create table t2 ( productid char(15) not null default '', zlevelprice char(21) default null, primary key (productid) );
...
drop table t1;
create table t1 as select from_days(s) as date,t from (select 1 as s,'t' as t union select null, null ) as sub1;
...
drop table t1;
create table t1 ( ua_id decimal(22,0) not null, ua_invited_by_id decimal(22,0) default NULL, primary key(ua_id) );
...
SELECT 1 FROM t1  JOIN (  SELECT t2.col0 FROM t2 RIGHT JOIN t1 USING(col0)  GROUP BY t2.col0 ) AS subq   WHERE t1.col1 + CAST(subq.col0 AS DECIMAL);
SELECT 1 FROM t1  JOIN (  SELECT t2.col0 FROM t2 RIGHT JOIN t1 USING(col0)  GROUP BY t2.col0 ) AS subq   WHERE CONCAT(t1.col1, CAST(subq.col0 AS DECIMAL));
DROP TABLE t1, t2;
