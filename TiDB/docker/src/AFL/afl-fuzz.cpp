...
class MysqlClient {
  ...
  SQLSTATUS execute(string cmd_str, string& res_str)
  {
    bool conn = true;
    if (m_ == NULL) {
      conn = connect();
...
//#define DEBUG
      ...
      if (server_response == CR_SERVER_LOST || server_response == CR_SERVER_GONE_ERROR || findStringIn(res_str, "Lost connection") ||
          findStringIn(res_str, "ERROR 1105") || findStringIn(res_str, "ERROR 2013") || findStringIn(res_str, "ERROR 8108")
          ) {
...
void log_map_id(u32 i, u8 byte, const string& cur_seed_str)
{
  ...
  if (queue_cur) {
    ...
    map_id_seed_output.open("./queue_coverage_id_core/" + to_string(queue_cur->depth) + "_" + to_string(map_file_id) + ".txt",
...
//   string query = " SELECT t0.c0 FROM t3*, t0* INNER JOIN ONLY t1 ON
//   (initcap('YY') SIMILAR TO upper((('')||(B'1110110010011100100000011000'))))
//   WHERE NOT (((NOT
//   (pg_is_other_temp_schema()))OR(((0.7888163054438270815893474718905054032802581787109375)=(t1.c0)))));";
//   string expect = "SELECT SUM(count) FROM (SELECT ALL (NOT (((NOT
//   (pg_is_other_temp_schema()))OR(((0.7888163054438270815893474718905054032802581787109375)=(t1.c0))))))::INT
//   as count FROM t3*, t0* JOIN ONLY t1 ON (initcap('YY') SIMILAR TO
//   upper((('')||(B'1110110010011100100000011000'))))) as res;"; int mul_run_id
//   =1; string rew_1 = "", rew_2 = "", rew_3 = "";
...
static u8 run_target(char** argv, u32 timeout, string cmd_string,
    int is_reset_server,
    string& res_str)
{
...
inline void print_fuzzer_exec_debug_info()
{
  ...
  if (is_debug_info) {
    ...
    cout << "\n\33[2K \n"
         ...
         << "\33[2K num_ora_select_succeed:  "
         << p_oracle->num_oracle_select_succeed << "\n"
         << "\33[2K num_ora_select_mutate:   "
         << p_oracle->num_oracle_select_mutate << endl;
  }
...
#define CASE_PREFIX "id_"
    ...
    if (!strncmp(rsl, CASE_PREFIX, 3) && sscanf(rsl + 3, "%06u", &orig_id) == 1 && orig_id == id) {
    ...
    const vector<int>& explain_diff_id = {})
{
  ...
  case FAULT_TMOUT:
    ...
    //        timeout actually uncovers a crash. Make sure we don't discard it
    //        if so. */
...
static void maybe_update_plot_file(double bitmap_cvg, double eps)
{
  ...
  fprintf(
      ...
      get_total_grammar_edge_cov_size_num()); /* ignore errors */
  fflush(plot_file);
}
...
#undef DEBUG
    ...
    if (p_oracle->is_oracle_select_stmt(cur_stmt_str)) {
      ...
      if (p_oracle->is_res_str_error(g_cockroach_output)) {
        ...
        SemanticErrorType err_type = p_oracle->detect_semantic_error_type(g_cockroach_output);
...
int main(int argc, char** argv)
{
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QDc:lO:P:F:XRGK:")) > 0)
