...
func (ft *FieldType) LogCurrentNode(depth int) *sql_ir.SqlRsgIR {
	prefix := TypeToStr(ft.tp, ft.charset)
	...
	if precision != UnspecifiedLength {
		...
		rNode := &sql_ir.SqlRsgIR{
			...
			DataType: sql_ir.DataNone,
			IValue:   int64(precision),
			Str:      strconv.FormatInt(int64(precision), 10),
			Depth:    depth,
		}
		rootNode = &sql_ir.SqlRsgIR{
			IRType:   sql_ir.TypeUnknown,
		...
		if scale != UnspecifiedLength {
			...
			rNode = &sql_ir.SqlRsgIR{
				...
				DataType: sql_ir.DataNone,
				IValue:   int64(scale),
				Str:      strconv.FormatInt(int64(scale), 10),
				Depth:    depth,
			}
			rootNode = &sql_ir.SqlRsgIR{
				IRType:   sql_ir.TypeUnknown,
	...
	rootNode.IRType = sql_ir.TypeFieldType
	return rootNode
}
