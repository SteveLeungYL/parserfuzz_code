...
class Oracle_LIKELY():
    ...
    def comp_query_res(cls, queries_l, all_res_str_l):
        ...
        for idx, valid_type in enumerate(valid_type_list):
            ...
            if valid_type == VALID_TYPE_LIKELY.NORM:
                curr_res = cls._check_result_NORM(all_res_str_l[idx][0], all_res_str_l[idx][1], all_res_str_l[idx][2])
                all_res_out.append(curr_res)
            elif valid_type == VALID_TYPE_LIKELY.UNIQUE:
                curr_res = cls._check_result_UNIQ(all_res_str_l[idx][0], all_res_str_l[idx][1], all_res_str_l[idx][2], valid_type)
                all_res_out.append(curr_res)
            else:
                raise ValueError("Encounter unknown VALID_TYPE_LIKELY in the check_query_exec_correctness_under_commitID func. ")
    ...
    def _check_result_NORM(cls, ori:str, likely:str, unlikely:str) -> RESULT:
        ...
        for cur_ori in ori_list:
            if re.match(r"""^[\|\s]*$""", cur_ori, re.MULTILINE | re.IGNORECASE):  # Only spaces or | (separator)
                continue
            ori_out_int += 1
        for cur_likely in likely_list:
            if re.match(r"""^[\|\s]*$""", cur_likely, re.MULTILINE | re.IGNORECASE):  # Only spaces or | (separator)
                continue
            likely_out_int += 1
        for cur_unlikely in unlikely_list:
            if re.match(r"""^[\|\s]*$""", cur_unlikely, re.MULTILINE | re.IGNORECASE):  # Only spaces or | (separator)
                continue
            unlikely_out_int += 1
    ...
    @classmethod
    def _check_result_UNIQ(cls, ori:str, likely:str, unlikely:str, valid_type)-> RESULT:
        if "Error" in ori or "Error" in likely or "Error" in unlikely:
            return RESULT.ERROR
