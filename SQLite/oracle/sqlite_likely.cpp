...
void SQL_LIKELY::rewrite_valid_stmt_from_ori(string& query, string& rew_1, string& rew_2)
{
...
int SQL_LIKELY::compare_results(const vector<string>& result_1, const vector<string>& result_2, const vector<string>& result_3){
  
  bool is_all_errors = true;
  int current_result_int_1, current_result_int_2, current_result_int_3;

  for (int i = 0; i < std::min({ result_1.size(), result_2.size(), result_3.size() }); i++){
    try {
      current_result_int_1 = stoi(result_1[i]);
      current_result_int_2 = stoi(result_2[i]);
      current_result_int_3 = stoi(result_3[i]);
    }
    ...
    is_all_errors = false;
    if (current_result_int_1 != current_result_int_2 || current_result_int_1 != current_result_int_3) return 0; // Found mismatched. 
  }
