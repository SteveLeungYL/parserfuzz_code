...
void SQL_LIKELY::rewrite_valid_stmt_from_ori(string& query, string& rew_1, string& rew_2, string& rew_3, unsigned multi_run_id)
{
  ...
  rew_1 = before_select_stmt + " SELECT COUNT ( * ) FROM " + from_stmt + " WHERE LIKELY ( " + where_stmt + " )";
  rew_2 = before_select_stmt + " SELECT COUNT ( * ) FROM " + from_stmt + " WHERE UNLIKELY ( " + where_stmt + " )";
  rew_3 = "";
}
...
void SQL_LIKELY::get_v_valid_type(const string& cmd_str, vector<VALID_STMT_TYPE_LIKELY>& v_valid_type){
...
void SQL_LIKELY::compare_results(ALL_COMP_RES& res_out) {
  ...
  if (is_all_errors && res_out.final_res != ORA_COMP_RES::Fail) res_out.final_res = ORA_COMP_RES::ALL_Error;

  return;

}

bool SQL_LIKELY::compare_norm(COMP_RES& res){ /* Handle normal valid stmt: SELECT * FROM ...; Return is_err */
    ...
    if (res_str_0.find("Error") != string::npos || res_str_1.find("Error") != string::npos || res_str_2.find("Error") != string::npos){
