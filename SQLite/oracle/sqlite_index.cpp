...
int SQL_INDEX::count_valid_stmts(const string& input){
  ...
  for (string &query : queries_vector) 
    if (this->is_oracle_valid_stmt(query) || this->is_oracle_valid_stmt_2(query))
      norec_select_count++;
...
void SQL_INDEX::rewrite_valid_stmt_from_ori(string& query, string& rew_1, string& rew_2, string& rew_3, unsigned multi_run_id)
{
    rew_1 = "";
    rew_2 = "";
    rew_3 = "";
...
void SQL_INDEX::rewrite_valid_stmt_from_ori_2(string& query, unsigned multi_run_id)
{   
...
void SQL_INDEX::get_v_valid_type(const string& cmd_str, vector<VALID_STMT_TYPE_INDEX>& v_valid_type){
