...
select * from int4_tbl i4, tenk1 awhere exists(select * from tenk1 b             where a.twothousand = b.twothousand and a.fivethous <> b.fivethous)      and i4.f1 = a.tenthous;
...
select * from tt4x t1where not exists (  select 1 from tt4x t2    left join tt4x t3 on t2.c3 = t3.c1    left join ( select t5.c1 as c1                from tt4x t4 left join tt4x t5 on t4.c2 = t5.c1              ) a1 on t3.c2 = a1.c1  where t1.c1 = t2.c2);
...
analyze q1;
analyze q2;
select * from  tenk1 join int4_tbl on f1 = twothousand,  q1, q2where q1 = thousand or q2 = thousand;
select * from  tenk1 join int4_tbl on f1 = twothousand,  q1, q2where thousand = (q1 + q2);
select * from  tenk1, int8_tbl a, int8_tbl bwhere thousand = a.q1 and tenthous = b.q1 and a.q2 = 1 and b.q2 = 2;
...
create function f_immutable_int4(i integer) returns integer as begin return i;
  ...
  language plpgsql immutable;
select unique1 from tenk1, (select * from f_immutable_int4(1) x) xwhere x = unique1;
select unique1, x.*from tenk1, (select *, random() from f_immutable_int4(1) x) xwhere x = unique1;
...
select nt3.idfrom nt3 as nt3  left join    (select nt2.*, (nt2.b1 or i4 = 42) AS b3     from nt2 as nt2       left join         f_immutable_int4(0) i4         on i4 = nt2.nt1_id    ) as ss2    on ss2.id = nt3.nt2_idwhere nt3.id = 1 and ss2.b3;
...
select * from tenk1 a join tenk1 b on  (a.unique1 = 1 and b.unique1 = 2) or (a.unique2 = 3 and b.hundred = 4);
select * from tenk1 a join tenk1 b on  (a.unique1 = 1 and b.unique1 = 2) or (a.unique2 = 3 and b.ten = 4);
select * from tenk1 a join tenk1 b on  (a.unique1 = 1 and b.unique1 = 2) or  ((a.unique2 = 3 or a.unique2 = 7) and b.hundred = 4);
select * from tenk1 t1 left join  (tenk1 t2 join tenk1 t3 on t2.thousand = t3.unique2)  on t1.hundred = t2.hundred and t1.ten = t3.tenwhere t1.unique1 = 1;
select * from tenk1 t1 left join  (tenk1 t2 join tenk1 t3 on t2.thousand = t3.unique2)  on t1.hundred = t2.hundred and t1.ten + t2.ten = t3.tenwhere t1.unique1 = 1;
...
INSERT INTO d VALUES (1,3), (2,2), (3,1);
 ...
 SELECT b.* FROM b LEFT JOIN c ON b.c_id = c.id;
  SELECT a.* FROM a LEFT JOIN (b left join c on b.c_id = c.id)  ON (a.b_id = b.id);
select id from a where id in (	select b.id from b left join c on b.id = c.id);
select d.* from d left join (select * from b group by b.id, b.c_id) s  on d.a = s.id and d.b = s.c_id;
select d.* from d left join (select distinct * from b) s  on d.a = s.id and d.b = s.c_id;
select d.* from d left join (select * from b group by b.id, b.c_id) s  on d.a = s.id;
select d.* from d left join (select distinct * from b) s  on d.a = s.id;
select d.* from d left join (select id from a union select id from b) s  on d.a = s.id;
select i8.* from int8_tbl i8 left join (select f1 from int4_tbl group by f1) i4  on i8.q1 = i4.f1;
select 1 from (select a.id FROM a left join b on a.b_id = b.id) q,			  lateral generate_series(1, q.id) gs(i) where q.id = gs.i;
rollback;
...
select t1.* from  uniquetbl as t1  left join (select *, '***'::text as d1 from uniquetbl) t2  on t1.f1 = t2.f1  left join uniquetbl t3  on t2.d1 = t3.f1;
...
select unique2, x.*from int4_tbl x, lateral (select unique2 from tenk1 where f1 = unique1) ss;
  select unique2, x.*  from int4_tbl x, lateral (select unique2 from tenk1 where f1 = unique1) ss;
  select unique2, x.*  from int4_tbl x cross join lateral (select unique2 from tenk1 where f1 = unique1) ss;
...
select * from  int8_tbl a left join lateral  (select b.q1 as bq1, c.q1 as cq1, least(a.q1,b.q1,c.q1) from   int8_tbl b cross join int8_tbl c) ss  on a.q2 = ss.bq1;
select * from  int8_tbl c left join (    int8_tbl a left join (select q1, coalesce(q2,42) as x from int8_tbl b) ss1      on a.q2 = ss1.q1    cross join    lateral (select q1, coalesce(ss1.x,q2) as y from int8_tbl d) ss2  ) on c.q2 = ss2.q1,  lateral (select ss2.y offset 0) ss3;
select c.*,a.*,ss1.q1,ss2.q1,ss3.* from  int8_tbl c left join (    int8_tbl a left join      (select q1, coalesce(q2,f1) as x from int8_tbl b, int4_tbl b2       where q1 < f1) ss1      on a.q2 = ss1.q1    cross join    lateral (select q1, coalesce(ss1.x,q2) as y from int8_tbl d) ss2  ) on c.q2 = ss2.q1,  lateral (select * from int4_tbl i where ss2.y > f1) ss3;
select * from  (select 1 as x offset 0) x cross join (select 2 as y offset 0) y  left join lateral (    select * from (select 3 as z offset 0) z where z.z = x.x  ) zz on zz.z = y.y;
select * from int8_tbl i8 left join lateral  (select *, i8.q2 from int4_tbl where false) ss on true;
select * from int8_tbl i8 left join lateral  (select *, i8.q2 from int4_tbl i1, int4_tbl i2 where false) ss on true;
...
analyze fkest;
analyze fkest1;
select *from fkest f  left join fkest1 f1 on f.a = f1.a and f.b = f1.b  left join fkest1 f2 on f.a = f2.a and f.b = f2.b  left join fkest1 f3 on f.a = f3.a and f.b = f3.bwhere f.c = 1;
rollback;
...
select * from j1inner join (select distinct id from j3) j3 on j1.id = j3.id;
select * from j1inner join (select id from j3 group by id) j3 on j1.id = j3.id;
drop table j1;
drop table j2;
drop table j3;
...
analyze j2;
analyze j3;
select * from j1inner join j2 on j1.id1 = j2.id1;
select * from j1inner join j2 on j1.id1 = j2.id1 and j1.id2 = j2.id2;
select * from j1inner join j2 on j1.id1 = j2.id1 where j1.id2 = 1;
select * from j1left join j2 on j1.id1 = j2.id1 where j1.id2 = 1;
...
drop table j2;
drop table j3;
select t1.unique1, t2.hundredfrom onek t1, tenk1 t2where exists (select 1 from tenk1 t3              where t3.thousand = t1.unique1 and t3.tenthous = t2.hundred)      and t1.unique1 < 1;
create table j3 as select unique1, tenthous from onek;
vacuum analyze j3;
create unique index on j3(unique1, tenthous);
select t1.unique1, t2.hundredfrom onek t1, tenk1 t2where exists (select 1 from j3              where j3.unique1 = t1.unique1 and j3.tenthous = t2.hundred)      and t1.unique1 < 1;
drop table j3;
