...
select 1 = all (select (select 1));
select * from int4_tbl o where exists  (select 1 from int4_tbl i where i.f1=o.f1 limit null);
select * from int4_tbl o where not exists  (select 1 from int4_tbl i where i.f1=o.f1 limit 1);
select * from int4_tbl o where exists  (select 1 from int4_tbl i where i.f1=o.f1 limit 0);
...
rollback;
  ...
  select a.thousand from tenk1 a, tenk1 bwhere a.thousand = b.thousand  and exists ( select 1 from tenk1 c where b.hundred = c.hundred                   and not exists ( select 1 from tenk1 d                                    where a.thousand = d.thousand ) );
  select x, x from    (select (select now()) as x from (values(1),(2)) v(y)) ss;
  select x, x from    (select (select random()) as x from (values(1),(2)) v(y)) ss;
  select x, x from    (select (select now() where y=y) as x from (values(1),(2)) v(y)) ss;
  select x, x from    (select (select random() where y=y) as x from (values(1),(2)) v(y)) ss;
...
begin    for ln in                select * from (select pk,c2 from sq_limit order by c1,pk) as x limit 3    loop        ln := regexp_replace(ln, 'Memory: \S*',  'Memory: xxx');
        return next ln;
    end loop;
end;
;
...
commit;
with x as (select * from (select f1 from subselect_tbl) ss)select * from x where f1 = 1;
with x as materialized (select * from (select f1 from subselect_tbl) ss)select * from x where f1 = 1;
with x as (select * from (select f1, now() from subselect_tbl) ss)select * from x where f1 = 1;
with x as (select * from (select f1, random() from subselect_tbl) ss)select * from x where f1 = 1;
with x as (select * from (select f1 from subselect_tbl for update) ss)select * from x where f1 = 1;
with x as (select * from (select f1, now() as n from subselect_tbl) ss)select * from x, x x2 where x.n = x2.n;
with x as not materialized (select * from (select f1, now() as n from subselect_tbl) ss)select * from x, x x2 where x.n = x2.n;
...
with recursive x(a) as  ((values ('a'), ('b'))   union all   (with z as not materialized (select * from x)    select z.a || z.a as a from z    where length(z.a || z.a) < 5))select * from x;
with x as (select * from int4_tbl)select * from (with y as (select * from x) select * from y) ss;
with x as materialized (select * from int4_tbl)select * from (with y as (select * from x) select * from y) ss;
with x as (select 1 as y)select * from (with x as (select 2 as y) select * from x) ss;
with x as (select * from subselect_tbl)select * from x for update;
