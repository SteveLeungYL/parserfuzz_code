...
set enable_nestloop = off;
  select t1.* from matest0 t1, matest0 t2where t1.b = t2.b and t2.c = t2.dorder by t1.b limit 10;
  select t1.* from matest0 t1, matest0 t2where t1.b = t2.b and t2.c = t2.dorder by t1.b limit 10;
...
SELECT thousand, tenthous FROM tenk1UNION ALLSELECT thousand, thousand FROM tenk1ORDER BY thousand, tenthous;
SELECT thousand, tenthous, thousand+tenthous AS x FROM tenk1UNION ALLSELECT 42, 42, hundred FROM tenk1ORDER BY thousand, tenthous;
SELECT thousand, tenthous FROM tenk1UNION ALLSELECT thousand, random()::integer FROM tenk1ORDER BY thousand, tenthous;
SELECT min(x) FROM  (SELECT unique1 AS x FROM tenk1 a   UNION ALL   SELECT unique2 AS x FROM tenk1 b) s;
SELECT min(y) FROM  (SELECT unique1 AS x, unique1 AS y FROM tenk1 a   UNION ALL   SELECT unique2 AS x, unique2 AS y FROM tenk1 b) s;
SELECT x, y FROM  (SELECT thousand AS x, tenthous AS y FROM tenk1 a   UNION ALL   SELECT unique2 AS x, unique2 AS y FROM tenk1 b) sORDER BY x, y;
...
set session authorization regress_no_child_access;
  select * from permtest_parent p1 inner join permtest_parent p2  on p1.a = p2.a and p1.c ~ 'a1 ';
  select * from permtest_parent p1 inner join permtest_parent p2  on p1.a = p2.a and left(p1.c, 3) ~ 'a1 ';
reset session authorization;
revoke all on permtest_parent from regress_no_child_access;
grant select(a,c) on permtest_parent to regress_no_child_access;
set session authorization regress_no_child_access;
  select p2.a, p1.c from permtest_parent p1 inner join permtest_parent p2  on p1.a = p2.a and p1.c ~ 'a1 ';
  select p2.a, p1.c from permtest_parent p1 inner join permtest_parent p2  on p1.a = p2.a and left(p1.c, 3) ~ 'a1 ';
reset session authorization;
revoke all on permtest_parent from regress_no_child_access;
