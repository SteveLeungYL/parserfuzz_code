...
bool SQL_TLP::is_oracle_select_stmt(IR* cur_stmt) {
  ...
  if (
    ir_wrapper.is_exist_ir_node_in_stmt_with_type(cur_stmt, TypeSelect, false) &&
...
vector<IR*> SQL_TLP::post_fix_transform_select_stmt(IR* cur_stmt, unsigned multi_run_id){
  ...
  vector<IR*> where_clause_in_first_vec = ir_wrapper.get_ir_node_in_stmt_with_type(first_stmt, TypeWhere, false);
...
IR* SQL_TLP::transform_non_aggr(IR* cur_stmt, bool is_UNION_ALL, VALID_STMT_TYPE_TLP tlp_type) {
  ...
  vector<IR*> src_simple_select_vec_ = ir_wrapper.get_ir_node_in_stmt_with_type(cur_stmt, TypeSelect, false);
  ...
  vector<IR*> v_where_first_stmt = ir_wrapper.get_ir_node_in_stmt_with_type(first_part_TLP, TypeWhere, false);
  ...
  IR* unary_expr_ = new IR(TypeExprs, OP3("NOT", "", ""), operand_0); // NOT (kWhereClause)
  IR* operand_1 = new IR(TypeExprs, OP3("(", ")", ""), unary_expr_); // For the second brackets. (NOT (kWhereClause))
...
IR* SQL_TLP::transform_aggr(IR* cur_stmt, bool is_UNION_ALL, VALID_STMT_TYPE_TLP tlp_type) {
  ...
  if (
    ...
    if (v_targetel.size() > 0) {
      IR* targetel = v_targetel.front();
      vector<IR*> v_func_name_ir = ir_wrapper.get_ir_node_in_stmt_with_type(targetel, TypeIdentifier, false);
      if (v_func_name_ir.size() == 0 ){
        cerr << "Error: Cannot find FuncName identifiers inside kTargetEl. TLP oracle logic error. \n";
        cur_stmt->deep_drop();
        return NULL;
      }
      ...
      if (targetel->get_right() != NULL) {
        ...
        IR* target_list_ir = new IR(TypeUnknown, OP3("", ",", ""), res_0, res_1);
        cur_stmt->swap_node(targetel, target_list_ir);
        targetel->deep_drop();
  ...
  IR* ori_outer_expr = ir_wrapper.get_ir_node_in_stmt_with_type(cur_stmt_outer, TypeParenSelect, true).front()->get_left();
