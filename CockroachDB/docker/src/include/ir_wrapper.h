...
public:
  ...
  */
  ...
  IR *iter_cur_node(IR *cur_node, T ir_name, U cap_name);
  ...
  template <typename T, typename U>
  IR *find_closest_nearby_IR_with_type(IR *cur_node, T ir_name, U cap_name);
  template <typename T>
  IR *find_closest_nearby_IR_with_type(IR *cur_node, T ir_name) {
    vector<T> dummy_vec;
    return this->find_closest_nearby_IR_with_type(cur_node, ir_name, dummy_vec);
  }

  template <typename TYPE>
  bool is_find_closest_nearby_IR_with_type(IR *cur_node, TYPE ir_type) {
...
template <typename T, typename U>
IR *IRWrapper::iter_cur_node(IR *cur_node, T ir_type, U cap_type) {
  // Recursive function.
  // Depth first search.
  if (cur_node == NULL) {
    return NULL;
...
template <typename T, typename U>
IR *IRWrapper::find_closest_nearby_IR_with_type(IR *cur_node, T ir_type,
                                                U cap_type) {
  ...
  if (cur_node == NULL) {
    cerr << "ERROR: Inside the function: find_closest_nearby_IR_with_type, "
            "getting "
            "NULL cur_node. \n\n\n";
    return NULL;
  }
