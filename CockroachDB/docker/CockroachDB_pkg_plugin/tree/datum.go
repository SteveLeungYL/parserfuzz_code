...
func (node *DBool) LogCurrentNode(depth int) *SQLRightIR {
...
// SQLRight Code Injection.
func (node *DDecimal) LogCurrentNode(depth int) *SQLRightIR {

	quote := node.Decimal.Form != apd.Finite
	needParens := !quote && node.Negative
	prefix := ""
	infix := ""
	if needParens {
		prefix += "("
...
// SQLRight Code Injection.
func (node *DCollatedString) LogCurrentNode(depth int) *SQLRightIR {
...
// SQLRight Code Injection.
func (node *DTuple) LogCurrentNode(depth int) *SQLRightIR {

	typ := node.ResolvedType()
	tupleContents := typ.TupleContents()
	showLabels := len(typ.TupleLabels()) > 0
	...
	var datumList []*SQLRightIR
	for i, v := range node.D {
		datumNode := v.LogCurrentNode(depth + 1)
		if parsable && (v == DNull) && len(tupleContents) > i {
			if tupleContents[i].Family() != types.UnknownFamily {
				nullType := tupleContents[i]
				infix := "::"
				...
				datumNode = tmpDatumNode
			}
		}
		datumList = append(datumList, datumNode)
	}

	for idx, n := range datumList {
	...
	if showLabels {
		...
		for i := range typ.TupleLabels() {
			...
			nameList = append(nameList, nameNode)
		}
...
// SQLRight Code Injection.
func (node dNull) LogCurrentNode(depth int) *SQLRightIR {
...
// SQLRight Code Injection.
func (node DArray) LogCurrentNode(depth int) *SQLRightIR {
