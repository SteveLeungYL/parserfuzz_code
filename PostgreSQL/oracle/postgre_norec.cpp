  ...
  if (
    ...
    for (IR* count_func_ir : count_func_vec){
      ...
      if (
        ...
        ir_wrapper.get_parent_type(count_func_ir, 4) == kOptSelectTarget &&
        ir_wrapper.get_parent_type(count_func_ir, 5) == kSimpleSelect 
      ) {
...
vector<IR*> SQL_NOREC::post_fix_transform_select_stmt(IR* cur_stmt, unsigned multi_run_id){
  ...
  if (transformed_temp_vec.size() == 0) {
    ...
    // cerr << "Post_fix_temp is: " << this->post_fix_temp << ". \n\n\n";
  ...
  vector<IR*> dest_where_vec = ir_wrapper.get_ir_node_in_stmt_with_type(trans_stmt_ir, kCastExpr, true); // cerr << "Getting from the trans_stmt_ir, the first kFuncExpr subquery is: " << dest_where_vec[0] -> to_string() << "\n";
  IR* dest_where_expr = dest_where_vec[0]->left_->left_->left_->left_;  // kCastExpr->Expr->Operand->kExprList->Expr 
