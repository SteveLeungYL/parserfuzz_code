...
insert into trigtest default values;  alter table trigtest enable always trigger trigtest_a_stmt_tg;
insert into trigtest default values;  reset session_replication_role;
...
UPDATE city_view SET country_name = 'Japon' WHERE city_name = 'Tokyo'; UPDATE city_view SET country_name = 'Japan' WHERE city_name = 'Takyo'; UPDATE city_view SET country_name = 'Japan' WHERE city_name = 'Tokyo' RETURNING *; UPDATE city_view SET population = 13010279 WHERE city_name = 'Tokyo' RETURNING *;
...
select * from parent;
 select * from child;
 select * from child;
update parent set val1 = 'b' where aid = 1; select * from parent;
 select * from child;
 select * from child;
delete from parent where aid = 1; select * from parent;
 select * from child;
 select * from child;
...
insert into trigger_ddl_table values (1, 42);  create or replace function trigger_ddl_func() returns trigger as begin  create index on trigger_ddl_table (col2);
  return new;
end language plpgsql;
insert into trigger_ddl_table values (1, 42);  drop table trigger_ddl_table;
...
drop trigger trg1 on trigpart1;	drop trigger trg1 on trigpart2;	drop trigger trg1 on trigpart3;	drop table trigpart2;			select tgrelid::regclass, tgname, tgfoid::regproc from pg_trigger  where tgrelid::regclass::text like 'trigpart%' order by tgrelid::regclass::text;
drop trigger trg1 on trigpart;		select tgrelid::regclass, tgname, tgfoid::regproc from pg_trigger  where tgrelid::regclass::text like 'trigpart%' order by tgrelid::regclass::text;
create trigger trg1 after insert on trigpart for each row execute procedure trigger_nothing();
\d trigpart3alter table trigpart detach partition trigpart3;
drop trigger trg1 on trigpart3; alter table trigpart detach partition trigpart4;
drop trigger trg1 on trigpart41; drop table trigpart4;
alter table trigpart attach partition trigpart3 for values from (2000) to (3000);
...
\d trigpart3alter table trigpart attach partition trigpart3 FOR VALUES FROM (2000) to (3000); drop table trigpart3;
drop table trigpart;
...
;
insert into parted values (1, 1, 'uno uno v1');    create trigger t before insert or update or delete on parted  for each row execute function parted_trigfunc();
insert into parted values (1, 1, 'uno uno v2');    update parted set c = c || 'v3';                   create or replace function parted_trigfunc() returns trigger language plpgsql as begin  new.b = new.b + 1;
  return new;
end;
;
insert into parted values (1, 1, 'uno uno v4');    update parted set c = c || 'v5';                   create or replace function parted_trigfunc() returns trigger language plpgsql as begin  new.c = new.c || ' did '|| TG_OP;
  return new;
end;
;
insert into parted values (1, 1, 'uno uno');       update parted set c = c || ' v6';                   select tableoid::regclass, * from parted;
...
update parted_trigger set a = a + 2; drop table parted_trigger;
create table parted_referenced (a int);
create table unparted_trigger (a int, b text);	create table parted_trigger (a int, b text) partition by range (a);
create table parted_trigger_1 partition of parted_trigger for values from (0) to (1000);
create table parted_trigger_2 (drp int, a int, b text);
alter table parted_trigger_2 drop column drp;
alter table parted_trigger attach partition parted_trigger_2 for values from (1000) to (2000);
...
update parted_trigger set a = a + 2;	update parted_trigger set b = b || 'b';	drop table parted_trigger;
