...
EXPLAIN (COSTS OFF) SELECT * FROM document WHERE f_leak(dtitle);
EXPLAIN (COSTS OFF) SELECT * FROM document NATURAL JOIN category WHERE f_leak(dtitle);
INSERT INTO document VALUES (100, 44, 1, 'regress_rls_dave', 'testing sorting of policies'); INSERT INTO document VALUES (100, 55, 1, 'regress_rls_dave', 'testing sorting of policies'); ALTER POLICY p1 ON document USING (true);    DROP POLICY p1 ON document;                  SET SESSION AUTHORIZATION regress_rls_alice;
...
DELETE FROM category WHERE cid = 33;    SET SESSION AUTHORIZATION regress_rls_carol;
SELECT * FROM document d FULL OUTER JOIN category c on d.cid = c.cid ORDER BY d.did, c.cid;
INSERT INTO document VALUES (11, 33, 1, current_user, 'hoge');
SET SESSION AUTHORIZATION regress_rls_bob;
INSERT INTO document VALUES (8, 44, 1, 'regress_rls_bob', 'my third manga'); SELECT * FROM document WHERE did = 8; INSERT INTO document VALUES (8, 44, 1, 'regress_rls_carol', 'my third manga'); UPDATE document SET did = 8, dauthor = 'regress_rls_carol' WHERE did = 5; RESET SESSION AUTHORIZATION;
SET row_security TO ON;
...
ALTER TABLE t1 DROP COLUMN junk1;    GRANT ALL ON t1 TO public;
...
301	1	xxx	X302	2	yyy	Y303	3	zzz	Z\.CREATE POLICY p1 ON t1 FOR ALL TO PUBLIC USING (a % 2 = 0); CREATE POLICY p2 ON t2 FOR ALL TO PUBLIC USING (a % 2 = 1); ALTER TABLE t1 ENABLE ROW LEVEL SECURITY;
...
SET SESSION AUTHORIZATION regress_rls_dave;
INSERT INTO part_document_satire VALUES (101, 55, 1, 'regress_rls_dave', 'testing RLS with partitions'); SELECT * FROM part_document_satire WHERE f_leak(dtitle) ORDER BY did;
SELECT * FROM part_document WHERE f_leak(dtitle) ORDER BY did;
EXPLAIN (COSTS OFF) SELECT * FROM part_document WHERE f_leak(dtitle);
SET SESSION AUTHORIZATION regress_rls_carol;
SELECT * FROM part_document WHERE f_leak(dtitle) ORDER BY did;
EXPLAIN (COSTS OFF) SELECT * FROM part_document WHERE f_leak(dtitle);
ALTER POLICY pp1 ON part_document USING (true);    DROP POLICY pp1 ON part_document;                  SET SESSION AUTHORIZATION regress_rls_alice;
...
SET SESSION AUTHORIZATION regress_rls_carol;
INSERT INTO part_document VALUES (100, 11, 5, 'regress_rls_carol', 'testing pp3'); SET SESSION AUTHORIZATION regress_rls_alice;
SET row_security TO ON;
CREATE TABLE dependee (x integer, y integer);
CREATE TABLE dependent (x integer, y integer);
CREATE POLICY d1 ON dependent FOR ALL    TO PUBLIC    USING (x = (SELECT d.x FROM dependee d WHERE d.y = y));
DROP TABLE dependee; DROP TABLE dependee CASCADE;
EXPLAIN (COSTS OFF) SELECT * FROM dependent; SET SESSION AUTHORIZATION regress_rls_alice;
CREATE TABLE rec1 (x integer, y integer);
CREATE POLICY r1 ON rec1 USING (x = (SELECT r.x FROM rec1 r WHERE y = r.y));
ALTER TABLE rec1 ENABLE ROW LEVEL SECURITY;
SET SESSION AUTHORIZATION regress_rls_bob;
SELECT * FROM rec1; SET SESSION AUTHORIZATION regress_rls_alice;
CREATE TABLE rec2 (a integer, b integer);
ALTER POLICY r1 ON rec1 USING (x = (SELECT a FROM rec2 WHERE b = y));
CREATE POLICY r2 ON rec2 USING (a = (SELECT x FROM rec1 WHERE y = b));
ALTER TABLE rec2 ENABLE ROW LEVEL SECURITY;
SET SESSION AUTHORIZATION regress_rls_bob;
SELECT * FROM rec1;    SET SESSION AUTHORIZATION regress_rls_bob;
...
SELECT * FROM s1 WHERE f_leak(b); INSERT INTO s1 VALUES (1, 'foo'); SET SESSION AUTHORIZATION regress_rls_alice;
DROP POLICY p3 on s1;
ALTER POLICY p2 ON s2 USING (x % 2 = 0);
SET SESSION AUTHORIZATION regress_rls_bob;
SELECT * FROM s1 WHERE f_leak(b);	EXPLAIN (COSTS OFF) SELECT * FROM only s1 WHERE f_leak(b);
SET SESSION AUTHORIZATION regress_rls_alice;
ALTER POLICY p1 ON s1 USING (a in (select x from v2)); SET SESSION AUTHORIZATION regress_rls_bob;
SELECT * FROM s1 WHERE f_leak(b);	EXPLAIN (COSTS OFF) SELECT * FROM s1 WHERE f_leak(b);
SELECT (SELECT x FROM s1 LIMIT 1) xx, * FROM s2 WHERE y like '%28%';
EXPLAIN (COSTS OFF) SELECT (SELECT x FROM s1 LIMIT 1) xx, * FROM s2 WHERE y like '%28%';
SET SESSION AUTHORIZATION regress_rls_alice;
ALTER POLICY p2 ON s2 USING (x in (select a from s1 where b like '%d2%'));
SET SESSION AUTHORIZATION regress_rls_bob;
SELECT * FROM s1 WHERE f_leak(b);	PREPARE p1(int) AS SELECT * FROM t1 WHERE a <= $1;
EXECUTE p1(2);
...
INSERT INTO bv1 VALUES (-1, 'xxx'); INSERT INTO bv1 VALUES (11, 'xxx'); INSERT INTO bv1 VALUES (12, 'xxx'); EXPLAIN (COSTS OFF) UPDATE bv1 SET b = 'yyy' WHERE a = 4 AND f_leak(b);
...
INSERT INTO document VALUES (33, 22, 1, 'regress_rls_bob', 'okay science fiction'); INSERT INTO document VALUES (33, (SELECT cid from category WHERE cname = 'novel'), 1, 'regress_rls_bob', 'Some novel, replaces sci-fi')     ON CONFLICT (did) DO UPDATE SET dtitle = EXCLUDED.dtitle;
...
EXPLAIN (COSTS OFF) SELECT * FROM rls_view;
SET SESSION AUTHORIZATION regress_rls_carol;
SELECT * FROM rls_view; EXPLAIN (COSTS OFF) SELECT * FROM rls_view; SET SESSION AUTHORIZATION regress_rls_bob;
...
CREATE POLICY p1 ON y1 FOR SELECT USING (a % 2 = 1);  CREATE POLICY p1 ON y2 FOR ALL USING (a % 2 = 0);  ALTER TABLE y1 ENABLE ROW LEVEL SECURITY;
...
WITH cte1 AS (UPDATE t1 SET a = a + 1 RETURNING *) SELECT * FROM cte1; WITH cte1 AS (UPDATE t1 SET a = a RETURNING *) SELECT * FROM cte1; WITH cte1 AS (INSERT INTO t1 VALUES (21, 'Fail') RETURNING *) SELECT * FROM cte1; WITH cte1 AS (INSERT INTO t1 VALUES (20, 'Success') RETURNING *) SELECT * FROM cte1; RESET SESSION AUTHORIZATION;
ALTER POLICY p1 ON t1 RENAME TO p1; SELECT polname, relname    FROM pg_policy pol    JOIN pg_class pc ON (pc.oid = pol.polrelid)    WHERE relname = 't1';
ALTER POLICY p1 ON t1 RENAME TO p2; SELECT polname, relname    FROM pg_policy pol    JOIN pg_class pc ON (pc.oid = pol.polrelid)    WHERE relname = 't1';
SET SESSION AUTHORIZATION regress_rls_bob;
...
SET SESSION AUTHORIZATION regress_rls_bob;
SET row_security TO OFF;
COPY (SELECT * FROM copy_t ORDER BY a ASC) TO STDOUT WITH DELIMITER ','; SET row_security TO ON;
COPY (SELECT * FROM copy_t ORDER BY a ASC) TO STDOUT WITH DELIMITER ','; SET SESSION AUTHORIZATION regress_rls_exempt_user;
SET row_security TO OFF;
COPY (SELECT * FROM copy_t ORDER BY a ASC) TO STDOUT WITH DELIMITER ','; SET row_security TO ON;
COPY (SELECT * FROM copy_t ORDER BY a ASC) TO STDOUT WITH DELIMITER ','; SET SESSION AUTHORIZATION regress_rls_carol;
SET row_security TO OFF;
COPY (SELECT * FROM copy_t ORDER BY a ASC) TO STDOUT WITH DELIMITER ','; SET row_security TO ON;
COPY (SELECT * FROM copy_t ORDER BY a ASC) TO STDOUT WITH DELIMITER ','; RESET SESSION AUTHORIZATION;
SET row_security TO ON;
...
SET SESSION AUTHORIZATION regress_rls_bob;
SET row_security TO OFF;
COPY copy_rel_to TO STDOUT WITH DELIMITER ','; SET row_security TO ON;
COPY copy_rel_to TO STDOUT WITH DELIMITER ','; SET SESSION AUTHORIZATION regress_rls_exempt_user;
SET row_security TO OFF;
COPY copy_rel_to TO STDOUT WITH DELIMITER ','; SET row_security TO ON;
COPY copy_rel_to TO STDOUT WITH DELIMITER ','; SET SESSION AUTHORIZATION regress_rls_carol;
SET row_security TO OFF;
COPY copy_rel_to TO STDOUT WITH DELIMITER ','; SET row_security TO ON;
COPY copy_rel_to TO STDOUT WITH DELIMITER ','; RESET SESSION AUTHORIZATION;
SET row_security TO OFF;
COPY copy_t FROM STDIN; 1	abc2	bcd3	cde4	def\.SET row_security TO ON;
COPY copy_t FROM STDIN; 1	abc2	bcd3	cde4	def\.SET SESSION AUTHORIZATION regress_rls_bob;
SET row_security TO OFF;
COPY copy_t FROM STDIN; SET row_security TO ON;
COPY copy_t FROM STDIN; SET SESSION AUTHORIZATION regress_rls_exempt_user;
SET row_security TO ON;
COPY copy_t FROM STDIN; 1	abc2	bcd3	cde4	def\.SET SESSION AUTHORIZATION regress_rls_carol;
SET row_security TO OFF;
COPY copy_t FROM STDIN; SET row_security TO ON;
COPY copy_t FROM STDIN; RESET SESSION AUTHORIZATION;
DROP TABLE copy_t;
...
SAVEPOINT q;
DROP ROLE regress_rls_eve; ROLLBACK TO q;
...
DROP ROLE regress_rls_eve; ROLLBACK TO q;
SAVEPOINT q;
DROP ROLE regress_rls_frank; ROLLBACK TO q;
...
SAVEPOINT q;
CREATE RULE "_RETURN" AS ON SELECT TO t DO INSTEAD  SELECT * FROM generate_series(1,5) t0(c); ROLLBACK TO q;
ALTER TABLE t DISABLE ROW LEVEL SECURITY;
SAVEPOINT q;
CREATE RULE "_RETURN" AS ON SELECT TO t DO INSTEAD  SELECT * FROM generate_series(1,5) t0(c); ROLLBACK TO q;
DROP POLICY p ON t;
CREATE RULE "_RETURN" AS ON SELECT TO t DO INSTEAD  SELECT * FROM generate_series(1,5) t0(c); ROLLBACK;
BEGIN;
CREATE TABLE t (c) AS VALUES ('bar'::text);
CREATE POLICY p ON t USING (max(c)); ROLLBACK;
SET SESSION AUTHORIZATION regress_rls_alice;
...
SELECT * FROM r1;
SELECT * FROM r2;
INSERT INTO r2 VALUES (2); UPDATE r2 SET a = 2 RETURNING *; DELETE FROM r2 RETURNING *; INSERT INTO r1 SELECT a + 1 FROM r2 RETURNING *; UPDATE r1 SET a = r2.a + 2 FROM r2 WHERE r1.a = r2.a RETURNING *; DELETE FROM r1 USING r2 WHERE r1.a = r2.a + 2 RETURNING *; SELECT * FROM r1;
SELECT * FROM r2;
SET SESSION AUTHORIZATION regress_rls_alice;
DROP TABLE r1;
DROP TABLE r2;
...
DROP POLICY p1 ON dob_t1; CREATE POLICY p1 ON dob_t1 TO regress_rls_dob_role1,regress_rls_dob_role2 USING (true);
DROP OWNED BY regress_rls_dob_role1;
DROP POLICY p1 ON dob_t1; CREATE POLICY p1 ON dob_t2 TO regress_rls_dob_role1,regress_rls_dob_role2 USING (true);
DROP OWNED BY regress_rls_dob_role1;
DROP POLICY p1 ON dob_t2; DROP USER regress_rls_dob_role1;
...
GRANT SELECT ON rls_view TO regress_rls_alice;
SET SESSION AUTHORIZATION regress_rls_alice;
SELECT * FROM ref_tbl; SELECT * FROM rls_tbl; SELECT * FROM rls_view; RESET SESSION AUTHORIZATION;
DROP VIEW rls_view;
DROP TABLE rls_tbl;
DROP TABLE ref_tbl;
CREATE TABLE rls_tbl (a int);
