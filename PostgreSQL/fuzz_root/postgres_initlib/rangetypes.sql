...
select numrange(1.0, 2.0) + numrange(2.5, 3.0); select range_merge(numrange(1.0, 2.0), numrange(2.0, 3.0));
select range_merge(numrange(1.0, 2.0), numrange(1.5, 3.0));
select range_merge(numrange(1.0, 2.0), numrange(2.5, 3.0)); select numrange(1.0, 2.0) * numrange(2.0, 3.0);
select numrange(1.0, 2.0) * numrange(1.5, 3.0);
...
select '[4,50)'::restrictedrange @> 7; drop domain restrictedrange;
...
select rangetypes_sql(numrange(1,10), ARRAY[2,20]);  create function anycompatiblearray_anycompatiblerange_func(a anycompatiblearray, r anycompatiblerange)  returns anycompatible as 'select $1[1] + lower($2);
' language sql;
' language sql;
...
select arrayrange(ARRAY[2,1], ARRAY[1,2]);  select array[1,1] <@ arrayrange(array[1,2], array[2,1]);
...
set enable_sort = off;  select '(2,5)'::cashrange except select '(5,6)'::cashrange;
reset enable_sort;
