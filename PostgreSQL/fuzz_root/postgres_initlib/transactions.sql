...
BEGIN;
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE, READ ONLY, DEFERRABLE; SELECT * FROM writetest; SET TRANSACTION READ WRITE; COMMIT;
BEGIN;
SET TRANSACTION READ ONLY; SET TRANSACTION READ WRITE; SET TRANSACTION READ ONLY; SELECT * FROM writetest; SAVEPOINT x;
SET TRANSACTION READ ONLY; SELECT * FROM writetest; SET TRANSACTION READ ONLY; SET TRANSACTION READ WRITE; COMMIT;
BEGIN;
SET TRANSACTION READ WRITE; SAVEPOINT x;
SET TRANSACTION READ WRITE; SET TRANSACTION READ ONLY; SELECT * FROM writetest; SET TRANSACTION READ ONLY; SET TRANSACTION READ WRITE; COMMIT;
BEGIN;
SET TRANSACTION READ WRITE; SAVEPOINT x;
SET TRANSACTION READ ONLY; SELECT * FROM writetest; ROLLBACK TO SAVEPOINT x;
SHOW transaction_read_only;  SAVEPOINT y;
SET TRANSACTION READ ONLY; SELECT * FROM writetest; RELEASE SAVEPOINT y;
SHOW transaction_read_only;  COMMIT;
SET SESSION CHARACTERISTICS AS TRANSACTION READ ONLY;
DROP TABLE writetest; INSERT INTO writetest VALUES (1); SELECT * FROM writetest; DELETE FROM temptest; UPDATE temptest SET a = 0 FROM writetest WHERE temptest.a = 1 AND writetest.a = temptest.a; PREPARE test AS UPDATE writetest SET a = 0; EXECUTE test; SELECT * FROM writetest, temptest; CREATE TABLE test AS SELECT * FROM writetest; START TRANSACTION READ WRITE;
DROP TABLE writetest; COMMIT;
...
COMMIT;
SELECT * FROM trans_foo;		SELECT * FROM trans_bar;		SELECT * FROM trans_barbaz;	SELECT * FROM trans_baz;		BEGIN;
...
COMMIT;
SELECT * FROM trans_foo;		SELECT * FROM trans_barbaz;	BEGIN;
	SAVEPOINT one;
		SELECT trans_foo;
	ROLLBACK TO SAVEPOINT one;
	RELEASE SAVEPOINT one;
...
BEGIN;
  SAVEPOINT one;
  SELECT 0/0;
  SAVEPOINT two;      RELEASE SAVEPOINT one;        ROLLBACK TO SAVEPOINT one;
  SELECT 1;
COMMIT;
SELECT 1;			BEGIN;
...
FETCH ok;
SAVEPOINT s1;
FETCH ok;  FETCH ctt; ROLLBACK TO s1;
FETCH ok;  FETCH ctt; COMMIT;
...
INSERT INTO abc VALUES (3);  COMMIT AND CHAIN;  SHOW transaction_isolation;
SHOW transaction_read_only;
SHOW transaction_deferrable;
...
SHOW transaction_deferrable;
ROLLBACK;
COMMIT AND CHAIN;  ROLLBACK AND CHAIN;  SELECT * FROM abc ORDER BY 1;
...
START TRANSACTION ISOLATION LEVEL REPEATABLE READ\;
 ...
 COMMIT AND CHAIN;
 COMMIT AND CHAIN;
SHOW transaction_isolation;  START TRANSACTION ISOLATION LEVEL REPEATABLE READ\;
 INSERT INTO abc VALUES (19)\;
 ROLLBACK\;
 INSERT INTO abc VALUES (20)\;
 ROLLBACK AND CHAIN;
 ROLLBACK AND CHAIN;
SHOW transaction_isolation;  SELECT * FROM abc ORDER BY 1;
DROP TABLE abc;
begin;
select 1/0;
rollback to X;
