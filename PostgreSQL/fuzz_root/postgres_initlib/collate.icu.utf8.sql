SELECT getdatabaseencoding() <> 'UTF8' OR       (SELECT count(*) FROM pg_collation WHERE collprovider = 'i') = 0       AS skip_test \gset\if :skip_test\quit\endifSET client_encoding TO UTF8;
CREATE SCHEMA collate_tests;
SET search_path = collate_tests;
CREATE TABLE collate_test1 (    a int,    b text COLLATE "en-x-icu" NOT NULL);
\d collate_test1CREATE TABLE collate_test_fail (    a int,    b text COLLATE "ja_JP.eucjp-x-icu");
...
\d collate_test_likeCREATE TABLE collate_test2 (    a int,    b text COLLATE "sv-x-icu");
...
CREATE DOMAIN testdomain_i AS int COLLATE "sv-x-icu"; CREATE TABLE collate_test4 (    a int,    b testdomain_sv);
...
SELECT 'Türkiye' COLLATE "tr-x-icu" ~* 'KI' AS "true";  SELECT 'bıt' ~* 'BIT' COLLATE "en-x-icu" AS "false";
SELECT 'bıt' ~* 'BIT' COLLATE "tr-x-icu" AS "false";  SELECT relname FROM pg_class WHERE relname ~* '^abc';
...
SELECT a, b FROM collate_test1 UNION ALL SELECT a, b FROM collate_test3 ORDER BY 2; SELECT a, b FROM collate_test1 UNION ALL SELECT a, b FROM collate_test3; SELECT a, b FROM collate_test1 UNION SELECT a, b FROM collate_test3 ORDER BY 2; SELECT a, b COLLATE "C" FROM collate_test1 UNION SELECT a, b FROM collate_test3 ORDER BY 2; SELECT a, b FROM collate_test1 INTERSECT SELECT a, b FROM collate_test3 ORDER BY 2; SELECT a, b FROM collate_test1 EXCEPT SELECT a, b FROM collate_test3 ORDER BY 2; CREATE TABLE test_u AS SELECT a, b FROM collate_test1 UNION ALL SELECT a, b FROM collate_test3; select x < y from collate_test10; select x || y from collate_test10; select x, y from collate_test10 order by x || y; WITH RECURSIVE foo(x) AS   (SELECT x FROM (VALUES('a' COLLATE "en-x-icu"),('b')) t(x)   UNION ALL   SELECT (x || 'c') COLLATE "de-x-icu" FROM foo WHERE length(x) < 10)SELECT * FROM foo;
...
SELECT mylt2('a', 'B' collate "C") as fail; SELECT mylt2('a', 'B' collate "POSIX") as f;
...
CREATE INDEX collate_test1_idx3 ON collate_test1 ((b COLLATE "C")); CREATE INDEX collate_test1_idx4 ON collate_test1 (((b||'foo') COLLATE "POSIX"));
CREATE INDEX collate_test1_idx5 ON collate_test1 (a COLLATE "C"); CREATE INDEX collate_test1_idx6 ON collate_test1 ((a COLLATE "C")); SELECT relname, pg_get_indexdef(oid) FROM pg_class WHERE relname LIKE 'collate_test%_idx%' ORDER BY 1;
...
';
END$$;
CREATE COLLATION test0 FROM "C"; do $$BEGIN  EXECUTE 'CREATE COLLATION test1 (provider = icu, lc_collate = ' ||          quote_literal(current_setting('lc_collate')) ||          ', lc_ctype = ' ||          quote_literal(current_setting('lc_ctype')) || ');
';
';
END$$;
CREATE COLLATION test3 (provider = icu, lc_collate = 'en_US.utf8'); CREATE COLLATION test4 FROM nonsense;
CREATE COLLATION test5 FROM test0;
SELECT collname FROM pg_collation WHERE collname LIKE 'test%' ORDER BY 1;
ALTER COLLATION test1 RENAME TO test11;
ALTER COLLATION test0 RENAME TO test11; ALTER COLLATION test1 RENAME TO test22; ALTER COLLATION test11 OWNER TO regress_test_role;
...
DROP COLLATION test0; DROP COLLATION IF EXISTS test0;
...
DROP COLLATION test0 RESTRICT; DROP COLLATION test0 CASCADE;
...
CREATE UNIQUE INDEX ON test1cs (x);  INSERT INTO test1cs VALUES ('ABC');  CREATE UNIQUE INDEX ON test3cs (x);  SELECT string_to_array('ABC,DEF,GHI' COLLATE case_sensitive, ',', 'abc');
...
CREATE INDEX ON test3ci (x text_pattern_ops);  INSERT INTO test1ci VALUES ('abc'), ('def'), ('ghi');
...
CREATE UNIQUE INDEX ON test1ci (x);  INSERT INTO test1ci VALUES ('ABC');  CREATE UNIQUE INDEX ON test3ci (x);  SELECT string_to_array('ABC,DEF,GHI' COLLATE case_insensitive, ',', 'abc');
...
CREATE INDEX ON test3bpci (x bpchar_pattern_ops);  INSERT INTO test1bpci VALUES ('abc'), ('def'), ('ghi');
...
CREATE UNIQUE INDEX ON test1bpci (x);  INSERT INTO test1bpci VALUES ('ABC');  CREATE UNIQUE INDEX ON test3bpci (x);  SELECT string_to_array('ABC,DEF,GHI'::char(11) COLLATE case_insensitive, ',', 'abc');
...
SET enable_seqscan = off;
SELECT x FROM test4c WHERE x LIKE 'ABC' COLLATE case_sensitive;  SELECT x FROM test4c WHERE x LIKE 'ABC%' COLLATE case_sensitive;  SELECT x FROM test4c WHERE x LIKE 'ABC' COLLATE case_insensitive;  SELECT x FROM test4c WHERE x LIKE 'ABC%' COLLATE case_insensitive;  RESET enable_seqscan;
...
SELECT * FROM test4 WHERE b = 'Cote' COLLATE ignore_accents;  SELECT * FROM test4 WHERE b = 'Cote' COLLATE case_insensitive;
...
INSERT INTO test10fk VALUES ('abc');  INSERT INTO test10fk VALUES ('ABC');  INSERT INTO test10fk VALUES ('xyz');  SELECT * FROM test10pk;
...
INSERT INTO test11fk VALUES ('abc');  INSERT INTO test11fk VALUES ('ABC');  INSERT INTO test11fk VALUES ('xyz');  SELECT * FROM test11pk;
