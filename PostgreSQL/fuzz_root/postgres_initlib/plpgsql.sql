...
create function test_found()  returns boolean as '  declare  begin  insert into found_test_tbl values (1);
  ...
  delete from found_test_tbl where a = 9999;   if not FOUND then    insert into found_test_tbl values (4);
  end if;
...
select f1(point(3,4));  drop function f1(x anyelement);
...
select f1(stavalues1) from pg_statistic;  drop function f1(x anyarray);
...
select f1(int4range(42, 49), 11, 4.5) as fail;  drop function f1(x anycompatiblerange, y anycompatible, z anycompatible);
...
select x, pg_typeof(x), y, pg_typeof(y)  from f1(11, array[1, 2.2], 42, 34.5);  drop function f1(a anyelement, b anyarray,                 c anycompatible, d anycompatible);
...
begin  my_id_user = sp_id_user( a_login );
  IF  my_id_user > 0 THEN    RETURN -1;    END IF;
  INSERT INTO users ( login ) VALUES ( a_login );
  my_id_user = sp_id_user( a_login );
  IF  my_id_user = 0 THEN    RETURN -2;    END IF;
  RETURN my_id_user;
end language plpgsql;
...
do declare  x int;
  y int;
begin  select * from test_01 into x, y;   raise notice 'ok';
  select * from test_01 into x;    end;
;
do declare  t test_01;
begin  select 1, 2 into t;    raise notice 'ok';
  select 1, 2, 3 into t; end;
;
do declare  t test_01;
begin  select 1 into t; end;
;
drop table test_01;
reset plpgsql.extra_errors;
reset plpgsql.extra_warnings;
...
begin  open c;
 ...
 language plpgsql;
select * from sc_test();  create or replace function sc_test() returns setof integer as declare  c refcursor;
  x integer;
...
begin  begin    i := (SELECT NULL::integer);    exception    WHEN OTHERS THEN      i := (SELECT 1::integer);
  end;
  return i;
end;
 LANGUAGE plpgsql;
...
select cast_invoker(20150718);  begin;
select cast_invoker(20150717);
select cast_invoker(20150718);
savepoint s1;
select cast_invoker(20150718);
select cast_invoker(-1); rollback to savepoint s1;
...
select arrayassign1();
select arrayassign1(); create domain orderedarray as int[2]  constraint sorted check (value[1] < value[2]);
select '{1,2}'::orderedarray;
select '{2,1}'::orderedarray;  create function testoa(x1 int, x2 int, x3 int) returns orderedarraylanguage plpgsql as declare res orderedarray;
begin  res := array[x1, x2];
  res[2] := x3;
  return res;
end;
select testoa(1,2,3);
select testoa(1,2,3); select testoa(2,1,3); select testoa(1,2,1); drop function arrayassign1();
...
drop function outer_func(int);
drop function inner_func(int);
do begin  assert 1=1;  end;
;
do begin  assert 1=0;  end;
;
