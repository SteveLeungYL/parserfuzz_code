...
create index on idxpart (a);
create table idxpart1 partition of idxpart for values from (0) to (10);
drop index idxpart1_a_idx;	drop index concurrently idxpart_a_idx;	drop index idxpart_a_idx;	select relname, relkind from pg_class  where relname like 'idxpart%' order by relname;
create index on idxpart (a);
drop table idxpart1;		select relname, relkind from pg_class  where relname like 'idxpart%' order by relname;
drop table idxpart;
...
drop index idxpart1_temp_a_idx; drop index concurrently idxpart_temp_a_idx;
...
alter index idxpart_a_b_idx attach partition idxpart1_a_b_idx; create index idxpart1_2_a_b on idxpart1 (a, b);
...
alter index idxpart_1_idx attach partition idxpart1_1b_idx;	alter index idxpart_1_idx attach partition idxpart1_1_idx;
...
alter table idxpart add primary key (a);	alter table idxpart add primary key (a, b);	\d idxpartcreate table idxpart1 partition of idxpart for values from (0, 0) to (1000, 1000);
\d idxpart1drop table idxpart;
create table idxpart (a int, b int) partition by range (a, b);
alter table idxpart add unique (a);			alter table idxpart add unique (b, a);		\d idxpartdrop table idxpart;
create table idxpart (a int, b int) partition by range (a);
...
create table idxpart2 partition of idxpartfor values from (0) to (1000) partition by range (b); drop table idxpart;
...
alter table only idxpart add primary key (a);  alter table idxpart0 alter column a set not null;
alter table only idxpart add primary key (a);  alter table idxpart0 alter column a drop not null;  drop table idxpart;
create table idxpart (a int, b int) partition by range (a);
...
alter table idxpart attach partition idxpart1 for values from (1) to (1000);
alter table only idxpart add primary key (a);
alter index idxpart_pkey attach partition idxpart1_a_idx;	drop table idxpart;
...
insert into covidxpart values (4, 1);
create unique index on covidxpart (b) include (a); create table parted_pk_detach_test (a int primary key) partition by list (a);
create table parted_pk_detach_test1 partition of parted_pk_detach_test for values in (1);
alter table parted_pk_detach_test1 drop constraint parted_pk_detach_test1_pkey;	alter table parted_pk_detach_test detach partition parted_pk_detach_test1;
...
alter table parted_uniq_detach_test1 drop constraint parted_uniq_detach_test1_a_key;	alter table parted_uniq_detach_test detach partition parted_uniq_detach_test1;
