...
CREATE DOMAIN testdomain_i AS int COLLATE "sv_SE"; CREATE TABLE collate_test4 (    a int,    b testdomain_sv);
...
SELECT to_date('01 Şub 2010', 'DD TMMON YYYY');
SELECT to_date('1234567890ab 2010', 'TMMONTH YYYY'); CREATE VIEW collview1 AS SELECT * FROM collate_test1 WHERE b COLLATE "C" >= 'bbc';
...
';
END$$;
CREATE COLLATION test0 FROM "C"; CREATE COLLATION IF NOT EXISTS test0 FROM "C"; CREATE COLLATION IF NOT EXISTS test0 (locale = 'foo'); do $$BEGIN  EXECUTE 'CREATE COLLATION test1 (lc_collate = ' ||          quote_literal(current_setting('lc_collate')) ||          ', lc_ctype = ' ||          quote_literal(current_setting('lc_ctype')) || ');
';
';
END$$;
CREATE COLLATION test3 (lc_collate = 'en_US.utf8'); CREATE COLLATION testx (locale = 'nonsense'); CREATE COLLATION test4 FROM nonsense;
