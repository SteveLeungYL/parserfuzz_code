...
CREATE TABLE shoe_data (	shoename   char(10),      	sh_avail   integer,       	slcolor    char(10),      	slminlen   float,         	slmaxlen   float,         	slunit     char(8)        );
CREATE TABLE shoelace_data (	sl_name    char(10),      	sl_avail   integer,       	sl_color   char(10),      	sl_len     float,         	sl_unit    char(8)        );
...
select xmin, * from rules_fooview;  select reltoastrelid, relkind, relfrozenxid  from pg_class where oid = 'rules_fooview'::regclass;
drop view rules_fooview;
create table rules_fooview (x int, y text);
...
\d+ rules_srccreate rule r4 as on insert to rules_src do instead insert into rules_log AS trgt SELECT NEW.* RETURNING trgt.f1, trgt.f2;
create rule r5 as on update to rules_src do instead UPDATE rules_log AS trgt SET tag = 'updated' WHERE trgt.f1 = new.f1;
\d+ rules_srccreate table rule_t1(f1 int, f2 int);
...
\d+ rule_t1drop table rule_t1, rule_dest;
...
SELECT * FROM rule_v1;
\d+ rule_v1ALTER RULE InsertRule ON rule_v1 RENAME TO NewInsertRule; ALTER RULE NewInsertRule ON rule_v1 RENAME TO "_RETURN"; ALTER RULE "_RETURN" ON rule_v1 RENAME TO abc; DROP VIEW rule_v1;
DROP TABLE rule_t1;
create view rule_v1 as values(1,2);
\d+ rule_v1alter table rule_v1 rename column column2 to q2;
\d+ rule_v1drop view rule_v1;
