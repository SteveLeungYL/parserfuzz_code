...
INSERT INTO z VALUES (NULL, 'text'); create table some_tab (a int, b int);
...
CREATE TABLE jointchild () INHERITS (firstparent, secondparent);  CREATE TABLE thirdparent (tomorrow date default now()::date - 1);
CREATE TABLE otherchild () INHERITS (firstparent, thirdparent);  CREATE TABLE otherchild (tomorrow date default now())  INHERITS (firstparent, thirdparent);  DROP TABLE firstparent, secondparent, jointchild, thirdparent, otherchild;
...
alter table parent1 alter column f1 type bigint;  alter table parent1 alter column f2 type bigint;  create table p1(ff1 int);
alter table p1 add constraint p1chk check (ff1 > 0) no inherit;
alter table p1 add constraint p2chk check (ff1 > 10);
select pc.relname, pgc.conname, pgc.contype, pgc.conislocal, pgc.coninhcount, pgc.connoinherit from pg_class as pc inner join pg_constraint as pgc on (pgc.conrelid = pc.oid) where pc.relname = 'p1' order by 1,2;
create table c1 () inherits (p1);
\d p1\d c1create table c2 (constraint p2chk check (ff1 > 10) no inherit) inherits (p1);	drop table p1 cascade;
...
alter table bc drop constraint ac_check;  alter table ac drop constraint ac_check;
select pc.relname, pgc.conname, pgc.contype, pgc.conislocal, pgc.coninhcount, pg_get_expr(pgc.conbin, pc.oid) as consrc from pg_class as pc inner join pg_constraint as pgc on (pgc.conrelid = pc.oid) where pc.relname in ('ac', 'bc') order by 1,2;
alter table ac add check (aa is not null);
select pc.relname, pgc.conname, pgc.contype, pgc.conislocal, pgc.coninhcount, pg_get_expr(pgc.conbin, pc.oid) as consrc from pg_class as pc inner join pg_constraint as pgc on (pgc.conrelid = pc.oid) where pc.relname in ('ac', 'bc') order by 1,2;
insert into ac (aa) values (NULL);
insert into bc (aa) values (NULL);
alter table bc drop constraint ac_aa_check;  alter table ac drop constraint ac_aa_check;
select pc.relname, pgc.conname, pgc.contype, pgc.conislocal, pgc.coninhcount, pg_get_expr(pgc.conbin, pc.oid) as consrc from pg_class as pc inner join pg_constraint as pgc on (pgc.conrelid = pc.oid) where pc.relname in ('ac', 'bc') order by 1,2;
alter table ac add constraint ac_check check (aa is not null);
...
alter table p2 add constraint cc check (f2>0);  alter table p2 add check (f2>0);  delete from c1;
insert into c1 values(1,1,2);
alter table p2 add check (f2>0);
insert into c1 values(1,-1,2);  create table c2(f3 int) inherits(p1,p2);
...
ALTER TABLE inht1 RENAME b TO bb;                ALTER TABLE inhts RENAME aa TO aaa;      ALTER TABLE inhts RENAME d TO dd;
...
alter table invalid_check_con add constraint inh_check_constraint check(f1 > 0); alter table invalid_check_con_child drop constraint inh_check_constraint;
...
insert into invalid_check_con values(0); insert into invalid_check_con_child values(0); select conrelid::regclass::text as relname, conname,       convalidated, conislocal, coninhcount, connoinheritfrom pg_constraint where conname like 'inh\_check\_constraint%'order by 1, 2;
...
set enable_indexscan = off;  explain (verbose, costs off) select * from matest0 order by 1-id;
select * from matest0 order by 1-id;
explain (verbose, costs off) select min(1-id) from matest0;
select min(1-id) from matest0;
reset enable_indexscan;
set enable_seqscan = off;  set enable_parallel_append = off; explain (verbose, costs off) select * from matest0 order by 1-id;
select * from matest0 order by 1-id;
explain (verbose, costs off) select min(1-id) from matest0;
select min(1-id) from matest0;
...
set enable_nestloop = off;  explain (costs off)select t1.* from matest0 t1, matest0 t2where t1.b = t2.b and t2.c = t2.dorder by t1.b limit 10;
...
create temp table inh_temp_child () inherits (inh_perm_parent); create table inh_perm_child () inherits (inh_temp_parent); create temp table inh_temp_child_2 () inherits (inh_temp_parent); insert into inh_perm_parent values (1);
...
explain (costs off) select * from mcrparted where a = 0;	explain (costs off) select * from mcrparted where a = 10 and abs(b) < 5;	explain (costs off) select * from mcrparted where a = 10 and abs(b) = 5;	explain (costs off) select * from mcrparted where abs(b) = 5;	explain (costs off) select * from mcrparted where a > -1;	explain (costs off) select * from mcrparted where a = 20 and abs(b) = 10 and c > 10;	explain (costs off) select * from mcrparted where a = 20 and c > 20; create table parted_minmax (a int, b varchar(16)) partition by range (a);
