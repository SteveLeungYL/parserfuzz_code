...
SELECT * FROM arrtest_s;
UPDATE arrtest_s SET a[:] = '{23, 24, 25}';  INSERT INTO arrtest_s VALUES(NULL, NULL);
UPDATE arrtest_s SET a[:] = '{11, 12, 13, 14, 15}';  SELECT f1[0:1] FROM POINT_TBL;
...
select 'foo' like any (array['%a', '%o']); select 'foo' like any (array['%a', '%b']); select 'foo' like all (array['f%', '%o']); select 'foo' like all (array['f%', '%b']); select 'foo' not like any (array['%a', '%b']); select 'foo' not like all (array['%a', '%o']); select 'foo' ilike any (array['%A', '%O']); select 'foo' ilike all (array['F%', '%O']); select '{{1,{2}},{2,3}}'::text[];
select '{{},{}}'::text[];
...
select array_remove('{{1,2,2},{1,4,3}}', 2); select array_remove(array['X','X','X'], 'X') = '{}';
