...
select '(Joe,)'::fullname;	select '(Joe)'::fullname;	select '(Joe,,)'::fullname;	select '[]'::fullname;          select ' (Joe,Blow)  '::fullname;  select '(Joe,Blow) /'::fullname;  create temp table quadtable(f1 int, q quad);
insert into quadtable values (1, ((3.3,4.4),(5.5,6.6)));
insert into quadtable values (2, ((null,4.4),(5.5,6.6)));
select * from quadtable;
select f1, q.c1 from quadtable;		select f1, (q).c1, (qq.q).c1.i from quadtable qq;
...
select ROW(1,2,3) < ROW(1,3,NULL) as true; select ROW(11,'ABC') < ROW(11,'DEF') as true;
...
reset enable_sort;
select * from int8_tbl i8 where i8 in (row(123,456));  explain (costs off)select * from int8_tbl i8where i8 in (row(123,456)::int8_tbl, '(4567890123456789,123)');
select * from int8_tbl i8where i8 in (row(123,456)::int8_tbl, '(4567890123456789,123)');
select (row(1, 2.0)).f1;
select (row(1, 2.0)).f2;
select (row(1, 2.0)).nosuch;  select (row(1, 2.0)).*;
select (r).f1 from (select row(1, 2.0) as r) ss;
select (r).f3 from (select row(1, 2.0) as r) ss;  select (r).* from (select row(1, 2.0) as r) ss;
select ROW();
select ROW() IS NULL;
select ROW() = ROW();
...
select * from cc order by f1; create type testtype1 as (a int, b int);
...
select row(1, -3)::testtype1 *> row(1, -2)::testtype1;
select row(1, -2)::testtype1 *< row(1, 3)::testtype1;
create type testtype2 as (a smallint, b bool);  select row(1, true)::testtype2 *< row(2, true)::testtype2;
select row(-2, true)::testtype2 *< row(-1, true)::testtype2;
select row(0, false)::testtype2 *< row(0, true)::testtype2;
select row(0, false)::testtype2 *<> row(0, true)::testtype2;
create type testtype3 as (a int, b text);  select row(1, 'abc')::testtype3 *< row(1, 'abd')::testtype3;
select row(1, 'abc')::testtype3 *< row(1, 'abcd')::testtype3;
select row(1, 'abc')::testtype3 *> row(1, 'abd')::testtype3;
select row(1, 'abc')::testtype3 *<> row(1, 'abd')::testtype3;
create type testtype4 as (a int, b point);  select row(1, '(1,2)')::testtype4 *< row(1, '(1,3)')::testtype4;
...
select text(fullname) from fullname;  select fullname.text from fullname;  select cast (row('Jim', 'Beam') as text);
select (row('Jim', 'Beam'))::text;
select text(row('Jim', 'Beam'));  select (row('Jim', 'Beam')).text;  insert into fullname values ('Joe', 'Blow');
