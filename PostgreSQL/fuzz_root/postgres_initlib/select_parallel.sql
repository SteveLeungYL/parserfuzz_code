...
set work_mem='64kB';  explain (costs off)	select count(*) from tenk1, tenk2 where tenk1.hundred > 1 and tenk2.thousand=0;
...
alter table tenk2 reset (parallel_workers);
reset work_mem;
create function explain_parallel_sort_stats() returns setof textlanguage plpgsql asdeclare ln text;
...
CREATE FUNCTION make_record(n int)  RETURNS RECORD LANGUAGE plpgsql PARALLEL SAFE ASBEGIN  RETURN CASE n           WHEN 1 THEN ROW(1)           WHEN 2 THEN ROW(1, 2)           WHEN 3 THEN ROW(1, 2, 3)           WHEN 4 THEN ROW(1, 2, 3, 4)           ELSE ROW(1, 2, 3, 4, 5)         END;
END;
;
SAVEPOINT settings;
SET LOCAL force_parallel_mode = 1;
...
CREATE FUNCTION make_some_array(int,int) returns int[] asdeclare x int[];
  begin    x[1] :=  1;
    x[2] :=  2;
    return x;
