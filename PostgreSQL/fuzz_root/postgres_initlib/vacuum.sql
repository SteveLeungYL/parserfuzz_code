...
DELETE FROM vactst WHERE i % 7 = 0; BEGIN;
INSERT INTO vactst SELECT generate_series(301, 400);
DELETE FROM vactst WHERE i % 5 <> 0; ANALYZE vactst;
COMMIT;
VACUUM FULL pg_am;
...
VACUUM (PARALLEL 2) pvactst;
UPDATE pvactst SET i = i WHERE i < 1000;
VACUUM (PARALLEL 0) pvactst; VACUUM (PARALLEL -1) pvactst; VACUUM (PARALLEL 2, INDEX_CLEANUP FALSE) pvactst;
VACUUM (PARALLEL 2, FULL TRUE) pvactst; VACUUM (PARALLEL) pvactst; CREATE TEMPORARY TABLE tmp (a int PRIMARY KEY);
CREATE INDEX tmp_idx1 ON tmp (a);
VACUUM (PARALLEL 1, FULL FALSE) tmp; VACUUM (PARALLEL 0, FULL TRUE) tmp; RESET min_parallel_index_scan_size;
DROP TABLE pvactst;
...
VACUUM (INDEX_CLEANUP FALSE) vactst; VACUUM (INDEX_CLEANUP FALSE, FREEZE TRUE) vaccluster;
