...
SELECT jsonb_build_array('a', NULL); SELECT jsonb_build_array(VARIADIC NULL::text[]); SELECT jsonb_build_array(VARIADIC '{}'::text[]); SELECT jsonb_build_array(VARIADIC '{a,b,c}'::text[]); SELECT jsonb_build_array(VARIADIC ARRAY['a', NULL]::text[]); SELECT jsonb_build_array(VARIADIC '{1,2,3,4}'::text[]); SELECT jsonb_build_array(VARIADIC '{1,2,3,4}'::int[]); SELECT jsonb_build_array(VARIADIC '{{1,4},{2,5},{3,6}}'::int[][]); SELECT jsonb_build_object('a',1,'b',1.2,'c',true,'d',null,'e',json '{"x": 3, "y": [1,2,3]}');
...
SELECT jsonb_build_object('{a,b,c}'::text[]); SELECT jsonb_build_object('{a,b,c}'::text[], '{d,e,f}'::text[]); SELECT jsonb_build_object('a', 'b', 'c'); SELECT jsonb_build_object(NULL, 'a'); SELECT jsonb_build_object('a', NULL); SELECT jsonb_build_object(VARIADIC NULL::text[]); SELECT jsonb_build_object(VARIADIC '{}'::text[]); SELECT jsonb_build_object(VARIADIC '{a,b,c}'::text[]); SELECT jsonb_build_object(VARIADIC ARRAY['a', NULL]::text[]); SELECT jsonb_build_object(VARIADIC ARRAY[NULL, 'a']::text[]); SELECT jsonb_build_object(VARIADIC '{1,2,3,4}'::text[]); SELECT jsonb_build_object(VARIADIC '{1,2,3,4}'::int[]); SELECT jsonb_build_object(VARIADIC '{{1,4},{2,5},{3,6}}'::int[][]); SELECT jsonb_build_array();
...
SELECT '{"n":null,"a":1,"b":[1,2],"c":{"1":2},"d":{"1":[2,3]}}'::jsonb -> 0; SELECT '["a","b","c",[1,2],null]'::jsonb -> 0;
...
select '{"n":null, "a":1, "b":[1,2], "c":{"1":2}, "d":{"1":[2,3]}}'::jsonb #- '{b,-1e}'; select '{"n":null, "a":1, "b":[1,2], "c":{"1":2}, "d":{"1":[2,3]}}'::jsonb #- '{d,1,0}';
select '"a"'::jsonb - 'a'; select '{}'::jsonb - 'a';
select '[]'::jsonb - 'a';
select '"a"'::jsonb - 1; select '{}'::jsonb -  1; select '[]'::jsonb - 1;
select '"a"'::jsonb #- '{a}'; select '{}'::jsonb #- '{a}';
select '[]'::jsonb #- '{a}';
select jsonb_set('"a"','{a}','"b"'); select jsonb_set('{}','{a}','"b"', false);
...
\pset null NULLselect jsonb_set_lax('{"a":1,"b":2}','{b}','5') ;
...
\pset nullselect jsonb_insert('{"a": [0,1,2]}', '{a, 1}', '"new_value"');
