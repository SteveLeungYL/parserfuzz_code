CALL nonexistent();  CALL random();  CREATE FUNCTION cp_testfunc1(a int) RETURNS int LANGUAGE SQL AS  SELECT a ;
...
\dfn public.*test*1\dfp public.*test*1SELECT ptest1('x');  CALL ptest1('a');  CALL ptest1('xy' || 'zzy');  CALL ptest1(substring(random()::numeric(20,15)::text, 1, 1));  SELECT * FROM cp_test ORDER BY b COLLATE "C";
...
CALL version();  CALL sum(1);  CREATE PROCEDURE ptestx() LANGUAGE SQL WINDOW AS  INSERT INTO cp_test VALUES (1, 'a') ;
CREATE PROCEDURE ptestx() LANGUAGE SQL STRICT AS  INSERT INTO cp_test VALUES (1, 'a') ;
CREATE PROCEDURE ptestx(OUT a int) LANGUAGE SQL AS  INSERT INTO cp_test VALUES (1, 'a') ;
ALTER PROCEDURE ptest1(text) STRICT;
ALTER FUNCTION ptest1(text) VOLATILE;  ALTER PROCEDURE cp_testfunc1(int) VOLATILE;  ALTER PROCEDURE nonexistent() VOLATILE;
DROP FUNCTION ptest1(text);  DROP PROCEDURE cp_testfunc1(int);  DROP PROCEDURE nonexistent();
CREATE USER regress_cp_user1;
GRANT INSERT ON cp_test TO regress_cp_user1;
REVOKE EXECUTE ON PROCEDURE ptest1(text) FROM PUBLIC;
SET ROLE regress_cp_user1;
CALL ptest1('a');  RESET ROLE;
