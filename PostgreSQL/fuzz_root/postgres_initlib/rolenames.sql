...
CREATE ROLE current_user; CREATE ROLE current_role; CREATE ROLE session_user; CREATE ROLE user; CREATE ROLE all; CREATE ROLE public; CREATE ROLE "public"; CREATE ROLE none; CREATE ROLE "none"; CREATE ROLE pg_abc; CREATE ROLE "pg_abc"; CREATE ROLE pg_abcdef; CREATE ROLE "pg_abcdef"; CREATE ROLE regress_testrol0 SUPERUSER LOGIN;
...
SELECT * FROM chkrolattr();
ROLLBACK;
ALTER ROLE USER WITH LOGIN; ALTER ROLE CURRENT_ROLE WITH LOGIN; ALTER ROLE ALL WITH REPLICATION; ALTER ROLE SESSION_ROLE WITH NOREPLICATION; ALTER ROLE PUBLIC WITH NOREPLICATION; ALTER ROLE "public" WITH NOREPLICATION; ALTER ROLE NONE WITH NOREPLICATION; ALTER ROLE "none" WITH NOREPLICATION; ALTER ROLE nonexistent WITH NOREPLICATION; BEGIN;
SELECT * FROM chkrolattr();
...
SELECT * FROM chkrolattr();
ROLLBACK;
ALTER USER USER WITH LOGIN; ALTER USER CURRENT_ROLE WITH LOGIN; ALTER USER ALL WITH REPLICATION; ALTER USER SESSION_ROLE WITH NOREPLICATION; ALTER USER PUBLIC WITH NOREPLICATION; ALTER USER "public" WITH NOREPLICATION; ALTER USER NONE WITH NOREPLICATION; ALTER USER "none" WITH NOREPLICATION; ALTER USER nonexistent WITH NOREPLICATION; SELECT * FROM chksetconfig();
...
SELECT * FROM chksetconfig();
ALTER ROLE CURRENT_ROLE SET application_name to 'BAZ'; ALTER ROLE USER SET application_name to 'BOOM'; ALTER ROLE PUBLIC SET application_name to 'BOMB'; ALTER ROLE nonexistent SET application_name to 'BOMB'; SELECT * FROM chksetconfig();
...
SELECT * FROM chksetconfig();
ALTER USER CURRENT_USER SET application_name to 'BAZ'; ALTER USER USER SET application_name to 'BOOM'; ALTER USER PUBLIC SET application_name to 'BOMB'; ALTER USER NONE SET application_name to 'BOMB'; ALTER USER nonexistent SET application_name to 'BOMB'; CREATE SCHEMA newschema1 AUTHORIZATION CURRENT_USER;
...
CREATE SCHEMA newschema6 AUTHORIZATION USER; CREATE SCHEMA newschema6 AUTHORIZATION CURRENT_ROLE; CREATE SCHEMA newschema6 AUTHORIZATION PUBLIC; CREATE SCHEMA newschema6 AUTHORIZATION "public"; CREATE SCHEMA newschema6 AUTHORIZATION NONE; CREATE SCHEMA newschema6 AUTHORIZATION nonexistent; SELECT n.nspname, r.rolname FROM pg_namespace n JOIN pg_roles r ON (r.oid = n.nspowner) WHERE n.nspname LIKE 'newschema_' ORDER BY 1;
...
CREATE SCHEMA IF NOT EXISTS newschema6 AUTHORIZATION USER; CREATE SCHEMA IF NOT EXISTS newschema6 AUTHORIZATION CURRENT_ROLE; CREATE SCHEMA IF NOT EXISTS newschema6 AUTHORIZATION PUBLIC; CREATE SCHEMA IF NOT EXISTS newschema6 AUTHORIZATION "public"; CREATE SCHEMA IF NOT EXISTS newschema6 AUTHORIZATION NONE; CREATE SCHEMA IF NOT EXISTS newschema6 AUTHORIZATION nonexistent; SELECT n.nspname, r.rolname FROM pg_namespace n JOIN pg_roles r ON (r.oid = n.nspowner) WHERE n.nspname LIKE 'newschema_' ORDER BY 1;
...
ALTER TABLE testtab6 OWNER TO CURRENT_ROLE; ALTER TABLE testtab6 OWNER TO USER; ALTER TABLE testtab6 OWNER TO PUBLIC; ALTER TABLE testtab6 OWNER TO "public"; ALTER TABLE testtab6 OWNER TO nonexistent; SELECT c.relname, r.rolname FROM pg_class c JOIN pg_roles r ON (r.oid = c.relowner) WHERE relname LIKE 'testtab_' ORDER BY 1;
\c -SET SESSION AUTHORIZATION regress_testrol0;
...
ALTER AGGREGATE testagg5(int2) OWNER TO CURRENT_ROLE; ALTER AGGREGATE testagg5(int2) OWNER TO USER; ALTER AGGREGATE testagg5(int2) OWNER TO PUBLIC; ALTER AGGREGATE testagg5(int2) OWNER TO "public"; ALTER AGGREGATE testagg5(int2) OWNER TO nonexistent; SELECT p.proname, r.rolname FROM pg_proc p JOIN pg_roles r ON (r.oid = p.proowner) WHERE proname LIKE 'testagg_' ORDER BY 1;
...
CREATE USER MAPPING FOR CURRENT_ROLE SERVER sv9	    OPTIONS (user 'CURRENT_ROLE'); CREATE USER MAPPING FOR nonexistent SERVER sv9	    OPTIONS (user 'nonexistent'); SELECT * FROM chkumapping();
...
ALTER USER MAPPING FOR nonexistent SERVER sv9 OPTIONS (SET user 'nonexistent_alt'); SELECT * FROM chkumapping();
...
DROP USER MAPPING FOR CURRENT_ROLE SERVER sv9; DROP USER MAPPING FOR nonexistent SERVER sv;  SELECT * FROM chkumapping();
CREATE USER MAPPING FOR CURRENT_USER SERVER sv1 OPTIONS (user 'CURRENT_USER');
CREATE USER MAPPING FOR "current_user" SERVER sv2 OPTIONS (user '"current_user"');
CREATE USER MAPPING FOR USER SERVER sv3 OPTIONS (user 'USER');
CREATE USER MAPPING FOR "user" SERVER sv4 OPTIONS (user '"USER"');
...
SELECT * FROM chkumapping();
DROP USER MAPPING IF EXISTS FOR CURRENT_ROLE SERVER sv9; DROP USER MAPPING IF EXISTS FOR nonexistent SERVER sv9;  GRANT regress_testrol0 TO pg_signal_backend; SET ROLE pg_signal_backend; RESET ROLE;
CREATE SCHEMA test_roles_schema AUTHORIZATION pg_signal_backend; SET ROLE regress_testrol2;
...
SELECT proname, proacl FROM pg_proc WHERE proname LIKE 'testagg_';
GRANT ALL PRIVILEGES ON FUNCTION testagg9(int2) TO CURRENT_ROLE; GRANT ALL PRIVILEGES ON FUNCTION testagg9(int2) TO USER; GRANT ALL PRIVILEGES ON FUNCTION testagg9(int2) TO NONE; GRANT ALL PRIVILEGES ON FUNCTION testagg9(int2) TO "none"; SELECT proname, proacl FROM pg_proc WHERE proname LIKE 'testagg_';
REVOKE ALL PRIVILEGES ON FUNCTION testagg1(int2) FROM PUBLIC;
...
SELECT proname, proacl FROM pg_proc WHERE proname LIKE 'testagg_';
REVOKE ALL PRIVILEGES ON FUNCTION testagg9(int2) FROM CURRENT_ROLE; REVOKE ALL PRIVILEGES ON FUNCTION testagg9(int2) FROM USER; REVOKE ALL PRIVILEGES ON FUNCTION testagg9(int2) FROM NONE; REVOKE ALL PRIVILEGES ON FUNCTION testagg9(int2) FROM "none"; SELECT proname, proacl FROM pg_proc WHERE proname LIKE 'testagg_';
