...
select polyf(point(3,4));  drop function polyf(x anyelement);
...
select polyf(stavalues1) from pg_statistic;  drop function polyf(x anyarray);
...
select polyf(int4range(42, 49), 11, 4.5) as fail;  drop function polyf(x anycompatiblerange, y anycompatible, z anycompatible);
...
select x, pg_typeof(x), y, pg_typeof(y)  from polyf(11, array[1, 2.2], 42, 34.5);  drop function polyf(a anyelement, b anyarray,                    c anycompatible, d anycompatible);
...
select array_in('{1,2,3}','int4'::regtype,-1);  select * from array_in('{1,2,3}','int4'::regtype,-1);  select anyrange_in('[10,20)','int4range'::regtype,-1);
...
select myleast(); select myleast(variadic array[1,2,3,4,-1]);
...
select formarray(1.1, array[1.2,55.5]); select formarray(1, 'x'::text); select formarray(1, variadic array['x'::text]); drop function formarray(anyelement, variadic anyarray);
select pg_typeof(null);           select pg_typeof(0);              select pg_typeof(0.0);            select pg_typeof(1+1 = 2);        select pg_typeof('x');            select pg_typeof('' || '');       select pg_typeof(pg_typeof(0));   select pg_typeof(array[1.2,55.5]); select pg_typeof(myleast(10, 1, 20, 33));  create function dfunc(a int = 1, int = 2) returns int as   select $1 + $2;
 language sql;
select dfunc();
select dfunc(10);
select dfunc(10, 20);
select dfunc(10, 20, 30);  drop function dfunc();  drop function dfunc(int);  drop function dfunc(int, int);  create function dfunc(a int = 1, b int) returns int as   select $1 + $2;
 language sql;
...
create function dfunc(a text DEFAULT 'Hello', b text DEFAULT 'World') returns text as   select $1 || ', ' || $2;
 language sql;
select dfunc();  select dfunc('Hi');  select dfunc('Hi', 'City');  select dfunc(0);  select dfunc(10, 20);  drop function dfunc(int, int);
...
create function dfunc(int = 1, int = 2, int = 3, int = 4) returns int as   select 4;
 language sql;
select dfunc();  select dfunc(1);  select dfunc(1, 2);  select dfunc(1, 2, 3);  select dfunc(1, 2, 3, 4);  drop function dfunc(int, int);
...
create function dfunc(text) returns text as   select $1;
 language sql;
select dfunc(1);  select dfunc('Hi');
...
select * from dfunc(0);  select * from dfunc(1,2);
select * from dfunc(1,2,c := 3);
select * from dfunc(1,2,d := 3);
select * from dfunc(x := 20, b := 10, x := 30);  select * from dfunc(10, b := 20, 30);  select * from dfunc(x := 10, b := 20, c := 30);  select * from dfunc(10, 10, a := 20);  select * from dfunc(1,c := 2,d := 3); drop function dfunc(int, int, int, int);
...
select * from dfunc('Hello World', c := 20, b := '2009-07-25'::date);  drop function dfunc(varchar, numeric, date);
...
create function dfunc(a anyelement, b anyelement = null, flag bool = true)returns anyelement as   select case when $3 then $1 else $2 end;
 language sql;
select dfunc(1,2);
select dfunc('a'::text, 'b'); select dfunc(a := 1, b := 2);
select dfunc(a := 'a'::text, b := 'b');
select dfunc(a := 'a'::text, b := 'b', flag := false); select dfunc(b := 'b'::text, a := 'a'); select dfunc(a := 'a'::text, flag := true); select dfunc(a := 'a'::text, flag := false); select dfunc(b := 'b'::text, a := 'a', flag := true); select dfunc('a'::text, 'b', false); select dfunc('a'::text, 'b', flag := false); select dfunc('a'::text, 'b', true); select dfunc('a'::text, 'b', flag := true); select dfunc(a => 1, b => 2);
select dfunc(a => 'a'::text, b => 'b');
select dfunc(a => 'a'::text, b => 'b', flag => false); select dfunc(b => 'b'::text, a => 'a'); select dfunc(a => 'a'::text, flag => true); select dfunc(a => 'a'::text, flag => false); select dfunc(b => 'b'::text, a => 'a', flag => true); select dfunc('a'::text, 'b', false); select dfunc('a'::text, 'b', flag => false); select dfunc('a'::text, 'b', true); select dfunc('a'::text, 'b', flag => true); select dfunc(a =>-1);
select dfunc(a =>+1);
select dfunc(a =>  1);
...
select x, pg_typeof(x) from anyctest(11, point(1,2)) x;  select x, pg_typeof(x) from anyctest('11', '12.3') x;  drop function anyctest(anycompatible, anycompatible);
create function anyctest(anycompatible, anycompatible)returns anycompatiblearray as   select array[$1, $2] language sql;
select x, pg_typeof(x) from anyctest(11, 12) x;
select x, pg_typeof(x) from anyctest(11, 12.3) x;
select x, pg_typeof(x) from anyctest(11, array[1,2]) x;  drop function anyctest(anycompatible, anycompatible);
...
select x, pg_typeof(x) from anyctest(11, array[point(1,2)]) x;  select x, pg_typeof(x) from anyctest(11, 12) x;  drop function anyctest(anycompatible, anycompatiblearray);
...
select x, pg_typeof(x) from anyctest(11, 12) x;  select x, pg_typeof(x) from anyctest(11.2, int4range(4,7)) x;  select x, pg_typeof(x) from anyctest(11.2, '[4,7)') x;  drop function anyctest(anycompatible, anycompatiblerange);
...
select x, pg_typeof(x) from anyctest(int4range(11,12), numrange(4,7)) x; drop function anyctest(anycompatiblerange, anycompatiblerange);
...
select x, pg_typeof(x) from anyctest(11, 12) x;
select x, pg_typeof(x) from anyctest(11, 12.3) x;
select x, pg_typeof(x) from anyctest(array[11], array[1,2]) x;  drop function anyctest(anycompatiblenonarray, anycompatiblenonarray);
...
select x, pg_typeof(x) from anyctest(11, array[1, 2.2], 42, 34.5) x;  drop function anyctest(a anyelement, b anyarray,                       c anycompatible, d anycompatible);
...
select x, pg_typeof(x) from anyctest(11, '12') x;
select x, pg_typeof(x) from anyctest(11, '12.2') x;  select x, pg_typeof(x) from anyctest(variadic array[11, 12]) x;
