...
CREATE FUNCTION func_d_1_15() RETURNS trigger AS $$BEGIN   NEW.c = NEW.c + 1;    return NEW;
END $$ LANGUAGE plpgsql;
...
insert into utrtest values (2, 'bar')  returning *, tableoid::regclass, xmin = pg_current_xact_id()::xid as xmin_ok;  insert into utrtest values (2, 'bar')  returning *, tableoid::regclass;
...
update utrtest set a = 3 - a from (values (1), (2)) s(x) where a = s.x  returning *, tableoid::regclass, xmin = pg_current_xact_id()::xid as xmin_ok;  update utrtest set a = 3 - a from (values (1), (2)) s(x) where a = s.x  returning *, tableoid::regclass;
...
SELECT tableoid::regclass::text, * FROM list_parted WHERE a = 2 ORDER BY 1;
CREATE FUNCTION func_parted_mod_b() returns trigger as $$BEGIN   NEW.b = 2;    return NEW;
END $$ LANGUAGE plpgsql;
