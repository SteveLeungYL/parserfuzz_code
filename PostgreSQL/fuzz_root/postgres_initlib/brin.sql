...
VACUUM brintest;  UPDATE brintest SET int8col = int8col * int4col;
UPDATE brintest SET textcol = '' WHERE textcol IS NOT NULL;
SELECT brin_summarize_new_values('brintest'); SELECT brin_summarize_new_values('tenk1_unique1'); SELECT brin_summarize_new_values('brinidx'); SELECT brin_desummarize_range('brinidx', -1); SELECT brin_desummarize_range('brinidx', 0);
SELECT brin_desummarize_range('brinidx', 0);
...
WITH rand_value AS (SELECT string_agg(md5(i::text),'') AS val FROM generate_series(1,60) s(i))INSERT INTO brintest_3SELECT val, val, val, val FROM rand_value;
