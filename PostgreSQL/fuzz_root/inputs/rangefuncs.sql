...
select * from rngfunct(1) with ordinality as z(a,b,ord) where b > 100;   select a,b,ord from rngfunct(1) with ordinality as z(a,b,ord);
...
SELECT dup(22);
SELECT dup('xyz');	SELECT dup('xyz'::text);
...
select * from array_to_set(array['one', 'two']); select * from array_to_set(array['one', 'two']) as t(f1 numeric(4,2),f2 text);
...
select * from testrngfunc(); drop function testrngfunc();
...
SELECT * FROM get_users() WITH ORDINALITY;   SELECT * FROM ROWS FROM(generate_series(10,11), get_users()) WITH ORDINALITY;
...
select * from usersview;  rollback;
alter table users alter column seq type numeric;
select * from usersview;  drop view usersview;
...
select * from rngfuncbar();  create or replace function rngfuncbar(out integer, out numeric) as select (1, 2.1, 3)  language sql;
select * from rngfuncbar();  drop function rngfuncbar();
