...
select xmin, * from rules_fooview;
  select reltoastrelid, relkind, relfrozenxid  from pg_class where oid = 'rules_fooview'::regclass;
  select reltoastrelid, relkind, relfrozenxid  from pg_class where oid = 'rules_fooview'::regclass;
drop view rules_fooview;
create table rules_fooview (x int, y text);
...
SELECT * FROM rule_v1;
\d+ rule_v1ALTER RULE InsertRule ON rule_v1 RENAME TO NewInsertRule;
