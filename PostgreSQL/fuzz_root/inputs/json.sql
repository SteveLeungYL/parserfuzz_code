SELECT '""'::json;				SELECT ''::json;			SELECT '"abc"'::json;			SELECT '"abc'::json;			SELECT '"abcdef"'::json;					SELECT '"\n\"\\"'::json;		SELECT '"\v"'::json;			SELECT '1'::json;				SELECT '0'::json;				SELECT '01'::json;				SELECT '0.1'::json;				SELECT '9223372036854775808'::json;	SELECT '1e100'::json;			SELECT '1.3e100'::json;			SELECT '1f2'::json;				SELECT '0.x1'::json;			SELECT '1.3ex100'::json;		SELECT '[]'::json;				SELECT '[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]'::json;  SELECT '[1,2]'::json;			SELECT '[1,2,]'::json;			SELECT '[1,2'::json;			SELECT '[1,[2]'::json;			SELECT '{}'::json;				SELECT '{"abc"}'::json;			SELECT '{"abc":1}'::json;		SELECT '{1:"abc"}'::json;		SELECT '{"abc",1}'::json;		SELECT '{"abc"=1}'::json;		SELECT '{"abc"::1}'::json;		SELECT '{"abc":1,"def":2,"ghi":[3,4],"hij":{"klm":5,"nop":[6]}}'::json; SELECT '{"abc":1:2}'::json;		SELECT '{"abc":1,3}'::json;		SET max_stack_depth = '100kB';
SELECT repeat('[', 10000)::json;
SELECT repeat('{"a":', 10000)::json;
RESET max_stack_depth;
SELECT 'true'::json;			SELECT 'false'::json;			SELECT 'null'::json;			SELECT ' true '::json;			SELECT 'true false'::json;		SELECT 'true, false'::json;		SELECT 'truf'::json;			SELECT 'trues'::json;			SELECT ''::json;				SELECT '    '::json;			SELECT array_to_json(array(select 1 as a));
...
SELECT json_build_array('a', NULL); SELECT json_build_array(VARIADIC NULL::text[]); SELECT json_build_array(VARIADIC '{}'::text[]); SELECT json_build_array(VARIADIC '{a,b,c}'::text[]); SELECT json_build_array(VARIADIC ARRAY['a', NULL]::text[]); SELECT json_build_array(VARIADIC '{1,2,3,4}'::text[]); SELECT json_build_array(VARIADIC '{1,2,3,4}'::int[]); SELECT json_build_array(VARIADIC '{{1,4},{2,5},{3,6}}'::int[][]); SELECT json_build_object('a',1,'b',1.2,'c',true,'d',null,'e',json '{"x": 3, "y": [1,2,3]}');
...
SELECT json_build_object('{a,b,c}'::text[]); SELECT json_build_object('{a,b,c}'::text[], '{d,e,f}'::text[]); SELECT json_build_object('a', 'b', 'c'); SELECT json_build_object(NULL, 'a'); SELECT json_build_object('a', NULL); SELECT json_build_object(VARIADIC NULL::text[]); SELECT json_build_object(VARIADIC '{}'::text[]); SELECT json_build_object(VARIADIC '{a,b,c}'::text[]); SELECT json_build_object(VARIADIC ARRAY['a', NULL]::text[]); SELECT json_build_object(VARIADIC ARRAY[NULL, 'a']::text[]); SELECT json_build_object(VARIADIC '{1,2,3,4}'::text[]); SELECT json_build_object(VARIADIC '{1,2,3,4}'::int[]); SELECT json_build_object(VARIADIC '{{1,4},{2,5},{3,6}}'::int[][]); SELECT json_build_array();
