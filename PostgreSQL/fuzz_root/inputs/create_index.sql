...
\d+ gin_relopts_testCREATE INDEX hash_i4_index ON hash_i4_heap USING hash (random int4_ops);
...
\d func_index_heap\d func_index_indexDROP TABLE func_index_heap;
CREATE TABLE func_index_heap (f1 text, f2 text);
...
INSERT INTO func_index_heap VALUES('ABCD', 'EF');
INSERT INTO func_index_heap VALUES('QWERTY');
\d func_index_heap\d func_index_indexcreate index on func_index_heap ((f1 || f2), (row(f1, f2)));
...
VACUUM FULL concur_heap;
\d concur_heapREINDEX TABLE concur_heap;
\d concur_heapCREATE TEMP TABLE concur_temp (f1 int, f2 text)  ON COMMIT PRESERVE ROWS;
...
DROP INDEX CONCURRENTLY concur_temp_ind;
DROP TABLE concur_temp;
DROP INDEX CONCURRENTLY "concur_index2";				DROP INDEX CONCURRENTLY IF EXISTS "concur_index2";		DROP INDEX CONCURRENTLY "concur_index2", "concur_index3";
BEGIN;
...
\d concur_heapDROP TABLE concur_heap;
CREATE TABLE cwi_test( a int , b varchar(10), c char);
INSERT INTO cwi_test VALUES(1, 2), (3, 4), (5, 6);
CREATE UNIQUE INDEX cwi_uniq_idx ON cwi_test(a , b);
ALTER TABLE cwi_test ADD primary key USING INDEX cwi_uniq_idx;
\d cwi_test\d cwi_uniq_idxCREATE UNIQUE INDEX cwi_uniq2_idx ON cwi_test(b , a);
ALTER TABLE cwi_test DROP CONSTRAINT cwi_uniq_idx,	ADD CONSTRAINT cwi_replaced_pkey PRIMARY KEY		USING INDEX cwi_uniq2_idx;
\d cwi_test\d cwi_replaced_pkeyDROP INDEX cwi_replaced_pkey;	CREATE UNIQUE INDEX cwi_uniq3_idx ON cwi_test(a desc);
ALTER TABLE cwi_test ADD UNIQUE USING INDEX cwi_uniq3_idx;  CREATE UNIQUE INDEX cwi_uniq4_idx ON cwi_test(b collate "POSIX");
ALTER TABLE cwi_test ADD UNIQUE USING INDEX cwi_uniq4_idx;  DROP TABLE cwi_test;
...
vacuum tenk1;		explain (costs off)SELECT unique1 FROM tenk1WHERE unique1 IN (1,42,7)ORDER BY unique1;
...
\set VERBOSITY terse \\ REINDEX (VERBOSE) TABLE reindex_verbose;
\set VERBOSITY defaultDROP TABLE reindex_verbose;
CREATE TABLE concur_reindex_tab (c1 int);
REINDEX TABLE concur_reindex_tab; REINDEX TABLE CONCURRENTLY concur_reindex_tab; ALTER TABLE concur_reindex_tab ADD COLUMN c2 text; CREATE UNIQUE INDEX concur_reindex_ind1 ON concur_reindex_tab(c1);
...
REINDEX INDEX CONCURRENTLY  concur_reindex_tab3_c2_excl;  REINDEX TABLE CONCURRENTLY concur_reindex_tab3;  INSERT INTO concur_reindex_tab3 VALUES  (4, '[2,4]');
...
REINDEX TABLE CONCURRENTLY concur_reindex_tab;
COMMIT;
REINDEX TABLE CONCURRENTLY pg_class; REINDEX INDEX CONCURRENTLY pg_class_oid_index; REINDEX TABLE CONCURRENTLY pg_toast.pg_toast_1260; REINDEX INDEX CONCURRENTLY pg_toast.pg_toast_1260_index; REINDEX SYSTEM CONCURRENTLY postgres; REINDEX SCHEMA CONCURRENTLY pg_catalog;
\d concur_reindex_tabDROP MATERIALIZED VIEW concur_reindex_matview;
DROP TABLE concur_reindex_tab, concur_reindex_tab2, concur_reindex_tab3;
CREATE TABLE concur_reindex_tab4 (c1 int);
INSERT INTO concur_reindex_tab4 VALUES (1), (1), (2);
CREATE UNIQUE INDEX CONCURRENTLY concur_reindex_ind5 ON concur_reindex_tab4 (c1);
REINDEX INDEX CONCURRENTLY concur_reindex_ind5;
\d concur_reindex_tab4DROP INDEX concur_reindex_ind5_ccnew;
DELETE FROM concur_reindex_tab4 WHERE c1 = 1;
REINDEX TABLE CONCURRENTLY concur_reindex_tab4;
\d concur_reindex_tab4REINDEX INDEX CONCURRENTLY concur_reindex_ind5;
\d concur_reindex_tab4DROP TABLE concur_reindex_tab4;
...
COMMIT;
CREATE TABLE reindex_temp_before ASSELECT oid, relname, relfilenode, relkind, reltoastrelid  FROM pg_class  WHERE relname IN ('concur_temp_ind_1', 'concur_temp_ind_2');
SELECT pg_my_temp_schema()::regnamespace as temp_schema_name \gsetREINDEX SCHEMA CONCURRENTLY :temp_schema_name;
...
REINDEX SCHEMA schema_to_reindex; CREATE SCHEMA schema_to_reindex;
...
REINDEX SCHEMA schema_to_reindex;
BEGIN;
REINDEX SCHEMA schema_to_reindex; END;
